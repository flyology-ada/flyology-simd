...
package body Flyology_SIMD.Wide is
   ...
   function Splat (Value : U8) return U8x32 is
     ((Low => Flyology_SIMD.Splat (Value),
       High => Flyology_SIMD.Splat (Value)));
   ...
   function Extract (Value : U8x32; Lane : Lane_Index_8x32) return U8 is
     (if Lane < 16 then Flyology_SIMD.Extract (Value.Low, Lane) else Flyology_SIMD.Extract (Value.High, Lane - 16));
   ...
   function Shift_Left_Logical (Value : U8x32; Count : Natural) return U8x32 is
     ((Low => Flyology_SIMD.Shift_Left_Logical (Value.Low, Count),
       High => Flyology_SIMD.Shift_Left_Logical (Value.High, Count)));
   ...
   function Select_Value (Mask : Mask_8x32; If_True, If_False : U8x32) return U8x32 is
     ((Low => Flyology_SIMD.Select_Value (Mask.Low, If_True.Low, If_False.Low),
       High => Flyology_SIMD.Select_Value (Mask.High, If_True.High, If_False.High)));
   ...
   function Interleave_High (Left, Right : U8x32) return U8x32 is
     (From_Lanes ([for Lane in Lane_Index_8x32 => (if Lane mod 2 = 0 then Extract (Left, 16 + Lane / 2) else Extract (Right, 16 + Lane / 2))]));

   function Deinterleave_Even (Left, Right : U8x32) return U8x32 is
     (From_Lanes ([for Lane in Lane_Index_8x32 => (if Lane < 16 then Extract (Left, 2 * Lane) else Extract (Right, 2 * (Lane - 16)))]));

   function Deinterleave_Odd (Left, Right : U8x32) return U8x32 is
     (From_Lanes ([for Lane in Lane_Index_8x32 => (if Lane < 16 then Extract (Left, 2 * Lane + 1) else Extract (Right, 2 * (Lane - 16) + 1))]));

   function Slide_Lanes_Toward_Low (Value : U8x32; Count : Natural) return U8x32 is
     (if Count >= 32 then Zero else From_Lanes ([for Lane in Lane_Index_8x32 => (if Lane + Count < 32 then Extract (Value, Lane + Count) else 0)]));

   function Slide_Lanes_Toward_High (Value : U8x32; Count : Natural) return U8x32 is
     (if Count >= 32 then Zero else From_Lanes ([for Lane in Lane_Index_8x32 => (if Lane >= Count then Extract (Value, Lane - Count) else 0)]));
   ...
   function Mask_From_Bit_Mask (Bits : Mask_Bits_8x32) return Mask_8x32 is
     ((Low => Flyology_SIMD.Mask_From_Bit_Mask (Interfaces.Unsigned_16 (Bits and Mask_Bits_8x32 (65535))),
       High => Flyology_SIMD.Mask_From_Bit_Mask (Interfaces.Unsigned_16 (Interfaces.Shift_Right (Bits, 16)))));

   function To_Bit_Mask (Mask : Mask_8x32) return Mask_Bits_8x32 is
     (Mask_Bits_8x32 (Flyology_SIMD.To_Bit_Mask (Mask.Low)) or Interfaces.Shift_Left (Mask_Bits_8x32 (Flyology_SIMD.To_Bit_Mask (Mask.High)), 16));

   function Mask_And (Left, Right : Mask_8x32) return Mask_8x32 is
     ((Low => Flyology_SIMD.Mask_And (Left.Low, Right.Low),
       High => Flyology_SIMD.Mask_And (Left.High, Right.High)));

   function Mask_Or (Left, Right : Mask_8x32) return Mask_8x32 is
     ((Low => Flyology_SIMD.Mask_Or (Left.Low, Right.Low),
       High => Flyology_SIMD.Mask_Or (Left.High, Right.High)));

   function Mask_Xor (Left, Right : Mask_8x32) return Mask_8x32 is
     ((Low => Flyology_SIMD.Mask_Xor (Left.Low, Right.Low),
       High => Flyology_SIMD.Mask_Xor (Left.High, Right.High)));

   function Mask_Not (Value : Mask_8x32) return Mask_8x32 is
     ((Low => Flyology_SIMD.Mask_Not (Value.Low),
       High => Flyology_SIMD.Mask_Not (Value.High)));

   function Test (Mask : Mask_8x32; Lane : Lane_Index_8x32) return Boolean is
     (if Lane < 16 then Flyology_SIMD.Test (Mask.Low, Lane) else Flyology_SIMD.Test (Mask.High, Lane - 16));

   function Any_True (Mask : Mask_8x32) return Boolean is (Flyology_SIMD.Any_True (Mask.Low) or else Flyology_SIMD.Any_True (Mask.High));

   function All_True (Mask : Mask_8x32) return Boolean is (Flyology_SIMD.All_True (Mask.Low) and then Flyology_SIMD.All_True (Mask.High));

   function None_True (Mask : Mask_8x32) return Boolean is (Flyology_SIMD.None_True (Mask.Low) and then Flyology_SIMD.None_True (Mask.High));

   function Population_Count (Mask : Mask_8x32) return Lane_Count_8x32 is (Lane_Count_8x32 (Flyology_SIMD.Population_Count (Mask.Low) + Flyology_SIMD.Population_Count (Mask.High)));
   ...
   function Load (Data : Byte_Array; Start : Natural) return U8x32 is
     ((Low => Flyology_SIMD.Load (Data, Start), High => Flyology_SIMD.Load (Data, Start + 16)));
   ...
   function Load_Unaligned (Data : Byte_Array; Start : Natural) return U8x32 is
     ((Low => Flyology_SIMD.Load_Unaligned (Data, Start), High => Flyology_SIMD.Load_Unaligned (Data, Start + 16)));
   ...
   function Load_Partial (Data : Byte_Array; Start : Natural; Count : Lane_Count_8x32) return U8x32 is
     (if Count <= 16
      ...
      else (Low => Flyology_SIMD.Load (Data, Start), High => Flyology_SIMD.Load_Partial (Data, Start + 16, Count - 16)));
   ...
   function Shift_Right_Arithmetic (Value : I8x32; Count : Natural) return I8x32 is
     ((Low => Flyology_SIMD.Shift_Right_Arithmetic (Value.Low, Count),
       High => Flyology_SIMD.Shift_Right_Arithmetic (Value.High, Count)));
   ...
   function Extract (Value : U16x16; Lane : Lane_Index_16x16) return U16 is
     (if Lane < 8 then Flyology_SIMD.Extract (Value.Low, Lane) else Flyology_SIMD.Extract (Value.High, Lane - 8));
   ...
   function Interleave_High (Left, Right : U16x16) return U16x16 is
     (From_Lanes ([for Lane in Lane_Index_16x16 => (if Lane mod 2 = 0 then Extract (Left, 8 + Lane / 2) else Extract (Right, 8 + Lane / 2))]));

   function Deinterleave_Even (Left, Right : U16x16) return U16x16 is
     (From_Lanes ([for Lane in Lane_Index_16x16 => (if Lane < 8 then Extract (Left, 2 * Lane) else Extract (Right, 2 * (Lane - 8)))]));

   function Deinterleave_Odd (Left, Right : U16x16) return U16x16 is
     (From_Lanes ([for Lane in Lane_Index_16x16 => (if Lane < 8 then Extract (Left, 2 * Lane + 1) else Extract (Right, 2 * (Lane - 8) + 1))]));

   function Slide_Lanes_Toward_Low (Value : U16x16; Count : Natural) return U16x16 is
     (if Count >= 16 then Zero else From_Lanes ([for Lane in Lane_Index_16x16 => (if Lane + Count < 16 then Extract (Value, Lane + Count) else 0)]));

   function Slide_Lanes_Toward_High (Value : U16x16; Count : Natural) return U16x16 is
     (if Count >= 16 then Zero else From_Lanes ([for Lane in Lane_Index_16x16 => (if Lane >= Count then Extract (Value, Lane - Count) else 0)]));
   ...
   function Mask_From_Bit_Mask (Bits : Mask_Bits_16x16) return Mask_16x16 is
     ((Low => Flyology_SIMD.Mask_From_Bit_Mask (Interfaces.Unsigned_8 (Bits and Mask_Bits_16x16 (255))),
       High => Flyology_SIMD.Mask_From_Bit_Mask (Interfaces.Unsigned_8 (Interfaces.Shift_Right (Bits, 8)))));

   function To_Bit_Mask (Mask : Mask_16x16) return Mask_Bits_16x16 is
     (Mask_Bits_16x16 (Flyology_SIMD.To_Bit_Mask (Mask.Low)) or Interfaces.Shift_Left (Mask_Bits_16x16 (Flyology_SIMD.To_Bit_Mask (Mask.High)), 8));
   ...
   function Test (Mask : Mask_16x16; Lane : Lane_Index_16x16) return Boolean is
     (if Lane < 8 then Flyology_SIMD.Test (Mask.Low, Lane) else Flyology_SIMD.Test (Mask.High, Lane - 8));

   function Any_True (Mask : Mask_16x16) return Boolean is (Flyology_SIMD.Any_True (Mask.Low) or else Flyology_SIMD.Any_True (Mask.High));

   function All_True (Mask : Mask_16x16) return Boolean is (Flyology_SIMD.All_True (Mask.Low) and then Flyology_SIMD.All_True (Mask.High));

   function None_True (Mask : Mask_16x16) return Boolean is (Flyology_SIMD.None_True (Mask.Low) and then Flyology_SIMD.None_True (Mask.High));

   function Population_Count (Mask : Mask_16x16) return Lane_Count_16x16 is (Lane_Count_16x16 (Flyology_SIMD.Population_Count (Mask.Low) + Flyology_SIMD.Population_Count (Mask.High)));
   ...
   function Load (Data : U16_Array; Start : Natural) return U16x16 is
     ((Low => Flyology_SIMD.Load (Data, Start), High => Flyology_SIMD.Load (Data, Start + 8)));
   ...
   function Load_Unaligned (Data : U16_Array; Start : Natural) return U16x16 is
     ((Low => Flyology_SIMD.Load_Unaligned (Data, Start), High => Flyology_SIMD.Load_Unaligned (Data, Start + 8)));
   ...
   function Load_Partial (Data : U16_Array; Start : Natural; Count : Lane_Count_16x16) return U16x16 is
     (if Count <= 8
      then (Low => Flyology_SIMD.Load_Partial (Data, Start, Count), High => Flyology_SIMD.Zero)
      else (Low => Flyology_SIMD.Load (Data, Start), High => Flyology_SIMD.Load_Partial (Data, Start + 8, Count - 8)));
   ...
   function Extract (Value : U32x8; Lane : Lane_Index_32x8) return U32 is
     (if Lane < 4 then Flyology_SIMD.Extract (Value.Low, Lane) else Flyology_SIMD.Extract (Value.High, Lane - 4));
   ...
   function Interleave_High (Left, Right : U32x8) return U32x8 is
     (From_Lanes ([for Lane in Lane_Index_32x8 => (if Lane mod 2 = 0 then Extract (Left, 4 + Lane / 2) else Extract (Right, 4 + Lane / 2))]));

   function Deinterleave_Even (Left, Right : U32x8) return U32x8 is
     (From_Lanes ([for Lane in Lane_Index_32x8 => (if Lane < 4 then Extract (Left, 2 * Lane) else Extract (Right, 2 * (Lane - 4)))]));

   function Deinterleave_Odd (Left, Right : U32x8) return U32x8 is
     (From_Lanes ([for Lane in Lane_Index_32x8 => (if Lane < 4 then Extract (Left, 2 * Lane + 1) else Extract (Right, 2 * (Lane - 4) + 1))]));

   function Slide_Lanes_Toward_Low (Value : U32x8; Count : Natural) return U32x8 is
     (if Count >= 8 then Zero else From_Lanes ([for Lane in Lane_Index_32x8 => (if Lane + Count < 8 then Extract (Value, Lane + Count) else 0)]));

   function Slide_Lanes_Toward_High (Value : U32x8; Count : Natural) return U32x8 is
     (if Count >= 8 then Zero else From_Lanes ([for Lane in Lane_Index_32x8 => (if Lane >= Count then Extract (Value, Lane - Count) else 0)]));
   ...
   function Mask_From_Bit_Mask (Bits : Mask_Bits_32x8) return Mask_32x8 is
     ((Low => Flyology_SIMD.Mask_From_Bit_Mask (Bits and Mask_Bits_32x8 (15)),
       High => Flyology_SIMD.Mask_From_Bit_Mask (Interfaces.Shift_Right (Bits, 4))));

   function To_Bit_Mask (Mask : Mask_32x8) return Mask_Bits_32x8 is
     (Mask_Bits_32x8 (Flyology_SIMD.To_Bit_Mask (Mask.Low)) or Interfaces.Shift_Left (Mask_Bits_32x8 (Flyology_SIMD.To_Bit_Mask (Mask.High)), 4));
   ...
   function Test (Mask : Mask_32x8; Lane : Lane_Index_32x8) return Boolean is
     (if Lane < 4 then Flyology_SIMD.Test (Mask.Low, Lane) else Flyology_SIMD.Test (Mask.High, Lane - 4));

   function Any_True (Mask : Mask_32x8) return Boolean is (Flyology_SIMD.Any_True (Mask.Low) or else Flyology_SIMD.Any_True (Mask.High));

   function All_True (Mask : Mask_32x8) return Boolean is (Flyology_SIMD.All_True (Mask.Low) and then Flyology_SIMD.All_True (Mask.High));

   function None_True (Mask : Mask_32x8) return Boolean is (Flyology_SIMD.None_True (Mask.Low) and then Flyology_SIMD.None_True (Mask.High));

   function Population_Count (Mask : Mask_32x8) return Lane_Count_32x8 is (Lane_Count_32x8 (Flyology_SIMD.Population_Count (Mask.Low) + Flyology_SIMD.Population_Count (Mask.High)));
   ...
   function Load (Data : U32_Array; Start : Natural) return U32x8 is
     ((Low => Flyology_SIMD.Load (Data, Start), High => Flyology_SIMD.Load (Data, Start + 4)));
   ...
   function Load_Unaligned (Data : U32_Array; Start : Natural) return U32x8 is
     ((Low => Flyology_SIMD.Load_Unaligned (Data, Start), High => Flyology_SIMD.Load_Unaligned (Data, Start + 4)));
   ...
   function Load_Partial (Data : U32_Array; Start : Natural; Count : Lane_Count_32x8) return U32x8 is
     (if Count <= 4
      then (Low => Flyology_SIMD.Load_Partial (Data, Start, Count), High => Flyology_SIMD.Zero)
      else (Low => Flyology_SIMD.Load (Data, Start), High => Flyology_SIMD.Load_Partial (Data, Start + 4, Count - 4)));
   ...
   function Extract (Value : U64x4; Lane : Lane_Index_64x4) return U64 is
     (if Lane < 2 then Flyology_SIMD.Extract (Value.Low, Lane) else Flyology_SIMD.Extract (Value.High, Lane - 2));
   ...
   function Interleave_High (Left, Right : U64x4) return U64x4 is
     (From_Lanes ([for Lane in Lane_Index_64x4 => (if Lane mod 2 = 0 then Extract (Left, 2 + Lane / 2) else Extract (Right, 2 + Lane / 2))]));

   function Deinterleave_Even (Left, Right : U64x4) return U64x4 is
     (From_Lanes ([for Lane in Lane_Index_64x4 => (if Lane < 2 then Extract (Left, 2 * Lane) else Extract (Right, 2 * (Lane - 2)))]));

   function Deinterleave_Odd (Left, Right : U64x4) return U64x4 is
     (From_Lanes ([for Lane in Lane_Index_64x4 => (if Lane < 2 then Extract (Left, 2 * Lane + 1) else Extract (Right, 2 * (Lane - 2) + 1))]));

   function Slide_Lanes_Toward_Low (Value : U64x4; Count : Natural) return U64x4 is
     (if Count >= 4 then Zero else From_Lanes ([for Lane in Lane_Index_64x4 => (if Lane + Count < 4 then Extract (Value, Lane + Count) else 0)]));

   function Slide_Lanes_Toward_High (Value : U64x4; Count : Natural) return U64x4 is
     (if Count >= 4 then Zero else From_Lanes ([for Lane in Lane_Index_64x4 => (if Lane >= Count then Extract (Value, Lane - Count) else 0)]));
   ...
   function Mask_From_Bit_Mask (Bits : Mask_Bits_64x4) return Mask_64x4 is
     ((Low => Flyology_SIMD.Mask_From_Bit_Mask (Bits and Mask_Bits_64x4 (3)),
       High => Flyology_SIMD.Mask_From_Bit_Mask (Interfaces.Shift_Right (Bits, 2))));

   function To_Bit_Mask (Mask : Mask_64x4) return Mask_Bits_64x4 is
     (Mask_Bits_64x4 (Flyology_SIMD.To_Bit_Mask (Mask.Low)) or Interfaces.Shift_Left (Mask_Bits_64x4 (Flyology_SIMD.To_Bit_Mask (Mask.High)), 2));
   ...
   function Test (Mask : Mask_64x4; Lane : Lane_Index_64x4) return Boolean is
     (if Lane < 2 then Flyology_SIMD.Test (Mask.Low, Lane) else Flyology_SIMD.Test (Mask.High, Lane - 2));

   function Any_True (Mask : Mask_64x4) return Boolean is (Flyology_SIMD.Any_True (Mask.Low) or else Flyology_SIMD.Any_True (Mask.High));

   function All_True (Mask : Mask_64x4) return Boolean is (Flyology_SIMD.All_True (Mask.Low) and then Flyology_SIMD.All_True (Mask.High));

   function None_True (Mask : Mask_64x4) return Boolean is (Flyology_SIMD.None_True (Mask.Low) and then Flyology_SIMD.None_True (Mask.High));

   function Population_Count (Mask : Mask_64x4) return Lane_Count_64x4 is (Lane_Count_64x4 (Flyology_SIMD.Population_Count (Mask.Low) + Flyology_SIMD.Population_Count (Mask.High)));
   ...
   function Load (Data : U64_Array; Start : Natural) return U64x4 is
     ((Low => Flyology_SIMD.Load (Data, Start), High => Flyology_SIMD.Load (Data, Start + 2)));
   ...
   function Load_Unaligned (Data : U64_Array; Start : Natural) return U64x4 is
     ((Low => Flyology_SIMD.Load_Unaligned (Data, Start), High => Flyology_SIMD.Load_Unaligned (Data, Start + 2)));
   ...
   function Load_Partial (Data : U64_Array; Start : Natural; Count : Lane_Count_64x4) return U64x4 is
     (if Count <= 2
      then (Low => Flyology_SIMD.Load_Partial (Data, Start, Count), High => Flyology_SIMD.Zero)
      else (Low => Flyology_SIMD.Load (Data, Start), High => Flyology_SIMD.Load_Partial (Data, Start + 2, Count - 2)));
   ...
   function Unordered (Left, Right : F32x8) return Mask_32x8 is
     ((Low => Flyology_SIMD.Unordered (Left.Low, Right.Low),
       High => Flyology_SIMD.Unordered (Left.High, Right.High)));
   ...
   function Slide_Lanes_Toward_Low (Value : F32x8; Count : Natural) return F32x8 is
     (if Count >= 8 then Zero else From_Lanes ([for Lane in Lane_Index_32x8 => (if Lane + Count < 8 then Extract (Value, Lane + Count) else 0.0)]));

   function Slide_Lanes_Toward_High (Value : F32x8; Count : Natural) return F32x8 is
     (if Count >= 8 then Zero else From_Lanes ([for Lane in Lane_Index_32x8 => (if Lane >= Count then Extract (Value, Lane - Count) else 0.0)]));
   ...
   function Slide_Lanes_Toward_Low (Value : F64x4; Count : Natural) return F64x4 is
     (if Count >= 4 then Zero else From_Lanes ([for Lane in Lane_Index_64x4 => (if Lane + Count < 4 then Extract (Value, Lane + Count) else 0.0)]));

   function Slide_Lanes_Toward_High (Value : F64x4; Count : Natural) return F64x4 is
     (if Count >= 4 then Zero else From_Lanes ([for Lane in Lane_Index_64x4 => (if Lane >= Count then Extract (Value, Lane - Count) else 0.0)]));
   ...
   function Narrow_Truncate (Low, High : U16x16) return U8x32 is
     ((Low => Flyology_SIMD.Narrow_Truncate (Low.Low, Low.High),
       High => Flyology_SIMD.Narrow_Truncate (High.Low, High.High)));

   function Narrow_Saturate (Low, High : U16x16) return U8x32 is
     ((Low => Flyology_SIMD.Narrow_Saturate (Low.Low, Low.High),
       High => Flyology_SIMD.Narrow_Saturate (High.Low, High.High)));
   ...
   function Narrow_Round (Low, High : F64x4) return F32x8 is
     ((Low => Flyology_SIMD.Narrow_Round (Low.Low, Low.High),
       High => Flyology_SIMD.Narrow_Round (High.Low, High.High)));

   function Convert_Round (Value : I32x8) return F32x8 is
     ((Low => Flyology_SIMD.Convert_Round (Value.Low),
       High => Flyology_SIMD.Convert_Round (Value.High)));
   ...
   function Convert_Truncate_Saturate (Value : F32x8) return I32x8 is
     ((Low => Flyology_SIMD.Convert_Truncate_Saturate (Value.Low),
       High => Flyology_SIMD.Convert_Truncate_Saturate (Value.High)));
   ...
   function Convert_Saturate (Value : I8x32) return U8x32 is
     ((Low => Flyology_SIMD.Convert_Saturate (Value.Low),
       High => Flyology_SIMD.Convert_Saturate (Value.High)));
