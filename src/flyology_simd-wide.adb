...
package body Flyology_SIMD.Wide is
   ...
   function Zero return U8x32 is
     ((Low => Flyology_SIMD.Zero, High => Flyology_SIMD.Zero)) with SPARK_Mode => On;

   function Splat (Value : U8) return U8x32 is
     ((Low => Flyology_SIMD.Splat (Value),
       High => Flyology_SIMD.Splat (Value))) with SPARK_Mode => On;

   function From_Lanes (Values : Lane_Values_U8x32) return U8x32 is
     ((Low => Flyology_SIMD.From_Lanes ([for Lane in 0 .. 15 => Values (Lane)]),
       High => Flyology_SIMD.From_Lanes ([for Lane in 0 .. 15 => Values (Lane + 16)]))) with SPARK_Mode => On;

   function To_Lanes (Value : U8x32) return Lane_Values_U8x32 with SPARK_Mode => On is
   ...
   function Extract (Value : U8x32; Lane : Lane_Index_8x32) return U8 is
     (if Lane < 16 then Flyology_SIMD.Extract (Value.Low, Lane) else Flyology_SIMD.Extract (Value.High, Lane - 16)) with SPARK_Mode => On;
   ...
   function Replace (Value : U8x32; Lane : Lane_Index_8x32; With_Value : U8) return U8x32 is
     (if Lane < 16
      ...
      else (Low => Value.Low, High => Flyology_SIMD.Replace (Value.High, Lane - 16, With_Value))) with SPARK_Mode => On;

   function Table_Lookup (Table, Indices : U8x32) return U8x32 with SPARK_Mode => On is
   ...
   function Horizontal_Sum (Value : U8x32) return Natural with SPARK_Mode => On is
   ...
   function Bit_Cast (Value : U8x32) return I8x32 is
     ((Low => Flyology_SIMD.Bit_Cast (Value.Low), High => Flyology_SIMD.Bit_Cast (Value.High))) with SPARK_Mode => On;

   function Add_Wrap (Left, Right : U8x32) return U8x32 is
     ((Low => Flyology_SIMD.Add_Wrap (Left.Low, Right.Low),
       High => Flyology_SIMD.Add_Wrap (Left.High, Right.High))) with SPARK_Mode => On;

   function Subtract_Wrap (Left, Right : U8x32) return U8x32 is
     ((Low => Flyology_SIMD.Subtract_Wrap (Left.Low, Right.Low),
       High => Flyology_SIMD.Subtract_Wrap (Left.High, Right.High))) with SPARK_Mode => On;

   function Multiply_Wrap (Left, Right : U8x32) return U8x32 is
     ((Low => Flyology_SIMD.Multiply_Wrap (Left.Low, Right.Low),
       High => Flyology_SIMD.Multiply_Wrap (Left.High, Right.High))) with SPARK_Mode => On;

   function Add_Saturate (Left, Right : U8x32) return U8x32 is
     ((Low => Flyology_SIMD.Add_Saturate (Left.Low, Right.Low),
       High => Flyology_SIMD.Add_Saturate (Left.High, Right.High))) with SPARK_Mode => On;

   function Subtract_Saturate (Left, Right : U8x32) return U8x32 is
     ((Low => Flyology_SIMD.Subtract_Saturate (Left.Low, Right.Low),
       High => Flyology_SIMD.Subtract_Saturate (Left.High, Right.High))) with SPARK_Mode => On;

   function Bitwise_And (Left, Right : U8x32) return U8x32 is
     ((Low => Flyology_SIMD.Bitwise_And (Left.Low, Right.Low),
       High => Flyology_SIMD.Bitwise_And (Left.High, Right.High))) with SPARK_Mode => On;

   function Bitwise_Or (Left, Right : U8x32) return U8x32 is
     ((Low => Flyology_SIMD.Bitwise_Or (Left.Low, Right.Low),
       High => Flyology_SIMD.Bitwise_Or (Left.High, Right.High))) with SPARK_Mode => On;

   function Bitwise_Xor (Left, Right : U8x32) return U8x32 is
     ((Low => Flyology_SIMD.Bitwise_Xor (Left.Low, Right.Low),
       High => Flyology_SIMD.Bitwise_Xor (Left.High, Right.High))) with SPARK_Mode => On;

   function Min (Left, Right : U8x32) return U8x32 is
     ((Low => Flyology_SIMD.Min (Left.Low, Right.Low),
       High => Flyology_SIMD.Min (Left.High, Right.High))) with SPARK_Mode => On;

   function Max (Left, Right : U8x32) return U8x32 is
     ((Low => Flyology_SIMD.Max (Left.Low, Right.Low),
       High => Flyology_SIMD.Max (Left.High, Right.High))) with SPARK_Mode => On;

   function Bitwise_Not (Value : U8x32) return U8x32 is
     ((Low => Flyology_SIMD.Bitwise_Not (Value.Low),
       High => Flyology_SIMD.Bitwise_Not (Value.High))) with SPARK_Mode => On;

   function Shift_Left_Logical (Value : U8x32; Count : Natural) return U8x32 is
     ((Low => Flyology_SIMD.Shift_Left_Logical (Value.Low, Count),
       High => Flyology_SIMD.Shift_Left_Logical (Value.High, Count))) with SPARK_Mode => On;

   function Shift_Right_Logical (Value : U8x32; Count : Natural) return U8x32 is
     ((Low => Flyology_SIMD.Shift_Right_Logical (Value.Low, Count),
       High => Flyology_SIMD.Shift_Right_Logical (Value.High, Count))) with SPARK_Mode => On;

   function Equal (Left, Right : U8x32) return Mask_8x32 is
     ((Low => Flyology_SIMD.Equal (Left.Low, Right.Low),
       High => Flyology_SIMD.Equal (Left.High, Right.High))) with SPARK_Mode => On;

   function Less_Than (Left, Right : U8x32) return Mask_8x32 is
     ((Low => Flyology_SIMD.Less_Than (Left.Low, Right.Low),
       High => Flyology_SIMD.Less_Than (Left.High, Right.High))) with SPARK_Mode => On;

   function Less_Equal (Left, Right : U8x32) return Mask_8x32 is
     ((Low => Flyology_SIMD.Less_Equal (Left.Low, Right.Low),
       High => Flyology_SIMD.Less_Equal (Left.High, Right.High))) with SPARK_Mode => On;

   function Greater_Than (Left, Right : U8x32) return Mask_8x32 is
     ((Low => Flyology_SIMD.Greater_Than (Left.Low, Right.Low),
       High => Flyology_SIMD.Greater_Than (Left.High, Right.High))) with SPARK_Mode => On;

   function Greater_Equal (Left, Right : U8x32) return Mask_8x32 is
     ((Low => Flyology_SIMD.Greater_Equal (Left.Low, Right.Low),
       High => Flyology_SIMD.Greater_Equal (Left.High, Right.High))) with SPARK_Mode => On;

   function Select_Value (Mask : Mask_8x32; If_True, If_False : U8x32) return U8x32 is
     ((Low => Flyology_SIMD.Select_Value (Mask.Low, If_True.Low, If_False.Low),
       High => Flyology_SIMD.Select_Value (Mask.High, If_True.High, If_False.High))) with SPARK_Mode => On;

   function Compress (Value : U8x32; Mask : Mask_8x32) return U8x32 with SPARK_Mode => On is
      Result : Lane_Values_U8x32 := [others => 0];
      Next : Natural := 0;
   begin
      for Lane in Lane_Index_8x32 loop
   ...
   function Expand (Value : U8x32; Mask : Mask_8x32) return U8x32 with SPARK_Mode => On is
      Result : Lane_Values_U8x32 := [others => 0];
      Next : Natural := 0;
   begin
      for Lane in Lane_Index_8x32 loop
   ...
   function Reduce_Add_Wrap (Value : U8x32) return U8 with SPARK_Mode => On is
   ...
   function Reduce_Min (Value : U8x32) return U8 with SPARK_Mode => On is
   ...
   function Reduce_Max (Value : U8x32) return U8 with SPARK_Mode => On is
   ...
   function Reverse_Lanes (Value : U8x32) return U8x32 is
     (From_Lanes ([for Lane in Lane_Index_8x32 => Extract (Value, 31 - Lane)])) with SPARK_Mode => On;

   function Permute_Lanes (Value : U8x32; Map : Lane_Map_8x32) return U8x32 is
     (From_Lanes ([for Lane in Lane_Index_8x32 => Extract (Value, Map.Selectors (Lane))])) with SPARK_Mode => On;

   function Permute_Lanes (Left, Right : U8x32; Map : Two_Source_Lane_Map_8x32) return U8x32 is
     (From_Lanes ([for Lane in Lane_Index_8x32 => Extract ((if Map.Selectors (Lane).From_Right then Right else Left), Map.Selectors (Lane).Lane)])) with SPARK_Mode => On;

   function Interleave_Low (Left, Right : U8x32) return U8x32 is
     (From_Lanes ([for Lane in Lane_Index_8x32 => (if Lane mod 2 = 0 then Extract (Left, Lane / 2) else Extract (Right, Lane / 2))])) with SPARK_Mode => On;

   function Interleave_High (Left, Right : U8x32) return U8x32 is
     (From_Lanes ([for Lane in Lane_Index_8x32 => (if Lane mod 2 = 0 then Extract (Left, 16 + Lane / 2) else Extract (Right, 16 + Lane / 2))])) with SPARK_Mode => On;

   function Deinterleave_Even (Left, Right : U8x32) return U8x32 is
     (From_Lanes ([for Lane in Lane_Index_8x32 => (if Lane < 16 then Extract (Left, 2 * Lane) else Extract (Right, 2 * (Lane - 16)))])) with SPARK_Mode => On;

   function Deinterleave_Odd (Left, Right : U8x32) return U8x32 is
     (From_Lanes ([for Lane in Lane_Index_8x32 => (if Lane < 16 then Extract (Left, 2 * Lane + 1) else Extract (Right, 2 * (Lane - 16) + 1))])) with SPARK_Mode => On;

   function Slide_Lanes_Toward_Low (Value : U8x32; Count : Natural) return U8x32 is
     (if Count >= 32 then Zero else From_Lanes ([for Lane in Lane_Index_8x32 => (if Lane + Count < 32 then Extract (Value, Lane + Count) else 0)])) with SPARK_Mode => On;

   function Slide_Lanes_Toward_High (Value : U8x32; Count : Natural) return U8x32 is
     (if Count >= 32 then Zero else From_Lanes ([for Lane in Lane_Index_8x32 => (if Lane >= Count then Extract (Value, Lane - Count) else 0)])) with SPARK_Mode => On;

   function Mask_From_Bit_Mask (Bits : Mask_Bits_8x32) return Mask_8x32 is
     ((Low => Flyology_SIMD.Mask_From_Bit_Mask (Interfaces.Unsigned_16 (Bits and Mask_Bits_8x32 (65535))),
       High => Flyology_SIMD.Mask_From_Bit_Mask (Interfaces.Unsigned_16 (Interfaces.Shift_Right (Bits, 16))))) with SPARK_Mode => On;

   function To_Bit_Mask (Mask : Mask_8x32) return Mask_Bits_8x32 is
     (Mask_Bits_8x32 (Flyology_SIMD.To_Bit_Mask (Mask.Low)) or Interfaces.Shift_Left (Mask_Bits_8x32 (Flyology_SIMD.To_Bit_Mask (Mask.High)), 16)) with SPARK_Mode => On;

   function Mask_And (Left, Right : Mask_8x32) return Mask_8x32 is
     ((Low => Flyology_SIMD.Mask_And (Left.Low, Right.Low),
       High => Flyology_SIMD.Mask_And (Left.High, Right.High))) with SPARK_Mode => On;

   function Mask_Or (Left, Right : Mask_8x32) return Mask_8x32 is
     ((Low => Flyology_SIMD.Mask_Or (Left.Low, Right.Low),
       High => Flyology_SIMD.Mask_Or (Left.High, Right.High))) with SPARK_Mode => On;

   function Mask_Xor (Left, Right : Mask_8x32) return Mask_8x32 is
     ((Low => Flyology_SIMD.Mask_Xor (Left.Low, Right.Low),
       High => Flyology_SIMD.Mask_Xor (Left.High, Right.High))) with SPARK_Mode => On;

   function Mask_Not (Value : Mask_8x32) return Mask_8x32 is
     ((Low => Flyology_SIMD.Mask_Not (Value.Low),
       High => Flyology_SIMD.Mask_Not (Value.High))) with SPARK_Mode => On;

   function Test (Mask : Mask_8x32; Lane : Lane_Index_8x32) return Boolean is
     (if Lane < 16 then Flyology_SIMD.Test (Mask.Low, Lane) else Flyology_SIMD.Test (Mask.High, Lane - 16)) with SPARK_Mode => On;

   function Any_True (Mask : Mask_8x32) return Boolean is (Flyology_SIMD.Any_True (Mask.Low) or else Flyology_SIMD.Any_True (Mask.High)) with SPARK_Mode => On;

   function All_True (Mask : Mask_8x32) return Boolean is (Flyology_SIMD.All_True (Mask.Low) and then Flyology_SIMD.All_True (Mask.High)) with SPARK_Mode => On;

   function None_True (Mask : Mask_8x32) return Boolean is (Flyology_SIMD.None_True (Mask.Low) and then Flyology_SIMD.None_True (Mask.High)) with SPARK_Mode => On;

   function Population_Count (Mask : Mask_8x32) return Lane_Count_8x32 is (Lane_Count_8x32 (Flyology_SIMD.Population_Count (Mask.Low) + Flyology_SIMD.Population_Count (Mask.High))) with SPARK_Mode => On;

   function First_True (Mask : Mask_8x32) return Lane_Count_8x32 with SPARK_Mode => On is
      Low : constant Natural := Flyology_SIMD.First_True (Mask.Low);
      High : constant Natural := Flyology_SIMD.First_True (Mask.High);
   begin
      return (if Low < 16 then Low elsif High < 16 then 16 + High else 32);
   end First_True;

   function Last_True (Mask : Mask_8x32) return Lane_Count_8x32 with SPARK_Mode => On is
      Low : constant Natural := Flyology_SIMD.Last_True (Mask.Low);
      High : constant Natural := Flyology_SIMD.Last_True (Mask.High);
   begin
   ...
   function Load (Data : Byte_Array; Start : Natural) return U8x32 is
     ((Low => Flyology_SIMD.Load (Data, Start), High => Flyology_SIMD.Load (Data, Start + 16))) with SPARK_Mode => On;

   procedure Store (Data : in out Byte_Array; Start : Natural; Value : U8x32) with SPARK_Mode => On is
   begin
      Flyology_SIMD.Store (Data, Start, Value.Low); Flyology_SIMD.Store (Data, Start + 16, Value.High);
   end Store;

   function Load_Unaligned (Data : Byte_Array; Start : Natural) return U8x32 is
     ((Low => Flyology_SIMD.Load_Unaligned (Data, Start), High => Flyology_SIMD.Load_Unaligned (Data, Start + 16))) with SPARK_Mode => On;

   procedure Store_Unaligned (Data : in out Byte_Array; Start : Natural; Value : U8x32) with SPARK_Mode => On is
   begin
      Flyology_SIMD.Store_Unaligned (Data, Start, Value.Low); Flyology_SIMD.Store_Unaligned (Data, Start + 16, Value.High);
   end Store_Unaligned;

   function Load_Aligned (Data : Byte_Array; Start : Natural) return U8x32 is
     ((Low => Flyology_SIMD.Load_Aligned (Data, Start), High => Flyology_SIMD.Load_Aligned (Data, Start + 16)));

   procedure Store_Aligned (Data : in out Byte_Array; Start : Natural; Value : U8x32) is
   begin
      Flyology_SIMD.Store_Aligned (Data, Start, Value.Low); Flyology_SIMD.Store_Aligned (Data, Start + 16, Value.High);
   end Store_Aligned;

   function Load_Partial (Data : Byte_Array; Start : Natural; Count : Lane_Count_8x32) return U8x32 is
     (if Count <= 16
      then (Low => Flyology_SIMD.Load_Partial (Data, Start, Count), High => Flyology_SIMD.Zero)
      else (Low => Flyology_SIMD.Load (Data, Start), High => Flyology_SIMD.Load_Partial (Data, Start + 16, Count - 16))) with SPARK_Mode => On;

   procedure Store_Partial (Data : in out Byte_Array; Start : Natural; Count : Lane_Count_8x32; Value : U8x32) with SPARK_Mode => On is
   begin
      if Count <= 16 then Flyology_SIMD.Store_Partial (Data, Start, Count, Value.Low);
      else Flyology_SIMD.Store (Data, Start, Value.Low); Flyology_SIMD.Store_Partial (Data, Start + 16, Count - 16, Value.High); end if;
   end Store_Partial;
   ...
   function To_Lanes (Value : I8x32) return Lane_Values_I8x32 with SPARK_Mode => On is
   ...
   function Shift_Right_Arithmetic (Value : I8x32; Count : Natural) return I8x32 is
     ((Low => Flyology_SIMD.Shift_Right_Arithmetic (Value.Low, Count),
       High => Flyology_SIMD.Shift_Right_Arithmetic (Value.High, Count))) with SPARK_Mode => On;
   ...
   function Compress (Value : I8x32; Mask : Mask_8x32) return I8x32 with SPARK_Mode => On is
   ...
   function Expand (Value : I8x32; Mask : Mask_8x32) return I8x32 with SPARK_Mode => On is
      Result : Lane_Values_I8x32 := [others => 0];
      Next : Natural := 0;
   begin
      for Lane in Lane_Index_8x32 loop
   ...
   function Reduce_Add_Wrap (Value : I8x32) return I8 with SPARK_Mode => On is
   ...
   function Reduce_Min (Value : I8x32) return I8 with SPARK_Mode => On is
   ...
   function Reduce_Max (Value : I8x32) return I8 with SPARK_Mode => On is
   ...
   procedure Store (Data : in out I8_Array; Start : Natural; Value : I8x32) with SPARK_Mode => On is
   begin
      Flyology_SIMD.Store (Data, Start, Value.Low); Flyology_SIMD.Store (Data, Start + 16, Value.High);
   end Store;
   ...
   procedure Store_Unaligned (Data : in out I8_Array; Start : Natural; Value : I8x32) with SPARK_Mode => On is
   begin
      Flyology_SIMD.Store_Unaligned (Data, Start, Value.Low); Flyology_SIMD.Store_Unaligned (Data, Start + 16, Value.High);
   end Store_Unaligned;
   ...
   procedure Store_Partial (Data : in out I8_Array; Start : Natural; Count : Lane_Count_8x32; Value : I8x32) with SPARK_Mode => On is
   begin
      if Count <= 16 then Flyology_SIMD.Store_Partial (Data, Start, Count, Value.Low);
      else Flyology_SIMD.Store (Data, Start, Value.Low); Flyology_SIMD.Store_Partial (Data, Start + 16, Count - 16, Value.High); end if;
   end Store_Partial;
   ...
   function From_Lanes (Values : Lane_Values_U16x16) return U16x16 is
     ((Low => Flyology_SIMD.From_Lanes ([for Lane in 0 .. 7 => Values (Lane)]),
       High => Flyology_SIMD.From_Lanes ([for Lane in 0 .. 7 => Values (Lane + 8)]))) with SPARK_Mode => On;

   function To_Lanes (Value : U16x16) return Lane_Values_U16x16 with SPARK_Mode => On is
   ...
   function Extract (Value : U16x16; Lane : Lane_Index_16x16) return U16 is
     (if Lane < 8 then Flyology_SIMD.Extract (Value.Low, Lane) else Flyology_SIMD.Extract (Value.High, Lane - 8)) with SPARK_Mode => On;

   function Replace (Value : U16x16; Lane : Lane_Index_16x16; With_Value : U16) return U16x16 is
     (if Lane < 8
      then (Low => Flyology_SIMD.Replace (Value.Low, Lane, With_Value), High => Value.High)
      else (Low => Value.Low, High => Flyology_SIMD.Replace (Value.High, Lane - 8, With_Value))) with SPARK_Mode => On;
   ...
   function Compress (Value : U16x16; Mask : Mask_16x16) return U16x16 with SPARK_Mode => On is
   ...
   function Expand (Value : U16x16; Mask : Mask_16x16) return U16x16 with SPARK_Mode => On is
      Result : Lane_Values_U16x16 := [others => 0];
      Next : Natural := 0;
   begin
      for Lane in Lane_Index_16x16 loop
   ...
   function Reduce_Add_Wrap (Value : U16x16) return U16 with SPARK_Mode => On is
   ...
   function Reduce_Min (Value : U16x16) return U16 with SPARK_Mode => On is
   ...
   function Reduce_Max (Value : U16x16) return U16 with SPARK_Mode => On is
   ...
   function Reverse_Lanes (Value : U16x16) return U16x16 is
     (From_Lanes ([for Lane in Lane_Index_16x16 => Extract (Value, 15 - Lane)])) with SPARK_Mode => On;

   function Permute_Lanes (Value : U16x16; Map : Lane_Map_16x16) return U16x16 is
     (From_Lanes ([for Lane in Lane_Index_16x16 => Extract (Value, Map.Selectors (Lane))])) with SPARK_Mode => On;

   function Permute_Lanes (Left, Right : U16x16; Map : Two_Source_Lane_Map_16x16) return U16x16 is
     (From_Lanes ([for Lane in Lane_Index_16x16 => Extract ((if Map.Selectors (Lane).From_Right then Right else Left), Map.Selectors (Lane).Lane)])) with SPARK_Mode => On;

   function Interleave_Low (Left, Right : U16x16) return U16x16 is
     (From_Lanes ([for Lane in Lane_Index_16x16 => (if Lane mod 2 = 0 then Extract (Left, Lane / 2) else Extract (Right, Lane / 2))])) with SPARK_Mode => On;

   function Interleave_High (Left, Right : U16x16) return U16x16 is
     (From_Lanes ([for Lane in Lane_Index_16x16 => (if Lane mod 2 = 0 then Extract (Left, 8 + Lane / 2) else Extract (Right, 8 + Lane / 2))])) with SPARK_Mode => On;

   function Deinterleave_Even (Left, Right : U16x16) return U16x16 is
     (From_Lanes ([for Lane in Lane_Index_16x16 => (if Lane < 8 then Extract (Left, 2 * Lane) else Extract (Right, 2 * (Lane - 8)))])) with SPARK_Mode => On;

   function Deinterleave_Odd (Left, Right : U16x16) return U16x16 is
     (From_Lanes ([for Lane in Lane_Index_16x16 => (if Lane < 8 then Extract (Left, 2 * Lane + 1) else Extract (Right, 2 * (Lane - 8) + 1))])) with SPARK_Mode => On;

   function Slide_Lanes_Toward_Low (Value : U16x16; Count : Natural) return U16x16 is
     (if Count >= 16 then Zero else From_Lanes ([for Lane in Lane_Index_16x16 => (if Lane + Count < 16 then Extract (Value, Lane + Count) else 0)])) with SPARK_Mode => On;

   function Slide_Lanes_Toward_High (Value : U16x16; Count : Natural) return U16x16 is
     (if Count >= 16 then Zero else From_Lanes ([for Lane in Lane_Index_16x16 => (if Lane >= Count then Extract (Value, Lane - Count) else 0)])) with SPARK_Mode => On;

   function Mask_From_Bit_Mask (Bits : Mask_Bits_16x16) return Mask_16x16 is
     ((Low => Flyology_SIMD.Mask_From_Bit_Mask (Interfaces.Unsigned_8 (Bits and Mask_Bits_16x16 (255))),
       High => Flyology_SIMD.Mask_From_Bit_Mask (Interfaces.Unsigned_8 (Interfaces.Shift_Right (Bits, 8))))) with SPARK_Mode => On;

   function To_Bit_Mask (Mask : Mask_16x16) return Mask_Bits_16x16 is
     (Mask_Bits_16x16 (Flyology_SIMD.To_Bit_Mask (Mask.Low)) or Interfaces.Shift_Left (Mask_Bits_16x16 (Flyology_SIMD.To_Bit_Mask (Mask.High)), 8)) with SPARK_Mode => On;
   ...
   function Test (Mask : Mask_16x16; Lane : Lane_Index_16x16) return Boolean is
     (if Lane < 8 then Flyology_SIMD.Test (Mask.Low, Lane) else Flyology_SIMD.Test (Mask.High, Lane - 8)) with SPARK_Mode => On;

   function Any_True (Mask : Mask_16x16) return Boolean is (Flyology_SIMD.Any_True (Mask.Low) or else Flyology_SIMD.Any_True (Mask.High)) with SPARK_Mode => On;

   function All_True (Mask : Mask_16x16) return Boolean is (Flyology_SIMD.All_True (Mask.Low) and then Flyology_SIMD.All_True (Mask.High)) with SPARK_Mode => On;

   function None_True (Mask : Mask_16x16) return Boolean is (Flyology_SIMD.None_True (Mask.Low) and then Flyology_SIMD.None_True (Mask.High)) with SPARK_Mode => On;

   function Population_Count (Mask : Mask_16x16) return Lane_Count_16x16 is (Lane_Count_16x16 (Flyology_SIMD.Population_Count (Mask.Low) + Flyology_SIMD.Population_Count (Mask.High))) with SPARK_Mode => On;

   function First_True (Mask : Mask_16x16) return Lane_Count_16x16 with SPARK_Mode => On is
      Low : constant Natural := Flyology_SIMD.First_True (Mask.Low);
      High : constant Natural := Flyology_SIMD.First_True (Mask.High);
   begin
   ...
   function Last_True (Mask : Mask_16x16) return Lane_Count_16x16 with SPARK_Mode => On is
      Low : constant Natural := Flyology_SIMD.Last_True (Mask.Low);
      High : constant Natural := Flyology_SIMD.Last_True (Mask.High);
   begin
   ...
   function Load (Data : U16_Array; Start : Natural) return U16x16 is
     ((Low => Flyology_SIMD.Load (Data, Start), High => Flyology_SIMD.Load (Data, Start + 8))) with SPARK_Mode => On;

   procedure Store (Data : in out U16_Array; Start : Natural; Value : U16x16) with SPARK_Mode => On is
   begin
      Flyology_SIMD.Store (Data, Start, Value.Low); Flyology_SIMD.Store (Data, Start + 8, Value.High);
   end Store;

   function Load_Unaligned (Data : U16_Array; Start : Natural) return U16x16 is
     ((Low => Flyology_SIMD.Load_Unaligned (Data, Start), High => Flyology_SIMD.Load_Unaligned (Data, Start + 8))) with SPARK_Mode => On;

   procedure Store_Unaligned (Data : in out U16_Array; Start : Natural; Value : U16x16) with SPARK_Mode => On is
   begin
      Flyology_SIMD.Store_Unaligned (Data, Start, Value.Low); Flyology_SIMD.Store_Unaligned (Data, Start + 8, Value.High);
   end Store_Unaligned;

   function Load_Aligned (Data : U16_Array; Start : Natural) return U16x16 is
     ((Low => Flyology_SIMD.Load_Aligned (Data, Start), High => Flyology_SIMD.Load_Aligned (Data, Start + 8)));

   procedure Store_Aligned (Data : in out U16_Array; Start : Natural; Value : U16x16) is
   begin
      Flyology_SIMD.Store_Aligned (Data, Start, Value.Low); Flyology_SIMD.Store_Aligned (Data, Start + 8, Value.High);
   end Store_Aligned;

   function Load_Partial (Data : U16_Array; Start : Natural; Count : Lane_Count_16x16) return U16x16 is
     (if Count <= 8
      then (Low => Flyology_SIMD.Load_Partial (Data, Start, Count), High => Flyology_SIMD.Zero)
      else (Low => Flyology_SIMD.Load (Data, Start), High => Flyology_SIMD.Load_Partial (Data, Start + 8, Count - 8))) with SPARK_Mode => On;

   procedure Store_Partial (Data : in out U16_Array; Start : Natural; Count : Lane_Count_16x16; Value : U16x16) with SPARK_Mode => On is
   begin
      if Count <= 8 then Flyology_SIMD.Store_Partial (Data, Start, Count, Value.Low);
      else Flyology_SIMD.Store (Data, Start, Value.Low); Flyology_SIMD.Store_Partial (Data, Start + 8, Count - 8, Value.High); end if;
   end Store_Partial;
   ...
   function To_Lanes (Value : I16x16) return Lane_Values_I16x16 with SPARK_Mode => On is
   ...
   function Compress (Value : I16x16; Mask : Mask_16x16) return I16x16 with SPARK_Mode => On is
   ...
   function Expand (Value : I16x16; Mask : Mask_16x16) return I16x16 with SPARK_Mode => On is
      Result : Lane_Values_I16x16 := [others => 0];
      Next : Natural := 0;
   begin
      for Lane in Lane_Index_16x16 loop
   ...
   function Reduce_Add_Wrap (Value : I16x16) return I16 with SPARK_Mode => On is
   ...
   function Reduce_Min (Value : I16x16) return I16 with SPARK_Mode => On is
   ...
   function Reduce_Max (Value : I16x16) return I16 with SPARK_Mode => On is
   ...
   procedure Store (Data : in out I16_Array; Start : Natural; Value : I16x16) with SPARK_Mode => On is
   begin
      Flyology_SIMD.Store (Data, Start, Value.Low); Flyology_SIMD.Store (Data, Start + 8, Value.High);
   end Store;
   ...
   procedure Store_Unaligned (Data : in out I16_Array; Start : Natural; Value : I16x16) with SPARK_Mode => On is
   begin
      Flyology_SIMD.Store_Unaligned (Data, Start, Value.Low); Flyology_SIMD.Store_Unaligned (Data, Start + 8, Value.High);
   end Store_Unaligned;
   ...
   procedure Store_Partial (Data : in out I16_Array; Start : Natural; Count : Lane_Count_16x16; Value : I16x16) with SPARK_Mode => On is
   begin
      if Count <= 8 then Flyology_SIMD.Store_Partial (Data, Start, Count, Value.Low);
      else Flyology_SIMD.Store (Data, Start, Value.Low); Flyology_SIMD.Store_Partial (Data, Start + 8, Count - 8, Value.High); end if;
   end Store_Partial;
   ...
   function From_Lanes (Values : Lane_Values_U32x8) return U32x8 is
     ((Low => Flyology_SIMD.From_Lanes ([for Lane in 0 .. 3 => Values (Lane)]),
       High => Flyology_SIMD.From_Lanes ([for Lane in 0 .. 3 => Values (Lane + 4)]))) with SPARK_Mode => On;

   function To_Lanes (Value : U32x8) return Lane_Values_U32x8 with SPARK_Mode => On is
   ...
   function Extract (Value : U32x8; Lane : Lane_Index_32x8) return U32 is
     (if Lane < 4 then Flyology_SIMD.Extract (Value.Low, Lane) else Flyology_SIMD.Extract (Value.High, Lane - 4)) with SPARK_Mode => On;

   function Replace (Value : U32x8; Lane : Lane_Index_32x8; With_Value : U32) return U32x8 is
     (if Lane < 4
      then (Low => Flyology_SIMD.Replace (Value.Low, Lane, With_Value), High => Value.High)
      else (Low => Value.Low, High => Flyology_SIMD.Replace (Value.High, Lane - 4, With_Value))) with SPARK_Mode => On;
   ...
   function Compress (Value : U32x8; Mask : Mask_32x8) return U32x8 with SPARK_Mode => On is
   ...
   function Expand (Value : U32x8; Mask : Mask_32x8) return U32x8 with SPARK_Mode => On is
      Result : Lane_Values_U32x8 := [others => 0];
      Next : Natural := 0;
   begin
      for Lane in Lane_Index_32x8 loop
   ...
   function Reduce_Add_Wrap (Value : U32x8) return U32 with SPARK_Mode => On is
   ...
   function Reduce_Min (Value : U32x8) return U32 with SPARK_Mode => On is
   ...
   function Reduce_Max (Value : U32x8) return U32 with SPARK_Mode => On is
   ...
   function Reverse_Lanes (Value : U32x8) return U32x8 is
     (From_Lanes ([for Lane in Lane_Index_32x8 => Extract (Value, 7 - Lane)])) with SPARK_Mode => On;

   function Permute_Lanes (Value : U32x8; Map : Lane_Map_32x8) return U32x8 is
     (From_Lanes ([for Lane in Lane_Index_32x8 => Extract (Value, Map.Selectors (Lane))])) with SPARK_Mode => On;

   function Permute_Lanes (Left, Right : U32x8; Map : Two_Source_Lane_Map_32x8) return U32x8 is
     (From_Lanes ([for Lane in Lane_Index_32x8 => Extract ((if Map.Selectors (Lane).From_Right then Right else Left), Map.Selectors (Lane).Lane)])) with SPARK_Mode => On;

   function Interleave_Low (Left, Right : U32x8) return U32x8 is
     (From_Lanes ([for Lane in Lane_Index_32x8 => (if Lane mod 2 = 0 then Extract (Left, Lane / 2) else Extract (Right, Lane / 2))])) with SPARK_Mode => On;

   function Interleave_High (Left, Right : U32x8) return U32x8 is
     (From_Lanes ([for Lane in Lane_Index_32x8 => (if Lane mod 2 = 0 then Extract (Left, 4 + Lane / 2) else Extract (Right, 4 + Lane / 2))])) with SPARK_Mode => On;

   function Deinterleave_Even (Left, Right : U32x8) return U32x8 is
     (From_Lanes ([for Lane in Lane_Index_32x8 => (if Lane < 4 then Extract (Left, 2 * Lane) else Extract (Right, 2 * (Lane - 4)))])) with SPARK_Mode => On;

   function Deinterleave_Odd (Left, Right : U32x8) return U32x8 is
     (From_Lanes ([for Lane in Lane_Index_32x8 => (if Lane < 4 then Extract (Left, 2 * Lane + 1) else Extract (Right, 2 * (Lane - 4) + 1))])) with SPARK_Mode => On;

   function Slide_Lanes_Toward_Low (Value : U32x8; Count : Natural) return U32x8 is
     (if Count >= 8 then Zero else From_Lanes ([for Lane in Lane_Index_32x8 => (if Lane + Count < 8 then Extract (Value, Lane + Count) else 0)])) with SPARK_Mode => On;

   function Slide_Lanes_Toward_High (Value : U32x8; Count : Natural) return U32x8 is
     (if Count >= 8 then Zero else From_Lanes ([for Lane in Lane_Index_32x8 => (if Lane >= Count then Extract (Value, Lane - Count) else 0)])) with SPARK_Mode => On;

   function Mask_From_Bit_Mask (Bits : Mask_Bits_32x8) return Mask_32x8 is
     ((Low => Flyology_SIMD.Mask_From_Bit_Mask (Bits and Mask_Bits_32x8 (15)),
       High => Flyology_SIMD.Mask_From_Bit_Mask (Interfaces.Shift_Right (Bits, 4)))) with SPARK_Mode => On;

   function To_Bit_Mask (Mask : Mask_32x8) return Mask_Bits_32x8 is
     (Mask_Bits_32x8 (Flyology_SIMD.To_Bit_Mask (Mask.Low)) or Interfaces.Shift_Left (Mask_Bits_32x8 (Flyology_SIMD.To_Bit_Mask (Mask.High)), 4)) with SPARK_Mode => On;
   ...
   function Test (Mask : Mask_32x8; Lane : Lane_Index_32x8) return Boolean is
     (if Lane < 4 then Flyology_SIMD.Test (Mask.Low, Lane) else Flyology_SIMD.Test (Mask.High, Lane - 4)) with SPARK_Mode => On;

   function Any_True (Mask : Mask_32x8) return Boolean is (Flyology_SIMD.Any_True (Mask.Low) or else Flyology_SIMD.Any_True (Mask.High)) with SPARK_Mode => On;

   function All_True (Mask : Mask_32x8) return Boolean is (Flyology_SIMD.All_True (Mask.Low) and then Flyology_SIMD.All_True (Mask.High)) with SPARK_Mode => On;

   function None_True (Mask : Mask_32x8) return Boolean is (Flyology_SIMD.None_True (Mask.Low) and then Flyology_SIMD.None_True (Mask.High)) with SPARK_Mode => On;

   function Population_Count (Mask : Mask_32x8) return Lane_Count_32x8 is (Lane_Count_32x8 (Flyology_SIMD.Population_Count (Mask.Low) + Flyology_SIMD.Population_Count (Mask.High))) with SPARK_Mode => On;

   function First_True (Mask : Mask_32x8) return Lane_Count_32x8 with SPARK_Mode => On is
      Low : constant Natural := Flyology_SIMD.First_True (Mask.Low);
      High : constant Natural := Flyology_SIMD.First_True (Mask.High);
   begin
   ...
   function Last_True (Mask : Mask_32x8) return Lane_Count_32x8 with SPARK_Mode => On is
      Low : constant Natural := Flyology_SIMD.Last_True (Mask.Low);
      High : constant Natural := Flyology_SIMD.Last_True (Mask.High);
   begin
   ...
   function Load (Data : U32_Array; Start : Natural) return U32x8 is
     ((Low => Flyology_SIMD.Load (Data, Start), High => Flyology_SIMD.Load (Data, Start + 4))) with SPARK_Mode => On;

   procedure Store (Data : in out U32_Array; Start : Natural; Value : U32x8) with SPARK_Mode => On is
   begin
      Flyology_SIMD.Store (Data, Start, Value.Low); Flyology_SIMD.Store (Data, Start + 4, Value.High);
   end Store;

   function Load_Unaligned (Data : U32_Array; Start : Natural) return U32x8 is
     ((Low => Flyology_SIMD.Load_Unaligned (Data, Start), High => Flyology_SIMD.Load_Unaligned (Data, Start + 4))) with SPARK_Mode => On;

   procedure Store_Unaligned (Data : in out U32_Array; Start : Natural; Value : U32x8) with SPARK_Mode => On is
   begin
      Flyology_SIMD.Store_Unaligned (Data, Start, Value.Low); Flyology_SIMD.Store_Unaligned (Data, Start + 4, Value.High);
   end Store_Unaligned;

   function Load_Aligned (Data : U32_Array; Start : Natural) return U32x8 is
     ((Low => Flyology_SIMD.Load_Aligned (Data, Start), High => Flyology_SIMD.Load_Aligned (Data, Start + 4)));

   procedure Store_Aligned (Data : in out U32_Array; Start : Natural; Value : U32x8) is
   begin
      Flyology_SIMD.Store_Aligned (Data, Start, Value.Low); Flyology_SIMD.Store_Aligned (Data, Start + 4, Value.High);
   end Store_Aligned;

   function Load_Partial (Data : U32_Array; Start : Natural; Count : Lane_Count_32x8) return U32x8 is
     (if Count <= 4
      then (Low => Flyology_SIMD.Load_Partial (Data, Start, Count), High => Flyology_SIMD.Zero)
      else (Low => Flyology_SIMD.Load (Data, Start), High => Flyology_SIMD.Load_Partial (Data, Start + 4, Count - 4))) with SPARK_Mode => On;

   procedure Store_Partial (Data : in out U32_Array; Start : Natural; Count : Lane_Count_32x8; Value : U32x8) with SPARK_Mode => On is
   begin
      if Count <= 4 then Flyology_SIMD.Store_Partial (Data, Start, Count, Value.Low);
      else Flyology_SIMD.Store (Data, Start, Value.Low); Flyology_SIMD.Store_Partial (Data, Start + 4, Count - 4, Value.High); end if;
   end Store_Partial;
   ...
   function To_Lanes (Value : I32x8) return Lane_Values_I32x8 with SPARK_Mode => On is
   ...
   function Compress (Value : I32x8; Mask : Mask_32x8) return I32x8 with SPARK_Mode => On is
   ...
   function Expand (Value : I32x8; Mask : Mask_32x8) return I32x8 with SPARK_Mode => On is
      Result : Lane_Values_I32x8 := [others => 0];
      Next : Natural := 0;
   begin
      for Lane in Lane_Index_32x8 loop
   ...
   function Reduce_Add_Wrap (Value : I32x8) return I32 with SPARK_Mode => On is
   ...
   function Reduce_Min (Value : I32x8) return I32 with SPARK_Mode => On is
   ...
   function Reduce_Max (Value : I32x8) return I32 with SPARK_Mode => On is
   ...
   procedure Store (Data : in out I32_Array; Start : Natural; Value : I32x8) with SPARK_Mode => On is
   begin
      Flyology_SIMD.Store (Data, Start, Value.Low); Flyology_SIMD.Store (Data, Start + 4, Value.High);
   end Store;
   ...
   procedure Store_Unaligned (Data : in out I32_Array; Start : Natural; Value : I32x8) with SPARK_Mode => On is
   begin
      Flyology_SIMD.Store_Unaligned (Data, Start, Value.Low); Flyology_SIMD.Store_Unaligned (Data, Start + 4, Value.High);
   end Store_Unaligned;
   ...
   procedure Store_Partial (Data : in out I32_Array; Start : Natural; Count : Lane_Count_32x8; Value : I32x8) with SPARK_Mode => On is
   begin
      if Count <= 4 then Flyology_SIMD.Store_Partial (Data, Start, Count, Value.Low);
      else Flyology_SIMD.Store (Data, Start, Value.Low); Flyology_SIMD.Store_Partial (Data, Start + 4, Count - 4, Value.High); end if;
   end Store_Partial;
   ...
   function From_Lanes (Values : Lane_Values_U64x4) return U64x4 is
     ((Low => Flyology_SIMD.From_Lanes ([for Lane in 0 .. 1 => Values (Lane)]),
       High => Flyology_SIMD.From_Lanes ([for Lane in 0 .. 1 => Values (Lane + 2)]))) with SPARK_Mode => On;

   function To_Lanes (Value : U64x4) return Lane_Values_U64x4 with SPARK_Mode => On is
   ...
   function Extract (Value : U64x4; Lane : Lane_Index_64x4) return U64 is
     (if Lane < 2 then Flyology_SIMD.Extract (Value.Low, Lane) else Flyology_SIMD.Extract (Value.High, Lane - 2)) with SPARK_Mode => On;

   function Replace (Value : U64x4; Lane : Lane_Index_64x4; With_Value : U64) return U64x4 is
     (if Lane < 2
      then (Low => Flyology_SIMD.Replace (Value.Low, Lane, With_Value), High => Value.High)
      else (Low => Value.Low, High => Flyology_SIMD.Replace (Value.High, Lane - 2, With_Value))) with SPARK_Mode => On;
   ...
   function Compress (Value : U64x4; Mask : Mask_64x4) return U64x4 with SPARK_Mode => On is
   ...
   function Expand (Value : U64x4; Mask : Mask_64x4) return U64x4 with SPARK_Mode => On is
      Result : Lane_Values_U64x4 := [others => 0];
      Next : Natural := 0;
   begin
      for Lane in Lane_Index_64x4 loop
   ...
   function Reduce_Add_Wrap (Value : U64x4) return U64 with SPARK_Mode => On is
   ...
   function Reduce_Min (Value : U64x4) return U64 with SPARK_Mode => On is
   ...
   function Reduce_Max (Value : U64x4) return U64 with SPARK_Mode => On is
   ...
   function Reverse_Lanes (Value : U64x4) return U64x4 is
     (From_Lanes ([for Lane in Lane_Index_64x4 => Extract (Value, 3 - Lane)])) with SPARK_Mode => On;

   function Permute_Lanes (Value : U64x4; Map : Lane_Map_64x4) return U64x4 is
     (From_Lanes ([for Lane in Lane_Index_64x4 => Extract (Value, Map.Selectors (Lane))])) with SPARK_Mode => On;

   function Permute_Lanes (Left, Right : U64x4; Map : Two_Source_Lane_Map_64x4) return U64x4 is
     (From_Lanes ([for Lane in Lane_Index_64x4 => Extract ((if Map.Selectors (Lane).From_Right then Right else Left), Map.Selectors (Lane).Lane)])) with SPARK_Mode => On;

   function Interleave_Low (Left, Right : U64x4) return U64x4 is
     (From_Lanes ([for Lane in Lane_Index_64x4 => (if Lane mod 2 = 0 then Extract (Left, Lane / 2) else Extract (Right, Lane / 2))])) with SPARK_Mode => On;

   function Interleave_High (Left, Right : U64x4) return U64x4 is
     (From_Lanes ([for Lane in Lane_Index_64x4 => (if Lane mod 2 = 0 then Extract (Left, 2 + Lane / 2) else Extract (Right, 2 + Lane / 2))])) with SPARK_Mode => On;

   function Deinterleave_Even (Left, Right : U64x4) return U64x4 is
     (From_Lanes ([for Lane in Lane_Index_64x4 => (if Lane < 2 then Extract (Left, 2 * Lane) else Extract (Right, 2 * (Lane - 2)))])) with SPARK_Mode => On;

   function Deinterleave_Odd (Left, Right : U64x4) return U64x4 is
     (From_Lanes ([for Lane in Lane_Index_64x4 => (if Lane < 2 then Extract (Left, 2 * Lane + 1) else Extract (Right, 2 * (Lane - 2) + 1))])) with SPARK_Mode => On;

   function Slide_Lanes_Toward_Low (Value : U64x4; Count : Natural) return U64x4 is
     (if Count >= 4 then Zero else From_Lanes ([for Lane in Lane_Index_64x4 => (if Lane + Count < 4 then Extract (Value, Lane + Count) else 0)])) with SPARK_Mode => On;

   function Slide_Lanes_Toward_High (Value : U64x4; Count : Natural) return U64x4 is
     (if Count >= 4 then Zero else From_Lanes ([for Lane in Lane_Index_64x4 => (if Lane >= Count then Extract (Value, Lane - Count) else 0)])) with SPARK_Mode => On;

   function Mask_From_Bit_Mask (Bits : Mask_Bits_64x4) return Mask_64x4 is
     ((Low => Flyology_SIMD.Mask_From_Bit_Mask (Bits and Mask_Bits_64x4 (3)),
       High => Flyology_SIMD.Mask_From_Bit_Mask (Interfaces.Shift_Right (Bits, 2)))) with SPARK_Mode => On;

   function To_Bit_Mask (Mask : Mask_64x4) return Mask_Bits_64x4 is
     (Mask_Bits_64x4 (Flyology_SIMD.To_Bit_Mask (Mask.Low)) or Interfaces.Shift_Left (Mask_Bits_64x4 (Flyology_SIMD.To_Bit_Mask (Mask.High)), 2)) with SPARK_Mode => On;
   ...
   function Test (Mask : Mask_64x4; Lane : Lane_Index_64x4) return Boolean is
     (if Lane < 2 then Flyology_SIMD.Test (Mask.Low, Lane) else Flyology_SIMD.Test (Mask.High, Lane - 2)) with SPARK_Mode => On;

   function Any_True (Mask : Mask_64x4) return Boolean is (Flyology_SIMD.Any_True (Mask.Low) or else Flyology_SIMD.Any_True (Mask.High)) with SPARK_Mode => On;

   function All_True (Mask : Mask_64x4) return Boolean is (Flyology_SIMD.All_True (Mask.Low) and then Flyology_SIMD.All_True (Mask.High)) with SPARK_Mode => On;

   function None_True (Mask : Mask_64x4) return Boolean is (Flyology_SIMD.None_True (Mask.Low) and then Flyology_SIMD.None_True (Mask.High)) with SPARK_Mode => On;

   function Population_Count (Mask : Mask_64x4) return Lane_Count_64x4 is (Lane_Count_64x4 (Flyology_SIMD.Population_Count (Mask.Low) + Flyology_SIMD.Population_Count (Mask.High))) with SPARK_Mode => On;

   function First_True (Mask : Mask_64x4) return Lane_Count_64x4 with SPARK_Mode => On is
      Low : constant Natural := Flyology_SIMD.First_True (Mask.Low);
      High : constant Natural := Flyology_SIMD.First_True (Mask.High);
   begin
   ...
   function Last_True (Mask : Mask_64x4) return Lane_Count_64x4 with SPARK_Mode => On is
      Low : constant Natural := Flyology_SIMD.Last_True (Mask.Low);
      High : constant Natural := Flyology_SIMD.Last_True (Mask.High);
   begin
   ...
   function Load (Data : U64_Array; Start : Natural) return U64x4 is
     ((Low => Flyology_SIMD.Load (Data, Start), High => Flyology_SIMD.Load (Data, Start + 2))) with SPARK_Mode => On;

   procedure Store (Data : in out U64_Array; Start : Natural; Value : U64x4) with SPARK_Mode => On is
   begin
      Flyology_SIMD.Store (Data, Start, Value.Low); Flyology_SIMD.Store (Data, Start + 2, Value.High);
   end Store;

   function Load_Unaligned (Data : U64_Array; Start : Natural) return U64x4 is
     ((Low => Flyology_SIMD.Load_Unaligned (Data, Start), High => Flyology_SIMD.Load_Unaligned (Data, Start + 2))) with SPARK_Mode => On;

   procedure Store_Unaligned (Data : in out U64_Array; Start : Natural; Value : U64x4) with SPARK_Mode => On is
   begin
      Flyology_SIMD.Store_Unaligned (Data, Start, Value.Low); Flyology_SIMD.Store_Unaligned (Data, Start + 2, Value.High);
   end Store_Unaligned;

   function Load_Aligned (Data : U64_Array; Start : Natural) return U64x4 is
     ((Low => Flyology_SIMD.Load_Aligned (Data, Start), High => Flyology_SIMD.Load_Aligned (Data, Start + 2)));

   procedure Store_Aligned (Data : in out U64_Array; Start : Natural; Value : U64x4) is
   begin
      Flyology_SIMD.Store_Aligned (Data, Start, Value.Low); Flyology_SIMD.Store_Aligned (Data, Start + 2, Value.High);
   end Store_Aligned;

   function Load_Partial (Data : U64_Array; Start : Natural; Count : Lane_Count_64x4) return U64x4 is
     (if Count <= 2
      then (Low => Flyology_SIMD.Load_Partial (Data, Start, Count), High => Flyology_SIMD.Zero)
      else (Low => Flyology_SIMD.Load (Data, Start), High => Flyology_SIMD.Load_Partial (Data, Start + 2, Count - 2))) with SPARK_Mode => On;

   procedure Store_Partial (Data : in out U64_Array; Start : Natural; Count : Lane_Count_64x4; Value : U64x4) with SPARK_Mode => On is
   begin
      if Count <= 2 then Flyology_SIMD.Store_Partial (Data, Start, Count, Value.Low);
      else Flyology_SIMD.Store (Data, Start, Value.Low); Flyology_SIMD.Store_Partial (Data, Start + 2, Count - 2, Value.High); end if;
   end Store_Partial;
   ...
   function To_Lanes (Value : I64x4) return Lane_Values_I64x4 with SPARK_Mode => On is
   ...
   function Compress (Value : I64x4; Mask : Mask_64x4) return I64x4 with SPARK_Mode => On is
   ...
   function Expand (Value : I64x4; Mask : Mask_64x4) return I64x4 with SPARK_Mode => On is
      Result : Lane_Values_I64x4 := [others => 0];
      Next : Natural := 0;
   begin
      for Lane in Lane_Index_64x4 loop
   ...
   function Reduce_Add_Wrap (Value : I64x4) return I64 with SPARK_Mode => On is
   ...
   function Reduce_Min (Value : I64x4) return I64 with SPARK_Mode => On is
   ...
   function Reduce_Max (Value : I64x4) return I64 with SPARK_Mode => On is
   ...
   procedure Store (Data : in out I64_Array; Start : Natural; Value : I64x4) with SPARK_Mode => On is
   begin
      Flyology_SIMD.Store (Data, Start, Value.Low); Flyology_SIMD.Store (Data, Start + 2, Value.High);
   end Store;
   ...
   procedure Store_Unaligned (Data : in out I64_Array; Start : Natural; Value : I64x4) with SPARK_Mode => On is
   begin
      Flyology_SIMD.Store_Unaligned (Data, Start, Value.Low); Flyology_SIMD.Store_Unaligned (Data, Start + 2, Value.High);
   end Store_Unaligned;
   ...
   procedure Store_Partial (Data : in out I64_Array; Start : Natural; Count : Lane_Count_64x4; Value : I64x4) with SPARK_Mode => On is
   begin
      if Count <= 2 then Flyology_SIMD.Store_Partial (Data, Start, Count, Value.Low);
      else Flyology_SIMD.Store (Data, Start, Value.Low); Flyology_SIMD.Store_Partial (Data, Start + 2, Count - 2, Value.High); end if;
   end Store_Partial;
   ...
   function To_Lanes (Value : F32x8) return Lane_Values_F32x8 with SPARK_Mode => On is
   ...
   function Add (Left, Right : F32x8) return F32x8 is
     ((Low => Flyology_SIMD.Add (Left.Low, Right.Low),
       High => Flyology_SIMD.Add (Left.High, Right.High))) with SPARK_Mode => On;

   function Subtract (Left, Right : F32x8) return F32x8 is
     ((Low => Flyology_SIMD.Subtract (Left.Low, Right.Low),
       High => Flyology_SIMD.Subtract (Left.High, Right.High))) with SPARK_Mode => On;

   function Multiply (Left, Right : F32x8) return F32x8 is
     ((Low => Flyology_SIMD.Multiply (Left.Low, Right.Low),
       High => Flyology_SIMD.Multiply (Left.High, Right.High))) with SPARK_Mode => On;

   function Divide (Left, Right : F32x8) return F32x8 is
     ((Low => Flyology_SIMD.Divide (Left.Low, Right.Low),
       High => Flyology_SIMD.Divide (Left.High, Right.High))) with SPARK_Mode => On;

   function Min_Number (Left, Right : F32x8) return F32x8 is
     ((Low => Flyology_SIMD.Min_Number (Left.Low, Right.Low),
       High => Flyology_SIMD.Min_Number (Left.High, Right.High))) with SPARK_Mode => On;

   function Max_Number (Left, Right : F32x8) return F32x8 is
     ((Low => Flyology_SIMD.Max_Number (Left.Low, Right.Low),
       High => Flyology_SIMD.Max_Number (Left.High, Right.High))) with SPARK_Mode => On;
   ...
   function Unordered (Left, Right : F32x8) return Mask_32x8 is
     ((Low => Flyology_SIMD.Unordered (Left.Low, Right.Low),
       High => Flyology_SIMD.Unordered (Left.High, Right.High))) with SPARK_Mode => On;

   function Select_Value (Mask : Mask_32x8; If_True, If_False : F32x8) return F32x8 is
     ((Low => Flyology_SIMD.Select_Value (Mask.Low, If_True.Low, If_False.Low),
       High => Flyology_SIMD.Select_Value (Mask.High, If_True.High, If_False.High))) with SPARK_Mode => On;

   function Compress (Value : F32x8; Mask : Mask_32x8) return F32x8 with SPARK_Mode => On is
   ...
   function Expand (Value : F32x8; Mask : Mask_32x8) return F32x8 with SPARK_Mode => On is
      Result : Lane_Values_F32x8 := [others => 0.0];
      Next : Natural := 0;
   begin
      for Lane in Lane_Index_32x8 loop
   ...
   function Reduce_Min_Number (Value : F32x8) return F32 with SPARK_Mode => On is
   ...
   function Reduce_Max_Number (Value : F32x8) return F32 with SPARK_Mode => On is
      Result : F32 := Extract (Value, 0);
   begin
   ...
   function Slide_Lanes_Toward_Low (Value : F32x8; Count : Natural) return F32x8 is
     (if Count >= 8 then Zero else From_Lanes ([for Lane in Lane_Index_32x8 => (if Lane + Count < 8 then Extract (Value, Lane + Count) else 0.0)])) with SPARK_Mode => On;

   function Slide_Lanes_Toward_High (Value : F32x8; Count : Natural) return F32x8 is
     (if Count >= 8 then Zero else From_Lanes ([for Lane in Lane_Index_32x8 => (if Lane >= Count then Extract (Value, Lane - Count) else 0.0)])) with SPARK_Mode => On;
   ...
   procedure Store (Data : in out F32_Array; Start : Natural; Value : F32x8) with SPARK_Mode => On is
   begin
      Flyology_SIMD.Store (Data, Start, Value.Low); Flyology_SIMD.Store (Data, Start + 4, Value.High);
   end Store;
   ...
   procedure Store_Unaligned (Data : in out F32_Array; Start : Natural; Value : F32x8) with SPARK_Mode => On is
   begin
      Flyology_SIMD.Store_Unaligned (Data, Start, Value.Low); Flyology_SIMD.Store_Unaligned (Data, Start + 4, Value.High);
   end Store_Unaligned;
   ...
   procedure Store_Partial (Data : in out F32_Array; Start : Natural; Count : Lane_Count_32x8; Value : F32x8) with SPARK_Mode => On is
   begin
      if Count <= 4 then Flyology_SIMD.Store_Partial (Data, Start, Count, Value.Low);
      else Flyology_SIMD.Store (Data, Start, Value.Low); Flyology_SIMD.Store_Partial (Data, Start + 4, Count - 4, Value.High); end if;
   end Store_Partial;
   ...
   function To_Lanes (Value : F64x4) return Lane_Values_F64x4 with SPARK_Mode => On is
   ...
   function Compress (Value : F64x4; Mask : Mask_64x4) return F64x4 with SPARK_Mode => On is
   ...
   function Expand (Value : F64x4; Mask : Mask_64x4) return F64x4 with SPARK_Mode => On is
      Result : Lane_Values_F64x4 := [others => 0.0];
      Next : Natural := 0;
   begin
      for Lane in Lane_Index_64x4 loop
   ...
   function Reduce_Min_Number (Value : F64x4) return F64 with SPARK_Mode => On is
   ...
   function Reduce_Max_Number (Value : F64x4) return F64 with SPARK_Mode => On is
      Result : F64 := Extract (Value, 0);
   begin
   ...
   function Slide_Lanes_Toward_Low (Value : F64x4; Count : Natural) return F64x4 is
     (if Count >= 4 then Zero else From_Lanes ([for Lane in Lane_Index_64x4 => (if Lane + Count < 4 then Extract (Value, Lane + Count) else 0.0)])) with SPARK_Mode => On;

   function Slide_Lanes_Toward_High (Value : F64x4; Count : Natural) return F64x4 is
     (if Count >= 4 then Zero else From_Lanes ([for Lane in Lane_Index_64x4 => (if Lane >= Count then Extract (Value, Lane - Count) else 0.0)])) with SPARK_Mode => On;
   ...
   procedure Store (Data : in out F64_Array; Start : Natural; Value : F64x4) with SPARK_Mode => On is
   begin
      Flyology_SIMD.Store (Data, Start, Value.Low); Flyology_SIMD.Store (Data, Start + 2, Value.High);
   end Store;
   ...
   procedure Store_Unaligned (Data : in out F64_Array; Start : Natural; Value : F64x4) with SPARK_Mode => On is
   begin
      Flyology_SIMD.Store_Unaligned (Data, Start, Value.Low); Flyology_SIMD.Store_Unaligned (Data, Start + 2, Value.High);
   end Store_Unaligned;
   ...
   procedure Store_Partial (Data : in out F64_Array; Start : Natural; Count : Lane_Count_64x4; Value : F64x4) with SPARK_Mode => On is
   begin
      if Count <= 2 then Flyology_SIMD.Store_Partial (Data, Start, Count, Value.Low);
      else Flyology_SIMD.Store (Data, Start, Value.Low); Flyology_SIMD.Store_Partial (Data, Start + 2, Count - 2, Value.High); end if;
   end Store_Partial;
   ...
   function Widen_Low (Value : U8x32) return U16x16 is
     ((Low => Flyology_SIMD.Widen_Low (Value.Low),
       High => Flyology_SIMD.Widen_High (Value.Low))) with SPARK_Mode => On;

   function Widen_High (Value : U8x32) return U16x16 is
     ((Low => Flyology_SIMD.Widen_Low (Value.High),
       High => Flyology_SIMD.Widen_High (Value.High))) with SPARK_Mode => On;
   ...
   function Narrow_Truncate (Low, High : U16x16) return U8x32 is
     ((Low => Flyology_SIMD.Narrow_Truncate (Low.Low, Low.High),
       High => Flyology_SIMD.Narrow_Truncate (High.Low, High.High))) with SPARK_Mode => On;

   function Narrow_Saturate (Low, High : U16x16) return U8x32 is
     ((Low => Flyology_SIMD.Narrow_Saturate (Low.Low, Low.High),
       High => Flyology_SIMD.Narrow_Saturate (High.Low, High.High))) with SPARK_Mode => On;
   ...
   function Narrow_Round (Low, High : F64x4) return F32x8 is
     ((Low => Flyology_SIMD.Narrow_Round (Low.Low, Low.High),
       High => Flyology_SIMD.Narrow_Round (High.Low, High.High))) with SPARK_Mode => On;

   function Convert_Round (Value : I32x8) return F32x8 is
     ((Low => Flyology_SIMD.Convert_Round (Value.Low),
       High => Flyology_SIMD.Convert_Round (Value.High))) with SPARK_Mode => On;
   ...
   function Convert_Truncate_Saturate (Value : F32x8) return I32x8 is
     ((Low => Flyology_SIMD.Convert_Truncate_Saturate (Value.Low),
       High => Flyology_SIMD.Convert_Truncate_Saturate (Value.High))) with SPARK_Mode => On;
   ...
   function Convert_Saturate (Value : I8x32) return U8x32 is
     ((Low => Flyology_SIMD.Convert_Saturate (Value.Low),
       High => Flyology_SIMD.Convert_Saturate (Value.High))) with SPARK_Mode => On;
