...
package body Flyology_SIMD.Wide.Lookup_Mechanism is
   ...
   is
      type Byte_Vector is array (Natural range 0 .. 31) of U8
        with Pack, Size => 256;
      Lane_Bias : aliased constant Byte_Vector :=
        [0 .. 15 => 0, 16 .. 31 => 16];
   ...
   begin
      Asm
        (Template =>
           ...
           "vpbroadcastb (%5), %%ymm4" & ASCII.LF & ASCII.HT &
           "vpsubusb %%ymm4, %%ymm1, %%ymm1" & ASCII.LF & ASCII.HT &
           "vpcmpeqb %%ymm5, %%ymm1, %%ymm1" & ASCII.LF & ASCII.HT &
           "vpand %%ymm1, %%ymm2, %%ymm2" & ASCII.LF & ASCII.HT &
