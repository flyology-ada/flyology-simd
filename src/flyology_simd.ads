...
is
   ...
   --  Convert the low source half according to the documented widening semantics.
   --  Cross-platform support: This overload uses the portable scalar implementation on every supported GNAT target. For the matching Native overload, the AArch64 backend uses a dedicated NEON fcvtl floating conversion instruction. The x86-64 backend uses scalar composition. A scalar build uses the portable scalar implementation.
   --  @param Value The input value.
   --  @return The operation result.
   function Widen_High (Value : F32x4) return F64x2;
   --  Convert the high source half according to the documented widening semantics.
   --  Cross-platform support: This overload uses the portable scalar implementation on every supported GNAT target. For the matching Native overload, the AArch64 backend uses a dedicated NEON fcvtl2 floating conversion instruction. The x86-64 backend uses scalar composition. A scalar build uses the portable scalar implementation.
   --  @param Value The input value.
   --  @return The operation result.
