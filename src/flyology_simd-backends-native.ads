...
is
   ...
   --  Cross-platform support: The AArch64 backend uses a dedicated NEON instruction that adds lanes with saturation. The x86-64 backend uses a dedicated SSE2 instruction that adds lanes with saturation. A scalar build uses the portable scalar implementation.
   --  @param Left The left input.
   --  @param Right The right input.
   --  @return The operation result.
   function Subtract_Saturate (Left, Right : U8x16) return U8x16;
   --  Subtract corresponding lanes and clamp to the lane range.
   --  Cross-platform support: The AArch64 backend uses a dedicated NEON instruction that subtracts lanes with saturation. The x86-64 backend uses a dedicated SSE2 instruction that subtracts lanes with saturation. A scalar build uses the portable scalar implementation.
   --  @param Left The left input.
   --  @param Right The right input.
   --  @return The operation result.
   ...
   --  Cross-platform support: The AArch64 backend uses a dedicated NEON sequence that clamps each lane to the destination type's range. The x86-64 backend uses a dedicated SSE2 sequence that derives a sign mask and selects the clamped lanes. A scalar build uses the portable scalar implementation.
   --  @param Value The input value.
   --  @return The operation result.
   ...
   --  Add corresponding lanes and clamp to the lane range.
   --  Cross-platform support: The AArch64 backend uses a dedicated NEON instruction that adds lanes with saturation. The x86-64 backend uses a dedicated SSE2 sequence that derives a carry mask and selects the unsigned maximum. A scalar build uses the portable scalar implementation.
   --  @param Left The left input.
   --  @param Right The right input.
   --  @return The operation result.
   function Subtract_Saturate (Left, Right : U32x4) return U32x4;
   --  Subtract corresponding lanes and clamp to the lane range.
   --  Cross-platform support: The AArch64 backend uses a dedicated NEON instruction that subtracts lanes with saturation. The x86-64 backend uses a dedicated SSE2 sequence that derives a borrow mask and selects zero. A scalar build uses the portable scalar implementation.
   --  @param Left The left input.
   --  @param Right The right input.
   --  @return The operation result.
   ...
   --  Add corresponding lanes and clamp to the lane range.
   --  Cross-platform support: The AArch64 backend uses a dedicated NEON instruction that adds lanes with saturation. The x86-64 backend uses a dedicated SSE2 sequence that derives a signed-overflow mask and selects the signed minimum or maximum. A scalar build uses the portable scalar implementation.
   --  @param Left The left input.
   --  @param Right The right input.
   --  @return The operation result.
   function Subtract_Saturate (Left, Right : I32x4) return I32x4;
   --  Subtract corresponding lanes and clamp to the lane range.
   --  Cross-platform support: The AArch64 backend uses a dedicated NEON instruction that subtracts lanes with saturation. The x86-64 backend uses a dedicated SSE2 sequence that derives a signed-overflow mask and selects the signed minimum or maximum. A scalar build uses the portable scalar implementation.
   --  @param Left The left input.
   --  @param Right The right input.
   --  @return The operation result.
