...
is
   ...
   --  Cross-platform support: The AArch64, composed x86-64, and optional AVX2 backends apply the selected 128-bit Mask_From_Bit_Mask operation to the low and high compact-bit halves and build the two-part mask result. In a scalar build, this overload uses the same composition through the portable 128-bit implementation.
   ...
   --  Cross-platform support: The AArch64, composed x86-64, and optional AVX2 backends apply the selected 128-bit To_Bit_Mask operation to both private parts. They place the low-part bits first and shift the high-part bits by the private lane count. In a scalar build, this overload uses the same composition through the portable 128-bit implementation.
   --  @param Mask The mask input.
   --  @return The operation result.
   function Mask_And (Left, Right : Mask_8x32) return Mask_8x32 with Inline_Always;
   --  Apply Mask_And to corresponding mask truths.
   --  Cross-platform support: The AArch64, composed x86-64, and optional AVX2 backends apply the selected 128-bit Mask_And operation to the corresponding private parts and build the two-part mask result. In a scalar build, this overload uses the same composition through the portable 128-bit implementation.
   --  @param Left The left input.
   --  @param Right The right input.
   --  @return The operation result.
   function Mask_Or (Left, Right : Mask_8x32) return Mask_8x32 with Inline_Always;
   --  Apply Mask_Or to corresponding mask truths.
   --  Cross-platform support: The AArch64, composed x86-64, and optional AVX2 backends apply the selected 128-bit Mask_Or operation to the corresponding private parts and build the two-part mask result. In a scalar build, this overload uses the same composition through the portable 128-bit implementation.
   --  @param Left The left input.
   --  @param Right The right input.
   --  @return The operation result.
   function Mask_Xor (Left, Right : Mask_8x32) return Mask_8x32 with Inline_Always;
   --  Apply Mask_Xor to corresponding mask truths.
   --  Cross-platform support: The AArch64, composed x86-64, and optional AVX2 backends apply the selected 128-bit Mask_Xor operation to the corresponding private parts and build the two-part mask result. In a scalar build, this overload uses the same composition through the portable 128-bit implementation.
   --  @param Left The left input.
   --  @param Right The right input.
   --  @return The operation result.
   function Mask_Not (Value : Mask_8x32) return Mask_8x32 with Inline_Always;
   --  Complement every mask truth.
   --  Cross-platform support: The AArch64, composed x86-64, and optional AVX2 backends apply the selected 128-bit Mask_Not operation to the corresponding private parts and build the two-part mask result. In a scalar build, this overload uses the same composition through the portable 128-bit implementation.
   --  @param Value The value input.
   --  @return The operation result.
   function Test (Mask : Mask_8x32; Lane : Lane_Index_8x32) return Boolean with Inline_Always;
   --  Return one mask truth.
   --  Cross-platform support: The AArch64, composed x86-64, and optional AVX2 backends apply the selected 128-bit Test operation only to the private part that contains the requested lane. They subtract the private lane count for a lane in the high part. In a scalar build, this overload uses the same selected-part composition through the portable 128-bit implementation.
   --  @param Mask The mask input.
   --  @param Lane The lane input.
   --  @return The operation result.
   function Any_True (Mask : Mask_8x32) return Boolean with Inline_Always;
   --  Return the Any_True mask reduction.
   --  Cross-platform support: The AArch64, composed x86-64, and optional AVX2 backends apply the selected 128-bit Any_True operation to both private parts and combine the results with or else. In a scalar build, this overload uses the same composition through the portable 128-bit implementation.
   --  @param Mask The mask input.
   --  @return The operation result.
   function All_True (Mask : Mask_8x32) return Boolean with Inline_Always;
   --  Return the All_True mask reduction.
   --  Cross-platform support: The AArch64, composed x86-64, and optional AVX2 backends apply the selected 128-bit All_True operation to both private parts and combine the results with and then. In a scalar build, this overload uses the same composition through the portable 128-bit implementation.
   --  @param Mask The mask input.
   --  @return The operation result.
   function None_True (Mask : Mask_8x32) return Boolean with Inline_Always;
   --  Return the None_True mask reduction.
   --  Cross-platform support: The AArch64, composed x86-64, and optional AVX2 backends apply the selected 128-bit None_True operation to both private parts and combine the results with and then. In a scalar build, this overload uses the same composition through the portable 128-bit implementation.
   --  @param Mask The mask input.
   --  @return The operation result.
   function Population_Count (Mask : Mask_8x32) return Lane_Count_8x32 with Inline_Always;
   --  Return the number of true lanes.
   --  Cross-platform support: The AArch64, composed x86-64, and optional AVX2 backends apply the selected 128-bit Population_Count operation to both private parts and add the two counts. In a scalar build, this overload uses the same composition through the portable 128-bit implementation.
   --  @param Mask The mask input.
   --  @return The operation result.
   ...
   --  Cross-platform support: The AArch64, composed x86-64, and optional AVX2 backends apply the selected 128-bit First_True operation to both private parts. They return a valid low-part result first, otherwise a valid high-part result plus the private lane count, or the Wide lane count when neither part is true. In a scalar build, this overload uses the same composition through the portable 128-bit implementation.
   --  @param Mask The mask input.
   --  @return The operation result.
   ...
   --  Cross-platform support: The AArch64, composed x86-64, and optional AVX2 backends apply the selected 128-bit Last_True operation to both private parts. They return a valid high-part result plus the private lane count first, otherwise a valid low-part result, or the Wide lane count when neither part is true. In a scalar build, this overload uses the same composition through the portable 128-bit implementation.
   --  @param Mask The mask input.
   --  @return The operation result.
