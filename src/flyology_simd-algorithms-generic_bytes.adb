   ...
   is
      function Find_Small
        (Needle_0, Needle_1, Needle_2, Needle_3 : U8;
         Count : Natural) return Search_Result
      is
         Offset : Natural := 0;
         Bits   : Interfaces.Unsigned_16;
         Value  : U8x16;
      ...
      begin
         while Data'Length - Offset >= 16 loop
            ...
            if Count >= 2 then
               Bits := Bits or
                 Backend_To_Bit_Mask (Backend_Equal (Value, Wanted_1));
            end if;
            if Count >= 3 then
               Bits := Bits or
                 Backend_To_Bit_Mask (Backend_Equal (Value, Wanted_2));
            end if;
            if Count >= 4 then
               Bits := Bits or
                 Backend_To_Bit_Mask (Backend_Equal (Value, Wanted_3));
            end if;
            if Bits /= 0 then
               for Lane in Lane_Index_8x16 loop
                  if (Bits and Interfaces.Shift_Left
