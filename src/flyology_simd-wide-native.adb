...
package body Flyology_SIMD.Wide.Native is
   ...
   function Zero return U8x32 is
     ((Low => Flyology_SIMD.Backends.Native.Zero, High => Flyology_SIMD.Backends.Native.Zero)) with SPARK_Mode => On;

   function Splat (Value : U8) return U8x32 is
     ((Low => Flyology_SIMD.Backends.Native.Splat (Value),
       High => Flyology_SIMD.Backends.Native.Splat (Value))) with SPARK_Mode => On;
   ...
   function From_Lanes (Values : Lane_Values_U8x32) return U8x32 is
     ((Low => Flyology_SIMD.Backends.Native.From_Lanes ([for Lane in 0 .. 15 => Values (Lane)]),
       High => Flyology_SIMD.Backends.Native.From_Lanes ([for Lane in 0 .. 15 => Values (Lane + 16)]))) with SPARK_Mode => On;

   function To_Lanes (Value : U8x32) return Lane_Values_U8x32 with SPARK_Mode => On is
      Low : constant Lane_Values_8x16 := Flyology_SIMD.Backends.Native.To_Lanes (Value.Low);
      High : constant Lane_Values_8x16 := Flyology_SIMD.Backends.Native.To_Lanes (Value.High);
   begin
   ...
   function Extract (Value : U8x32; Lane : Lane_Index_8x32) return U8 is
     (if Lane < 16 then Flyology_SIMD.Backends.Native.Extract (Value.Low, Lane) else Flyology_SIMD.Backends.Native.Extract (Value.High, Lane - 16)) with SPARK_Mode => On;
   ...
   function Replace (Value : U8x32; Lane : Lane_Index_8x32; With_Value : U8) return U8x32 is
     (if Lane < 16
      ...
      else (Low => Value.Low, High => Flyology_SIMD.Backends.Native.Replace (Value.High, Lane - 16, With_Value))) with SPARK_Mode => On;

   function Table_Lookup (Table, Indices : U8x32) return U8x32 is
     (Lookup_Mechanism.Table_Lookup_32 (Table, Indices)) with SPARK_Mode => On;
   ...
   function Bit_Cast (Value : U8x32) return I8x32 is
     ((Low => Flyology_SIMD.Backends.Native.Bit_Cast (Value.Low), High => Flyology_SIMD.Backends.Native.Bit_Cast (Value.High))) with SPARK_Mode => On;

   function Add_Wrap (Left, Right : U8x32) return U8x32 is
     (Byte_Mechanism.Add_Wrap (Left, Right)) with SPARK_Mode => On;

   function Subtract_Wrap (Left, Right : U8x32) return U8x32 is
     (Byte_Mechanism.Subtract_Wrap (Left, Right)) with SPARK_Mode => On;

   function Multiply_Wrap (Left, Right : U8x32) return U8x32 is
     (Byte_Mechanism.Multiply_Wrap (Left, Right)) with SPARK_Mode => On;

   function Add_Saturate (Left, Right : U8x32) return U8x32 is
     (Byte_Mechanism.Add_Saturate (Left, Right)) with SPARK_Mode => On;

   function Subtract_Saturate (Left, Right : U8x32) return U8x32 is
     (Byte_Mechanism.Subtract_Saturate (Left, Right)) with SPARK_Mode => On;

   function Bitwise_And (Left, Right : U8x32) return U8x32 is
     (Byte_Mechanism.Bitwise_And (Left, Right)) with SPARK_Mode => On;

   function Bitwise_Or (Left, Right : U8x32) return U8x32 is
     (Byte_Mechanism.Bitwise_Or (Left, Right)) with SPARK_Mode => On;

   function Bitwise_Xor (Left, Right : U8x32) return U8x32 is
     (Byte_Mechanism.Bitwise_Xor (Left, Right)) with SPARK_Mode => On;

   function Min (Left, Right : U8x32) return U8x32 is
     (Byte_Mechanism.Min (Left, Right)) with SPARK_Mode => On;

   function Max (Left, Right : U8x32) return U8x32 is
     (Byte_Mechanism.Max (Left, Right)) with SPARK_Mode => On;

   function Bitwise_Not (Value : U8x32) return U8x32 is
     (Byte_Mechanism.Bitwise_Not (Value)) with SPARK_Mode => On;
   ...
   function Shift_Left_Logical (Value : U8x32; Count : Natural) return U8x32 is
     ((Low => Flyology_SIMD.Backends.Native.Shift_Left_Logical (Value.Low, Count),
       High => Flyology_SIMD.Backends.Native.Shift_Left_Logical (Value.High, Count))) with SPARK_Mode => On;
   ...
   function Shift_Right_Logical (Value : U8x32; Count : Natural) return U8x32 is
     ((Low => Flyology_SIMD.Backends.Native.Shift_Right_Logical (Value.Low, Count),
       High => Flyology_SIMD.Backends.Native.Shift_Right_Logical (Value.High, Count))) with SPARK_Mode => On;

   function Equal (Left, Right : U8x32) return Mask_8x32 is
     (Mask_From_Bit_Mask (Byte_Mechanism.Equal (Left, Right))) with SPARK_Mode => On;

   function Less_Than (Left, Right : U8x32) return Mask_8x32 is
     (Mask_From_Bit_Mask (Byte_Mechanism.Less_Than (Left, Right))) with SPARK_Mode => On;

   function Less_Equal (Left, Right : U8x32) return Mask_8x32 is
     (Mask_From_Bit_Mask (Byte_Mechanism.Less_Equal (Left, Right))) with SPARK_Mode => On;

   function Greater_Than (Left, Right : U8x32) return Mask_8x32 is
     (Mask_From_Bit_Mask (Byte_Mechanism.Greater_Than (Left, Right))) with SPARK_Mode => On;

   function Greater_Equal (Left, Right : U8x32) return Mask_8x32 is
     (Mask_From_Bit_Mask (Byte_Mechanism.Greater_Equal (Left, Right))) with SPARK_Mode => On;

   function Select_Value (Mask : Mask_8x32; If_True, If_False : U8x32) return U8x32 is
     (Byte_Mechanism.Select_Value
        (To_Bit_Mask (Mask), If_True, If_False)) with SPARK_Mode => On;

   function Compress (Value : U8x32; Mask : Mask_8x32) return U8x32 is
     (Compact_Mechanism.Compress (Value, Mask)) with SPARK_Mode => On;

   function Expand (Value : U8x32; Mask : Mask_8x32) return U8x32 is
     (Compact_Mechanism.Expand (Value, Mask)) with SPARK_Mode => On;
   ...
   function Reverse_Lanes (Value : U8x32) return U8x32 is
     (Permute_Mechanism.Reverse_Lanes (Value)) with SPARK_Mode => On;

   function Permute_Lanes (Value : U8x32; Map : Lane_Map_8x32) return U8x32 is
     (Permute_Mechanism.Permute_Lanes (Value, Map)) with SPARK_Mode => On;

   function Permute_Lanes (Left, Right : U8x32; Map : Two_Source_Lane_Map_8x32) return U8x32 is
     (Permute_Mechanism.Permute_Lanes (Left, Right, Map)) with SPARK_Mode => On;

   function Interleave_Low (Left, Right : U8x32) return U8x32 is
     (Permute_Mechanism.Interleave_Low (Left, Right)) with SPARK_Mode => On;

   function Interleave_High (Left, Right : U8x32) return U8x32 is
     (Permute_Mechanism.Interleave_High (Left, Right)) with SPARK_Mode => On;

   function Deinterleave_Even (Left, Right : U8x32) return U8x32 is
     (Permute_Mechanism.Deinterleave_Even (Left, Right)) with SPARK_Mode => On;

   function Deinterleave_Odd (Left, Right : U8x32) return U8x32 is
     (Permute_Mechanism.Deinterleave_Odd (Left, Right)) with SPARK_Mode => On;

   function Slide_Lanes_Toward_Low (Value : U8x32; Count : Natural) return U8x32 is
     (Permute_Mechanism.Slide_Lanes_Toward_Low (Value, Count)) with SPARK_Mode => On;

   function Slide_Lanes_Toward_High (Value : U8x32; Count : Natural) return U8x32 is
     (Permute_Mechanism.Slide_Lanes_Toward_High (Value, Count)) with SPARK_Mode => On;
   ...
   function Mask_From_Bit_Mask (Bits : Mask_Bits_8x32) return Mask_8x32 is
     ((Low => Flyology_SIMD.Backends.Native.Mask_From_Bit_Mask (Interfaces.Unsigned_16 (Bits and Mask_Bits_8x32 (65535))),
       High => Flyology_SIMD.Backends.Native.Mask_From_Bit_Mask (Interfaces.Unsigned_16 (Interfaces.Shift_Right (Bits, 16))))) with SPARK_Mode => On;

   function To_Bit_Mask (Mask : Mask_8x32) return Mask_Bits_8x32 is
     (Mask_Bits_8x32 (Flyology_SIMD.Backends.Native.To_Bit_Mask (Mask.Low)) or Interfaces.Shift_Left (Mask_Bits_8x32 (Flyology_SIMD.Backends.Native.To_Bit_Mask (Mask.High)), 16)) with SPARK_Mode => On;

   function Mask_And (Left, Right : Mask_8x32) return Mask_8x32 is
     ((Low => Flyology_SIMD.Backends.Native.Mask_And (Left.Low, Right.Low),
       High => Flyology_SIMD.Backends.Native.Mask_And (Left.High, Right.High))) with SPARK_Mode => On;

   function Mask_Or (Left, Right : Mask_8x32) return Mask_8x32 is
     ((Low => Flyology_SIMD.Backends.Native.Mask_Or (Left.Low, Right.Low),
       High => Flyology_SIMD.Backends.Native.Mask_Or (Left.High, Right.High))) with SPARK_Mode => On;

   function Mask_Xor (Left, Right : Mask_8x32) return Mask_8x32 is
     ((Low => Flyology_SIMD.Backends.Native.Mask_Xor (Left.Low, Right.Low),
       High => Flyology_SIMD.Backends.Native.Mask_Xor (Left.High, Right.High))) with SPARK_Mode => On;

   function Mask_Not (Value : Mask_8x32) return Mask_8x32 is
     ((Low => Flyology_SIMD.Backends.Native.Mask_Not (Value.Low),
       High => Flyology_SIMD.Backends.Native.Mask_Not (Value.High))) with SPARK_Mode => On;

   function Test (Mask : Mask_8x32; Lane : Lane_Index_8x32) return Boolean is
     (if Lane < 16 then Flyology_SIMD.Backends.Native.Test (Mask.Low, Lane) else Flyology_SIMD.Backends.Native.Test (Mask.High, Lane - 16)) with SPARK_Mode => On;

   function Any_True (Mask : Mask_8x32) return Boolean is (Flyology_SIMD.Backends.Native.Any_True (Mask.Low) or else Flyology_SIMD.Backends.Native.Any_True (Mask.High)) with SPARK_Mode => On;

   function All_True (Mask : Mask_8x32) return Boolean is (Flyology_SIMD.Backends.Native.All_True (Mask.Low) and then Flyology_SIMD.Backends.Native.All_True (Mask.High)) with SPARK_Mode => On;

   function None_True (Mask : Mask_8x32) return Boolean is (Flyology_SIMD.Backends.Native.None_True (Mask.Low) and then Flyology_SIMD.Backends.Native.None_True (Mask.High)) with SPARK_Mode => On;

   function Population_Count (Mask : Mask_8x32) return Lane_Count_8x32 is (Lane_Count_8x32 (Flyology_SIMD.Backends.Native.Population_Count (Mask.Low) + Flyology_SIMD.Backends.Native.Population_Count (Mask.High))) with SPARK_Mode => On;
   ...
   function Load (Data : Byte_Array; Start : Natural) return U8x32 is
     ((Low => Flyology_SIMD.Backends.Native.Load (Data, Start), High => Flyology_SIMD.Backends.Native.Load (Data, Start + 16))) with SPARK_Mode => On;
   ...
   function Load_Unaligned (Data : Byte_Array; Start : Natural) return U8x32 is
     ((Low => Flyology_SIMD.Backends.Native.Load_Unaligned (Data, Start), High => Flyology_SIMD.Backends.Native.Load_Unaligned (Data, Start + 16))) with SPARK_Mode => On;
   ...
   function Load_Partial (Data : Byte_Array; Start : Natural; Count : Lane_Count_8x32) return U8x32 is
     (if Count <= 16
      ...
      else (Low => Flyology_SIMD.Backends.Native.Load (Data, Start), High => Flyology_SIMD.Backends.Native.Load_Partial (Data, Start + 16, Count - 16))) with SPARK_Mode => On;
   ...
   function To_Lanes (Value : I8x32) return Lane_Values_I8x32 with SPARK_Mode => On is
      Low : constant Lane_Values_I8x16 := Flyology_SIMD.Backends.Native.To_Lanes (Value.Low);
      High : constant Lane_Values_I8x16 := Flyology_SIMD.Backends.Native.To_Lanes (Value.High);
   begin
      return [for Lane in Lane_Index_8x32 =>
        (if Lane < 16 then Low (Lane)
         else High (Lane - 16))];
   ...
   function Shift_Right_Arithmetic (Value : I8x32; Count : Natural) return I8x32 is
     ((Low => Flyology_SIMD.Backends.Native.Shift_Right_Arithmetic (Value.Low, Count),
       High => Flyology_SIMD.Backends.Native.Shift_Right_Arithmetic (Value.High, Count))) with SPARK_Mode => On;
   ...
   function From_Lanes (Values : Lane_Values_U16x16) return U16x16 is
     ((Low => Flyology_SIMD.Backends.Native.From_Lanes ([for Lane in 0 .. 7 => Values (Lane)]),
       High => Flyology_SIMD.Backends.Native.From_Lanes ([for Lane in 0 .. 7 => Values (Lane + 8)]))) with SPARK_Mode => On;

   function To_Lanes (Value : U16x16) return Lane_Values_U16x16 with SPARK_Mode => On is
      Low : constant Lane_Values_U16x8 := Flyology_SIMD.Backends.Native.To_Lanes (Value.Low);
      High : constant Lane_Values_U16x8 := Flyology_SIMD.Backends.Native.To_Lanes (Value.High);
   begin
   ...
   function Extract (Value : U16x16; Lane : Lane_Index_16x16) return U16 is
     (if Lane < 8 then Flyology_SIMD.Backends.Native.Extract (Value.Low, Lane) else Flyology_SIMD.Backends.Native.Extract (Value.High, Lane - 8)) with SPARK_Mode => On;

   function Replace (Value : U16x16; Lane : Lane_Index_16x16; With_Value : U16) return U16x16 is
     (if Lane < 8
      then (Low => Flyology_SIMD.Backends.Native.Replace (Value.Low, Lane, With_Value), High => Value.High)
      else (Low => Value.Low, High => Flyology_SIMD.Backends.Native.Replace (Value.High, Lane - 8, With_Value))) with SPARK_Mode => On;
   ...
   function Add_Wrap (Left, Right : U16x16) return U16x16 is
     ((Low => Flyology_SIMD.Backends.Native.Add_Wrap (Left.Low, Right.Low),
       High => Flyology_SIMD.Backends.Native.Add_Wrap (Left.High, Right.High))) with SPARK_Mode => On;
   ...
   function Subtract_Wrap (Left, Right : U16x16) return U16x16 is
     ((Low => Flyology_SIMD.Backends.Native.Subtract_Wrap (Left.Low, Right.Low),
       High => Flyology_SIMD.Backends.Native.Subtract_Wrap (Left.High, Right.High))) with SPARK_Mode => On;
   ...
   function Multiply_Wrap (Left, Right : U16x16) return U16x16 is
     ((Low => Flyology_SIMD.Backends.Native.Multiply_Wrap (Left.Low, Right.Low),
       High => Flyology_SIMD.Backends.Native.Multiply_Wrap (Left.High, Right.High))) with SPARK_Mode => On;
   ...
   function Add_Saturate (Left, Right : U16x16) return U16x16 is
     ((Low => Flyology_SIMD.Backends.Native.Add_Saturate (Left.Low, Right.Low),
       High => Flyology_SIMD.Backends.Native.Add_Saturate (Left.High, Right.High))) with SPARK_Mode => On;
   ...
   function Equal (Left, Right : U16x16) return Mask_16x16 is
     ((Low => Flyology_SIMD.Backends.Native.Equal (Left.Low, Right.Low),
       High => Flyology_SIMD.Backends.Native.Equal (Left.High, Right.High))) with SPARK_Mode => On;

   function Less_Than (Left, Right : U16x16) return Mask_16x16 is
     ((Low => Flyology_SIMD.Backends.Native.Less_Than (Left.Low, Right.Low),
       High => Flyology_SIMD.Backends.Native.Less_Than (Left.High, Right.High))) with SPARK_Mode => On;
   ...
   function Less_Equal (Left, Right : U16x16) return Mask_16x16 is
     ((Low => Flyology_SIMD.Backends.Native.Less_Equal (Left.Low, Right.Low),
       High => Flyology_SIMD.Backends.Native.Less_Equal (Left.High, Right.High))) with SPARK_Mode => On;
   ...
   function Greater_Than (Left, Right : U16x16) return Mask_16x16 is
     ((Low => Flyology_SIMD.Backends.Native.Greater_Than (Left.Low, Right.Low),
       High => Flyology_SIMD.Backends.Native.Greater_Than (Left.High, Right.High))) with SPARK_Mode => On;
   ...
   function Greater_Equal (Left, Right : U16x16) return Mask_16x16 is
     ((Low => Flyology_SIMD.Backends.Native.Greater_Equal (Left.Low, Right.Low),
       High => Flyology_SIMD.Backends.Native.Greater_Equal (Left.High, Right.High))) with SPARK_Mode => On;
   ...
   function Select_Value (Mask : Mask_16x16; If_True, If_False : U16x16) return U16x16 is
     ((Low => Flyology_SIMD.Backends.Native.Select_Value (Mask.Low, If_True.Low, If_False.Low),
       High => Flyology_SIMD.Backends.Native.Select_Value (Mask.High, If_True.High, If_False.High))) with SPARK_Mode => On;
   ...
   function Mask_From_Bit_Mask (Bits : Mask_Bits_16x16) return Mask_16x16 is
     ((Low => Flyology_SIMD.Backends.Native.Mask_From_Bit_Mask (Interfaces.Unsigned_8 (Bits and Mask_Bits_16x16 (255))),
       High => Flyology_SIMD.Backends.Native.Mask_From_Bit_Mask (Interfaces.Unsigned_8 (Interfaces.Shift_Right (Bits, 8))))) with SPARK_Mode => On;

   function To_Bit_Mask (Mask : Mask_16x16) return Mask_Bits_16x16 is
     (Mask_Bits_16x16 (Flyology_SIMD.Backends.Native.To_Bit_Mask (Mask.Low)) or Interfaces.Shift_Left (Mask_Bits_16x16 (Flyology_SIMD.Backends.Native.To_Bit_Mask (Mask.High)), 8)) with SPARK_Mode => On;
   ...
   function Test (Mask : Mask_16x16; Lane : Lane_Index_16x16) return Boolean is
     (if Lane < 8 then Flyology_SIMD.Backends.Native.Test (Mask.Low, Lane) else Flyology_SIMD.Backends.Native.Test (Mask.High, Lane - 8)) with SPARK_Mode => On;

   function Any_True (Mask : Mask_16x16) return Boolean is (Flyology_SIMD.Backends.Native.Any_True (Mask.Low) or else Flyology_SIMD.Backends.Native.Any_True (Mask.High)) with SPARK_Mode => On;

   function All_True (Mask : Mask_16x16) return Boolean is (Flyology_SIMD.Backends.Native.All_True (Mask.Low) and then Flyology_SIMD.Backends.Native.All_True (Mask.High)) with SPARK_Mode => On;

   function None_True (Mask : Mask_16x16) return Boolean is (Flyology_SIMD.Backends.Native.None_True (Mask.Low) and then Flyology_SIMD.Backends.Native.None_True (Mask.High)) with SPARK_Mode => On;

   function Population_Count (Mask : Mask_16x16) return Lane_Count_16x16 is (Lane_Count_16x16 (Flyology_SIMD.Backends.Native.Population_Count (Mask.Low) + Flyology_SIMD.Backends.Native.Population_Count (Mask.High))) with SPARK_Mode => On;
   ...
   function Load (Data : U16_Array; Start : Natural) return U16x16 is
     ((Low => Flyology_SIMD.Backends.Native.Load (Data, Start), High => Flyology_SIMD.Backends.Native.Load (Data, Start + 8))) with SPARK_Mode => On;
   ...
   function Load_Unaligned (Data : U16_Array; Start : Natural) return U16x16 is
     ((Low => Flyology_SIMD.Backends.Native.Load_Unaligned (Data, Start), High => Flyology_SIMD.Backends.Native.Load_Unaligned (Data, Start + 8))) with SPARK_Mode => On;
   ...
   function Load_Partial (Data : U16_Array; Start : Natural; Count : Lane_Count_16x16) return U16x16 is
     (if Count <= 8
      then (Low => Flyology_SIMD.Backends.Native.Load_Partial (Data, Start, Count), High => Flyology_SIMD.Backends.Native.Zero)
      else (Low => Flyology_SIMD.Backends.Native.Load (Data, Start), High => Flyology_SIMD.Backends.Native.Load_Partial (Data, Start + 8, Count - 8))) with SPARK_Mode => On;
   ...
   function To_Lanes (Value : I16x16) return Lane_Values_I16x16 with SPARK_Mode => On is
      Low : constant Lane_Values_I16x8 := Flyology_SIMD.Backends.Native.To_Lanes (Value.Low);
      High : constant Lane_Values_I16x8 := Flyology_SIMD.Backends.Native.To_Lanes (Value.High);
   begin
      return [for Lane in Lane_Index_16x16 =>
        (if Lane < 8 then Low (Lane)
         else High (Lane - 8))];
   ...
   function From_Lanes (Values : Lane_Values_U32x8) return U32x8 is
     ((Low => Flyology_SIMD.Backends.Native.From_Lanes ([for Lane in 0 .. 3 => Values (Lane)]),
       High => Flyology_SIMD.Backends.Native.From_Lanes ([for Lane in 0 .. 3 => Values (Lane + 4)]))) with SPARK_Mode => On;

   function To_Lanes (Value : U32x8) return Lane_Values_U32x8 with SPARK_Mode => On is
      Low : constant Lane_Values_U32x4 := Flyology_SIMD.Backends.Native.To_Lanes (Value.Low);
      High : constant Lane_Values_U32x4 := Flyology_SIMD.Backends.Native.To_Lanes (Value.High);
   begin
   ...
   function Extract (Value : U32x8; Lane : Lane_Index_32x8) return U32 is
     (if Lane < 4 then Flyology_SIMD.Backends.Native.Extract (Value.Low, Lane) else Flyology_SIMD.Backends.Native.Extract (Value.High, Lane - 4)) with SPARK_Mode => On;

   function Replace (Value : U32x8; Lane : Lane_Index_32x8; With_Value : U32) return U32x8 is
     (if Lane < 4
      then (Low => Flyology_SIMD.Backends.Native.Replace (Value.Low, Lane, With_Value), High => Value.High)
      else (Low => Value.Low, High => Flyology_SIMD.Backends.Native.Replace (Value.High, Lane - 4, With_Value))) with SPARK_Mode => On;
   ...
   function Mask_From_Bit_Mask (Bits : Mask_Bits_32x8) return Mask_32x8 is
     ((Low => Flyology_SIMD.Backends.Native.Mask_From_Bit_Mask (Bits and Mask_Bits_32x8 (15)),
       High => Flyology_SIMD.Backends.Native.Mask_From_Bit_Mask (Interfaces.Shift_Right (Bits, 4)))) with SPARK_Mode => On;

   function To_Bit_Mask (Mask : Mask_32x8) return Mask_Bits_32x8 is
     (Mask_Bits_32x8 (Flyology_SIMD.Backends.Native.To_Bit_Mask (Mask.Low)) or Interfaces.Shift_Left (Mask_Bits_32x8 (Flyology_SIMD.Backends.Native.To_Bit_Mask (Mask.High)), 4)) with SPARK_Mode => On;
   ...
   function Test (Mask : Mask_32x8; Lane : Lane_Index_32x8) return Boolean is
     (if Lane < 4 then Flyology_SIMD.Backends.Native.Test (Mask.Low, Lane) else Flyology_SIMD.Backends.Native.Test (Mask.High, Lane - 4)) with SPARK_Mode => On;

   function Any_True (Mask : Mask_32x8) return Boolean is (Flyology_SIMD.Backends.Native.Any_True (Mask.Low) or else Flyology_SIMD.Backends.Native.Any_True (Mask.High)) with SPARK_Mode => On;

   function All_True (Mask : Mask_32x8) return Boolean is (Flyology_SIMD.Backends.Native.All_True (Mask.Low) and then Flyology_SIMD.Backends.Native.All_True (Mask.High)) with SPARK_Mode => On;

   function None_True (Mask : Mask_32x8) return Boolean is (Flyology_SIMD.Backends.Native.None_True (Mask.Low) and then Flyology_SIMD.Backends.Native.None_True (Mask.High)) with SPARK_Mode => On;

   function Population_Count (Mask : Mask_32x8) return Lane_Count_32x8 is (Lane_Count_32x8 (Flyology_SIMD.Backends.Native.Population_Count (Mask.Low) + Flyology_SIMD.Backends.Native.Population_Count (Mask.High))) with SPARK_Mode => On;
   ...
   function Load (Data : U32_Array; Start : Natural) return U32x8 is
     ((Low => Flyology_SIMD.Backends.Native.Load (Data, Start), High => Flyology_SIMD.Backends.Native.Load (Data, Start + 4))) with SPARK_Mode => On;
   ...
   function Load_Unaligned (Data : U32_Array; Start : Natural) return U32x8 is
     ((Low => Flyology_SIMD.Backends.Native.Load_Unaligned (Data, Start), High => Flyology_SIMD.Backends.Native.Load_Unaligned (Data, Start + 4))) with SPARK_Mode => On;
   ...
   function Load_Partial (Data : U32_Array; Start : Natural; Count : Lane_Count_32x8) return U32x8 is
     (if Count <= 4
      then (Low => Flyology_SIMD.Backends.Native.Load_Partial (Data, Start, Count), High => Flyology_SIMD.Backends.Native.Zero)
      else (Low => Flyology_SIMD.Backends.Native.Load (Data, Start), High => Flyology_SIMD.Backends.Native.Load_Partial (Data, Start + 4, Count - 4))) with SPARK_Mode => On;
   ...
   function To_Lanes (Value : I32x8) return Lane_Values_I32x8 with SPARK_Mode => On is
      Low : constant Lane_Values_I32x4 := Flyology_SIMD.Backends.Native.To_Lanes (Value.Low);
      High : constant Lane_Values_I32x4 := Flyology_SIMD.Backends.Native.To_Lanes (Value.High);
   begin
      return [for Lane in Lane_Index_32x8 =>
        (if Lane < 4 then Low (Lane)
         else High (Lane - 4))];
   ...
   function From_Lanes (Values : Lane_Values_U64x4) return U64x4 is
     ((Low => Flyology_SIMD.Backends.Native.From_Lanes ([for Lane in 0 .. 1 => Values (Lane)]),
       High => Flyology_SIMD.Backends.Native.From_Lanes ([for Lane in 0 .. 1 => Values (Lane + 2)]))) with SPARK_Mode => On;

   function To_Lanes (Value : U64x4) return Lane_Values_U64x4 with SPARK_Mode => On is
      Low : constant Lane_Values_U64x2 := Flyology_SIMD.Backends.Native.To_Lanes (Value.Low);
      High : constant Lane_Values_U64x2 := Flyology_SIMD.Backends.Native.To_Lanes (Value.High);
   begin
   ...
   function Extract (Value : U64x4; Lane : Lane_Index_64x4) return U64 is
     (if Lane < 2 then Flyology_SIMD.Backends.Native.Extract (Value.Low, Lane) else Flyology_SIMD.Backends.Native.Extract (Value.High, Lane - 2)) with SPARK_Mode => On;

   function Replace (Value : U64x4; Lane : Lane_Index_64x4; With_Value : U64) return U64x4 is
     (if Lane < 2
      then (Low => Flyology_SIMD.Backends.Native.Replace (Value.Low, Lane, With_Value), High => Value.High)
      else (Low => Value.Low, High => Flyology_SIMD.Backends.Native.Replace (Value.High, Lane - 2, With_Value))) with SPARK_Mode => On;
   ...
   function Mask_From_Bit_Mask (Bits : Mask_Bits_64x4) return Mask_64x4 is
     ((Low => Flyology_SIMD.Backends.Native.Mask_From_Bit_Mask (Bits and Mask_Bits_64x4 (3)),
       High => Flyology_SIMD.Backends.Native.Mask_From_Bit_Mask (Interfaces.Shift_Right (Bits, 2)))) with SPARK_Mode => On;

   function To_Bit_Mask (Mask : Mask_64x4) return Mask_Bits_64x4 is
     (Mask_Bits_64x4 (Flyology_SIMD.Backends.Native.To_Bit_Mask (Mask.Low)) or Interfaces.Shift_Left (Mask_Bits_64x4 (Flyology_SIMD.Backends.Native.To_Bit_Mask (Mask.High)), 2)) with SPARK_Mode => On;
   ...
   function Test (Mask : Mask_64x4; Lane : Lane_Index_64x4) return Boolean is
     (if Lane < 2 then Flyology_SIMD.Backends.Native.Test (Mask.Low, Lane) else Flyology_SIMD.Backends.Native.Test (Mask.High, Lane - 2)) with SPARK_Mode => On;

   function Any_True (Mask : Mask_64x4) return Boolean is (Flyology_SIMD.Backends.Native.Any_True (Mask.Low) or else Flyology_SIMD.Backends.Native.Any_True (Mask.High)) with SPARK_Mode => On;

   function All_True (Mask : Mask_64x4) return Boolean is (Flyology_SIMD.Backends.Native.All_True (Mask.Low) and then Flyology_SIMD.Backends.Native.All_True (Mask.High)) with SPARK_Mode => On;

   function None_True (Mask : Mask_64x4) return Boolean is (Flyology_SIMD.Backends.Native.None_True (Mask.Low) and then Flyology_SIMD.Backends.Native.None_True (Mask.High)) with SPARK_Mode => On;

   function Population_Count (Mask : Mask_64x4) return Lane_Count_64x4 is (Lane_Count_64x4 (Flyology_SIMD.Backends.Native.Population_Count (Mask.Low) + Flyology_SIMD.Backends.Native.Population_Count (Mask.High))) with SPARK_Mode => On;
   ...
   function Load (Data : U64_Array; Start : Natural) return U64x4 is
     ((Low => Flyology_SIMD.Backends.Native.Load (Data, Start), High => Flyology_SIMD.Backends.Native.Load (Data, Start + 2))) with SPARK_Mode => On;
   ...
   function Load_Unaligned (Data : U64_Array; Start : Natural) return U64x4 is
     ((Low => Flyology_SIMD.Backends.Native.Load_Unaligned (Data, Start), High => Flyology_SIMD.Backends.Native.Load_Unaligned (Data, Start + 2))) with SPARK_Mode => On;
   ...
   function Load_Partial (Data : U64_Array; Start : Natural; Count : Lane_Count_64x4) return U64x4 is
     (if Count <= 2
      then (Low => Flyology_SIMD.Backends.Native.Load_Partial (Data, Start, Count), High => Flyology_SIMD.Backends.Native.Zero)
      else (Low => Flyology_SIMD.Backends.Native.Load (Data, Start), High => Flyology_SIMD.Backends.Native.Load_Partial (Data, Start + 2, Count - 2))) with SPARK_Mode => On;
   ...
   function To_Lanes (Value : I64x4) return Lane_Values_I64x4 with SPARK_Mode => On is
      Low : constant Lane_Values_I64x2 := Flyology_SIMD.Backends.Native.To_Lanes (Value.Low);
      High : constant Lane_Values_I64x2 := Flyology_SIMD.Backends.Native.To_Lanes (Value.High);
   begin
      return [for Lane in Lane_Index_64x4 =>
        (if Lane < 2 then Low (Lane)
         else High (Lane - 2))];
   ...
   function To_Lanes (Value : F32x8) return Lane_Values_F32x8 with SPARK_Mode => On is
      Low : constant Lane_Values_F32x4 := Flyology_SIMD.Backends.Native.To_Lanes (Value.Low);
      High : constant Lane_Values_F32x4 := Flyology_SIMD.Backends.Native.To_Lanes (Value.High);
   begin
      return [for Lane in Lane_Index_32x8 =>
        (if Lane < 4 then Low (Lane)
         else High (Lane - 4))];
   ...
   function Add (Left, Right : F32x8) return F32x8 is
     (Float_Arithmetic_Mechanism.Add (Left, Right)) with SPARK_Mode => On;

   function Subtract (Left, Right : F32x8) return F32x8 is
     (Float_Arithmetic_Mechanism.Subtract (Left, Right)) with SPARK_Mode => On;

   function Multiply (Left, Right : F32x8) return F32x8 is
     (Float_Arithmetic_Mechanism.Multiply (Left, Right)) with SPARK_Mode => On;

   function Divide (Left, Right : F32x8) return F32x8 is
     (Float_Arithmetic_Mechanism.Divide (Left, Right)) with SPARK_Mode => On;

   function Min_Number (Left, Right : F32x8) return F32x8 is
     (Float_Arithmetic_Mechanism.Min_Number (Left, Right)) with SPARK_Mode => On;

   function Max_Number (Left, Right : F32x8) return F32x8 is
     (Float_Arithmetic_Mechanism.Max_Number (Left, Right)) with SPARK_Mode => On;
   ...
   function Unordered (Left, Right : F32x8) return Mask_32x8 is
     ((Low => Flyology_SIMD.Backends.Native.Unordered (Left.Low, Right.Low),
       High => Flyology_SIMD.Backends.Native.Unordered (Left.High, Right.High))) with SPARK_Mode => On;
   ...
   function Reduce_Min_Number (Value : F32x8) return F32 is
     (Float_Reduce_Mechanism.Reduce_Min_Number (Value)) with SPARK_Mode => On;

   function Reduce_Max_Number (Value : F32x8) return F32 is
     (Float_Reduce_Mechanism.Reduce_Max_Number (Value)) with SPARK_Mode => On;
   ...
   function To_Lanes (Value : F64x4) return Lane_Values_F64x4 with SPARK_Mode => On is
      Low : constant Lane_Values_F64x2 := Flyology_SIMD.Backends.Native.To_Lanes (Value.Low);
      High : constant Lane_Values_F64x2 := Flyology_SIMD.Backends.Native.To_Lanes (Value.High);
   begin
      return [for Lane in Lane_Index_64x4 =>
        (if Lane < 2 then Low (Lane)
         else High (Lane - 2))];
   ...
   function Widen_Low (Value : U8x32) return U16x16 is
     ((Low => Flyology_SIMD.Backends.Native.Widen_Low (Value.Low),
       High => Flyology_SIMD.Backends.Native.Widen_High (Value.Low))) with SPARK_Mode => On;

   function Widen_High (Value : U8x32) return U16x16 is
     ((Low => Flyology_SIMD.Backends.Native.Widen_Low (Value.High),
       High => Flyology_SIMD.Backends.Native.Widen_High (Value.High))) with SPARK_Mode => On;
   ...
   function Narrow_Truncate (Low, High : U16x16) return U8x32 is
     ((Low => Flyology_SIMD.Backends.Native.Narrow_Truncate (Low.Low, Low.High),
       High => Flyology_SIMD.Backends.Native.Narrow_Truncate (High.Low, High.High))) with SPARK_Mode => On;
   ...
   function Narrow_Saturate (Low, High : U16x16) return U8x32 is
     ((Low => Flyology_SIMD.Backends.Native.Narrow_Saturate (Low.Low, Low.High),
       High => Flyology_SIMD.Backends.Native.Narrow_Saturate (High.Low, High.High))) with SPARK_Mode => On;
   ...
   function Narrow_Round (Low, High : F64x4) return F32x8 is
     ((Low => Flyology_SIMD.Backends.Native.Narrow_Round (Low.Low, Low.High),
       High => Flyology_SIMD.Backends.Native.Narrow_Round (High.Low, High.High))) with SPARK_Mode => On;

   function Convert_Round (Value : I32x8) return F32x8 is
     ((Low => Flyology_SIMD.Backends.Native.Convert_Round (Value.Low),
       High => Flyology_SIMD.Backends.Native.Convert_Round (Value.High))) with SPARK_Mode => On;
   ...
   function Convert_Truncate_Saturate (Value : F32x8) return I32x8 is
     ((Low => Flyology_SIMD.Backends.Native.Convert_Truncate_Saturate (Value.Low),
       High => Flyology_SIMD.Backends.Native.Convert_Truncate_Saturate (Value.High))) with SPARK_Mode => On;
   ...
   function Convert_Saturate (Value : I8x32) return U8x32 is
     ((Low => Flyology_SIMD.Backends.Native.Convert_Saturate (Value.Low),
       High => Flyology_SIMD.Backends.Native.Convert_Saturate (Value.High))) with SPARK_Mode => On;
