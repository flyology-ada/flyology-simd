...
is
   ...
   begin
      Asm
        ("xgetbv",
         Outputs  =>
           [Interfaces.Unsigned_32'Asm_Output ("=a", Low), Interfaces.Unsigned_32'Asm_Output ("=d", High)],
         Inputs   => Interfaces.Unsigned_32'Asm_Input ("c", 0),
         Volatile => True);
      return Interfaces.Shift_Left (Interfaces.Unsigned_64 (High), 32) or Interfaces.Unsigned_64 (Low);
   end XCR0;
   ...
   Host_AVX2_Available : constant Boolean := (Configuration.AVX2_Compiled and then Host_Has_AVX2);
   ...
   is (if Available (AVX2) then AVX2 else SSE2);
