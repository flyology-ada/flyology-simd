...
package body Flyology_SIMD.Algorithms.Generic_Indexed_Byte_Search
  with SPARK_Mode => On
is
   ...
   function Find_First_Of (Data : Byte_Array_Type; Needles : Byte_Array_Type) return Search_Result is
      ...
      begin
         if Data'Length not in Natural'Range then
            for Index in Data'Range loop
               declare
                  Item : constant U8 := U8 (Data (Index));
               begin
                  if Item = Needle_0
         ...
         begin
            while Offset <= Length and then Length - Offset >= 16 loop
               pragma Loop_Invariant (Offset <= Length);
               pragma Loop_Variant (Decreases => Length - Offset);
               Value := Backend_Load_Unaligned (Data, Offset);
