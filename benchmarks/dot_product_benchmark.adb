...
procedure Dot_Product_Benchmark is
   ...
   Sizes : constant Size_List :=
     [3, 4, 5, 7, 8, 9, 4_096, 1_048_576];

   type F32_Array_Access is access all F32_Array;
   type F64_Array_Access is access all F64_Array;
   F32_Left  : constant F32_Array_Access :=
     new F32_Array (1 .. Sizes (Sizes'Last));
   F32_Right : constant F32_Array_Access :=
     new F32_Array (1 .. Sizes (Sizes'Last));
   F64_Left  : constant F64_Array_Access :=
     new F64_Array (1 .. Sizes (Sizes'Last));
   F64_Right : constant F64_Array_Access :=
     new F64_Array (1 .. Sizes (Sizes'Last));
         ...
         when Binary32 =>
            ...
            begin
               case Which is
                  ...
                  when Scalar =>
                     Value := Algorithms.Scalar_Floating.Dot_Product
                       (Left, Right);
                  when Native =>
                     Value := Algorithms.Native_Floating.Dot_Product
                       (Left, Right);
                  when Runtime =>
   ...
   begin
      if Ada.Environment_Variables.Exists ("FLYOLOGY_SIMD_BENCH_CPU") then
         ...
         begin
            Put_Line
              ("placement_cpu=" & CPU'Image &
               " strength=" & Strength'Image);
         end;
      else
            ...
            Config : constant Flyology_Bench.Configuration :=
              (if Output_Mode = "terminal"
               then Flyology_Bench.Reporters.Terminal_Mode
                 (Base_Config,
                  "dot elements=" & Size'Image &
                  " precision=" & Selected_Precision'Image)
               else Base_Config);
            Bytes_Per_Element : constant Positive :=
              (if Selected_Precision = Binary32 then 4 else 8);
            Input_Bytes : constant Positive :=
              2 * Size * Bytes_Per_Element;
         begin
            Compare_All (Config => Config, Result => Result);
            Put_Line
              ("input_elements=" & Size'Image &
               " input_bytes=" & Input_Bytes'Image &
               " precision=" & Selected_Precision'Image);
               ...
               for Which in Candidate loop
                  declare
                     Position : constant Flyology_Bench.Comparison_Case_Index :=
                       Flyology_Bench.Comparison_Case_Index
                         (Candidate'Pos (Which) + 1);
                     Median_NS : constant Long_Float :=
                       Flyology_Bench.Median_Nanoseconds
                         (Flyology_Bench.Case_Measurement
                            (Result, Position));
                  begin
                     Put_Line
                       ("throughput backend=" & Candidate'Image (Which) &
                        " precision=" & Selected_Precision'Image &
                        " median_GB_s=" &
                        Long_Float'Image
                          (Long_Float (Input_Bytes) / Median_NS));
                  end;
               end loop;
