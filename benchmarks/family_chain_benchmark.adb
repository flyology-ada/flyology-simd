...
procedure Family_Chain_Benchmark is
   ...
   Map_16     : constant Lane_Map_16x8 := Make_Lane_Map ([1, 0, 3, 2, 5, 4, 7, 6]);
   Two_Map_16 : constant Two_Source_Lane_Map_16x8 :=
     Make_Two_Source_Lane_Map
       ([for Lane in Lane_Index_16x8 =>
           (if Lane mod 2 = 0 then Select_Left_Lane (Lane) else Select_Right_Lane (Lane))]);
   ...
   generic
      ...
      with function Load_Unaligned (Data : U16_Array; Start : Natural) return U16x8;
      with function Greater_Than (Left, Right : U16x8) return Mask_16x8;
      with function Select_Value (Mask : Mask_16x8; If_True, If_False : U16x8) return U16x8;
      with function Permute_Lanes (Value : U16x8; Map : Lane_Map_16x8) return U16x8;
      with function Permute_Lanes (Left, Right : U16x8; Map : Two_Source_Lane_Map_16x8) return U16x8;
   ...
   begin
      while Start + 8 <= Limit loop
         declare
            ...
            Woven   : constant U16x8 := Permute_Lanes (Chosen, Rotated, Two_Map_16);
   ...
   generic
      ...
      with function Load_Unaligned (Data : U32_Array; Start : Natural) return U32x4;
      with function Equal (Left, Right : U32x4) return Mask_32x4;
      with function Select_Value (Mask : Mask_32x4; If_True, If_False : U32x4) return U32x4;
      with function Permute_Lanes (Value : U32x4; Map : Lane_Map_32x4) return U32x4;
   ...
   generic
      ...
      with function Load_Unaligned (Data : U64_Array; Start : Natural) return U64x2;
      with function Multiply_Wrap (Left, Right : U64x2) return U64x2;
      with function Greater_Than (Left, Right : U64x2) return Mask_64x2;
      with function Select_Value (Mask : Mask_64x2; If_True, If_False : U64x2) return U64x2;
      with function Permute_Lanes (Value : U64x2; Map : Lane_Map_64x2) return U64x2;
   ...
   generic
      ...
      with function Load_Unaligned (Data : F32_Array; Start : Natural) return F32x4;
      with function Greater_Than (Left, Right : F32x4) return Mask_32x4;
      with function Select_Value (Mask : Mask_32x4; If_True, If_False : F32x4) return F32x4;
      with function Permute_Lanes (Value : F32x4; Map : Lane_Map_32x4) return F32x4;
   ...
   begin
      while Start + 4 <= Limit loop
         ...
         begin
            Total := Total + Interfaces.Unsigned_64 (F32'Truncation (Reduce_Add (Floored)));
         end;
         Start := Start + 4;
      end loop;
      return Total;
   ...
   function Native_U16 is new
     Chain_U16
       (Backends.Native.Splat,
        Backends.Native.Load_Unaligned,
        Backends.Native.Greater_Than,
        Backends.Native.Select_Value,
        Backends.Native.Permute_Lanes,
        Backends.Native.Permute_Lanes,
        Backends.Native.Min,
        Backends.Native.Reduce_Add_Wrap);
   function Scalar_U16 is new
     Chain_U16
       (Backends.Scalar.Splat,
        Backends.Scalar.Load_Unaligned,
        Backends.Scalar.Greater_Than,
        Backends.Scalar.Select_Value,
        Backends.Scalar.Permute_Lanes,
        Backends.Scalar.Permute_Lanes,
        Backends.Scalar.Min,
        Backends.Scalar.Reduce_Add_Wrap);
   function Native_U32 is new
     Chain_U32
       (Backends.Native.Splat,
        Backends.Native.Load_Unaligned,
        Backends.Native.Equal,
        Backends.Native.Select_Value,
        Backends.Native.Permute_Lanes,
        Backends.Native.Max,
        Backends.Native.Narrow_Truncate,
        Backends.Native.Reduce_Add_Wrap);
   function Scalar_U32 is new
     Chain_U32
       (Backends.Scalar.Splat,
        Backends.Scalar.Load_Unaligned,
        Backends.Scalar.Equal,
        Backends.Scalar.Select_Value,
        Backends.Scalar.Permute_Lanes,
        Backends.Scalar.Max,
        Backends.Scalar.Narrow_Truncate,
        Backends.Scalar.Reduce_Add_Wrap);
   function Native_U64 is new
     Chain_U64
       (Backends.Native.Splat,
        Backends.Native.Load_Unaligned,
        Backends.Native.Multiply_Wrap,
        Backends.Native.Greater_Than,
        Backends.Native.Select_Value,
        Backends.Native.Permute_Lanes,
        Backends.Native.Reduce_Add_Wrap);
   function Scalar_U64 is new
     Chain_U64
       (Backends.Scalar.Splat,
        Backends.Scalar.Load_Unaligned,
        Backends.Scalar.Multiply_Wrap,
        Backends.Scalar.Greater_Than,
        Backends.Scalar.Select_Value,
        Backends.Scalar.Permute_Lanes,
        Backends.Scalar.Reduce_Add_Wrap);
   function Native_F32 is new
     Chain_F32
       (Backends.Native.Splat,
        Backends.Native.Load_Unaligned,
        Backends.Native.Greater_Than,
        Backends.Native.Select_Value,
        Backends.Native.Permute_Lanes,
        Backends.Native.Min_Number,
        Backends.Native.Reduce_Add);
   function Scalar_F32 is new
     Chain_F32
       (Backends.Scalar.Splat,
        Backends.Scalar.Load_Unaligned,
        Backends.Scalar.Greater_Than,
   ...
   begin
      case Current_Family is
         when Lanes_U16 =>
            return (if Which = Native then Native_U16 (Current_Size) else Scalar_U16 (Current_Size));

         when Lanes_U32 =>
            return (if Which = Native then Native_U32 (Current_Size) else Scalar_U32 (Current_Size));

         when Lanes_U64 =>
            return (if Which = Native then Native_U64 (Current_Size) else Scalar_U64 (Current_Size));

         when Lanes_F32 =>
            return (if Which = Native then Native_F32 (Current_Size) else Scalar_F32 (Current_Size));
      end case;
   end Run_Once;
   ...
   begin
      if Run_Once (Native) /= Expected then
         raise Program_Error
           with "benchmark validation failed for " & Current_Family'Image & Current_Size'Image;
      end if;
   end Validate;
   ...
   begin
      if Output_Mode /= "terminal" and then Output_Mode /= "json" and then Output_Mode /= "csv" then
              ...
              (if Output_Mode = "terminal"
               then
                 Flyology_Bench.Reporters.Terminal_Mode
                   (Config, "family=" & Which_Family'Image & " lanes=" & Size'Image)
               else Config);
         begin
            Compare_All (Config => Run_Config, Result => Result);
            Put_Line ("family=" & Which_Family'Image & " input_lanes=" & Size'Image);
