...
procedure SIMD_Benchmark is
   ...
   function Ordinary_Count
     (Buffer : Byte_Array; Needle : U8) return Natural
   is
      Result : Natural := 0;
   begin
...
begin
   ...
   Put_Line
     ("timer_cost_subtraction=false fast_math=false " &
      "quiescence=" & Boolean'Image (Quiet_CPU));
   Maybe_Pin;
         ...
         Config : constant Flyology_Bench.Configuration :=
           (if Output_Mode = "terminal"
            then Flyology_Bench.Reporters.Terminal_Mode
              (Base_Config, "count bytes=" & Size'Image)
            else Base_Config);
      begin
         Current_Size := Size;
         Validate;
               ...
               declare
                  ...
                  Median_NS : constant Long_Float :=
                    Flyology_Bench.Median_Nanoseconds
                      (Flyology_Bench.Case_Measurement (Result, Index));
               begin
                  Put_Line
