...
- optional AVX2 whole-buffer and floating-array objects and the optional Wide byte, floating,
...
two private parts independently. `Reduce_Add` starts from positive zero. The
minimum-number and maximum-number reductions start from lane 0. On AArch64,
dedicated Advanced SIMD sequences perform scalar `fadd`, `fminnm`, or `fmaxnm`
operations in ascending lane order. The x86-64 composed and AVX2 selections use
dedicated SSE2 leaves that process lanes in ascending order. A scalar build
...
`Flyology_SIMD.Algorithms.Generic_Bytes` and
`Flyology_SIMD.Algorithms.Generic_Floating` take backend operations as generic
formals. This makes static selection visible to the compiler and permits
...
generic supplies binary32 and binary64 scaling, clamping, AXPY, sums, number
extrema, and dot products. AVX2 runtime selection uses isolated complete-array routes. Scaling
...
Clamp reuses the exact selected 128-bit `Max_Number` then `Min_Number` loop in
the AVX2 facade so it retains the library's NaN and signed-zero semantics.
AXPY uses separate multiply and add operations. Its isolated AVX2 loop
broadcasts `A`, loads `X` and `Y`, multiplies, adds, and stores `Y` without
contracting the operations.
Complete-array number extrema reuse the exact selected 128-bit number-minimum
and number-maximum operations through the AVX2 facade.
