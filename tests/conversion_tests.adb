...
procedure Conversion_Tests is
   ...
   function Oracle_Convert_Saturate_I8x16_To_U8x16 (Item : I8) return U8 is (if Item < 0 then 0 else U8 (Item));
   function Oracle_Convert_Saturate_U8x16_To_I8x16 (Item : U8) return I8 is (if Item > U8 (I8'Last) then I8'Last else I8 (Item));
   function Oracle_Convert_Saturate_I16x8_To_U16x8 (Item : I16) return U16 is (if Item < 0 then 0 else U16 (Item));
   function Oracle_Convert_Saturate_U16x8_To_I16x8 (Item : U16) return I16 is (if Item > U16 (I16'Last) then I16'Last else I16 (Item));
   function Oracle_Convert_Saturate_I32x4_To_U32x4 (Item : I32) return U32 is (if Item < 0 then 0 else U32 (Item));
   function Oracle_Convert_Saturate_U32x4_To_I32x4 (Item : U32) return I32 is (if Item > U32 (I32'Last) then I32'Last else I32 (Item));
   function Oracle_Convert_Saturate_I64x2_To_U64x2 (Item : I64) return U64 is (if Item < 0 then 0 else U64 (Item));
   function Oracle_Convert_Saturate_U64x2_To_I64x2 (Item : U64) return I64 is (if Item > U64 (I64'Last) then I64'Last else I64 (Item));
   ...
   procedure Test_Bit_Cast_Special_Encodings is
      procedure Check_F32 (Bits : Lane_Values_U32x4) is
         Unsigned_Source : constant U32x4 := From_Lanes (Bits);
         Signed_Source : constant I32x4 := From_Lanes
           ([for Lane in Lane_Index_32x4 => I32_Of_Bits (Bits (Lane))]);
         Float_Source : constant F32x4 := From_Lanes
           ([for Lane in Lane_Index_32x4 => F32_Of_Bits (Bits (Lane))]);
         Scalar_U_To_I : constant I32x4 := Bit_Cast (Unsigned_Source);
         Native_U_To_I : constant I32x4 := Backends.Native.Bit_Cast (Unsigned_Source);
         Scalar_U_To_F : constant F32x4 := Bit_Cast (Unsigned_Source);
         Native_U_To_F : constant F32x4 := Backends.Native.Bit_Cast (Unsigned_Source);
         Scalar_I_To_U : constant U32x4 := Bit_Cast (Signed_Source);
         Native_I_To_U : constant U32x4 := Backends.Native.Bit_Cast (Signed_Source);
         Scalar_I_To_F : constant F32x4 := Bit_Cast (Signed_Source);
         Native_I_To_F : constant F32x4 := Backends.Native.Bit_Cast (Signed_Source);
         Scalar_F_To_U : constant U32x4 := Bit_Cast (Float_Source);
         Native_F_To_U : constant U32x4 := Backends.Native.Bit_Cast (Float_Source);
         Scalar_F_To_I : constant I32x4 := Bit_Cast (Float_Source);
         Native_F_To_I : constant I32x4 := Backends.Native.Bit_Cast (Float_Source);
      begin
         for Lane in Lane_Index_32x4 loop
            Check (U32_Of_Bits (Extract (Scalar_U_To_I, Lane)) = Bits (Lane) and then U32_Of_Bits (Extract (Native_U_To_I, Lane)) = Bits (Lane), "U32-to-I32 fixed bit cast");
            Check (U32_Of_Bits (Extract (Scalar_U_To_F, Lane)) = Bits (Lane) and then U32_Of_Bits (Extract (Native_U_To_F, Lane)) = Bits (Lane), "U32-to-F32 fixed bit cast");
            Check (Extract (Scalar_I_To_U, Lane) = Bits (Lane) and then Extract (Native_I_To_U, Lane) = Bits (Lane), "I32-to-U32 fixed bit cast");
            Check (U32_Of_Bits (Extract (Scalar_I_To_F, Lane)) = Bits (Lane) and then U32_Of_Bits (Extract (Native_I_To_F, Lane)) = Bits (Lane), "I32-to-F32 fixed bit cast");
            Check (Extract (Scalar_F_To_U, Lane) = Bits (Lane) and then Extract (Native_F_To_U, Lane) = Bits (Lane), "F32-to-U32 fixed bit cast");
            Check (U32_Of_Bits (Extract (Scalar_F_To_I, Lane)) = Bits (Lane) and then U32_Of_Bits (Extract (Native_F_To_I, Lane)) = Bits (Lane), "F32-to-I32 fixed bit cast");
         end loop;
      end Check_F32;

      procedure Check_F64 (Bits : Lane_Values_U64x2) is
         Unsigned_Source : constant U64x2 := From_Lanes (Bits);
         Signed_Source : constant I64x2 := From_Lanes
           ([for Lane in Lane_Index_64x2 => I64_Of_Bits (Bits (Lane))]);
         Float_Source : constant F64x2 := From_Lanes
           ([for Lane in Lane_Index_64x2 => F64_Of_Bits (Bits (Lane))]);
         Scalar_U_To_I : constant I64x2 := Bit_Cast (Unsigned_Source);
         Native_U_To_I : constant I64x2 := Backends.Native.Bit_Cast (Unsigned_Source);
         Scalar_U_To_F : constant F64x2 := Bit_Cast (Unsigned_Source);
         Native_U_To_F : constant F64x2 := Backends.Native.Bit_Cast (Unsigned_Source);
         Scalar_I_To_U : constant U64x2 := Bit_Cast (Signed_Source);
         Native_I_To_U : constant U64x2 := Backends.Native.Bit_Cast (Signed_Source);
         Scalar_I_To_F : constant F64x2 := Bit_Cast (Signed_Source);
         Native_I_To_F : constant F64x2 := Backends.Native.Bit_Cast (Signed_Source);
         Scalar_F_To_U : constant U64x2 := Bit_Cast (Float_Source);
         Native_F_To_U : constant U64x2 := Backends.Native.Bit_Cast (Float_Source);
         Scalar_F_To_I : constant I64x2 := Bit_Cast (Float_Source);
         Native_F_To_I : constant I64x2 := Backends.Native.Bit_Cast (Float_Source);
      begin
         for Lane in Lane_Index_64x2 loop
            Check (U64_Of_Bits (Extract (Scalar_U_To_I, Lane)) = Bits (Lane) and then U64_Of_Bits (Extract (Native_U_To_I, Lane)) = Bits (Lane), "U64-to-I64 fixed bit cast");
            Check (U64_Of_Bits (Extract (Scalar_U_To_F, Lane)) = Bits (Lane) and then U64_Of_Bits (Extract (Native_U_To_F, Lane)) = Bits (Lane), "U64-to-F64 fixed bit cast");
            Check (Extract (Scalar_I_To_U, Lane) = Bits (Lane) and then Extract (Native_I_To_U, Lane) = Bits (Lane), "I64-to-U64 fixed bit cast");
            Check (U64_Of_Bits (Extract (Scalar_I_To_F, Lane)) = Bits (Lane) and then U64_Of_Bits (Extract (Native_I_To_F, Lane)) = Bits (Lane), "I64-to-F64 fixed bit cast");
            Check (Extract (Scalar_F_To_U, Lane) = Bits (Lane) and then Extract (Native_F_To_U, Lane) = Bits (Lane), "F64-to-U64 fixed bit cast");
            Check (U64_Of_Bits (Extract (Scalar_F_To_I, Lane)) = Bits (Lane) and then U64_Of_Bits (Extract (Native_F_To_I, Lane)) = Bits (Lane), "F64-to-I64 fixed bit cast");
         end loop;
      end Check_F64;
   begin
      declare
         Bits : constant Lane_Values_8x16 :=
           [0, 1, 16#7F#, 16#80#, 16#FF#, 16#AA#, 16#55#, 2,
            16#FE#, 16#40#, 16#C0#, 3, 16#FD#, 16#0F#, 16#F0#, 16#81#];
         Unsigned_Source : constant U8x16 := From_Lanes (Bits);
         Signed_Source : constant I8x16 := From_Lanes
           ([for Lane in Lane_Index_8x16 => I8_Of_Bits (Bits (Lane))]);
         Scalar_U_To_I : constant I8x16 := Bit_Cast (Unsigned_Source);
         Native_U_To_I : constant I8x16 := Backends.Native.Bit_Cast (Unsigned_Source);
         Scalar_I_To_U : constant U8x16 := Bit_Cast (Signed_Source);
         Native_I_To_U : constant U8x16 := Backends.Native.Bit_Cast (Signed_Source);
      begin
         for Lane in Lane_Index_8x16 loop
            Check (Cast_I8_To_U8 (Extract (Scalar_U_To_I, Lane)) = Bits (Lane) and then Cast_I8_To_U8 (Extract (Native_U_To_I, Lane)) = Bits (Lane), "U8-to-I8 fixed bit cast");
            Check (Extract (Scalar_I_To_U, Lane) = Bits (Lane) and then Extract (Native_I_To_U, Lane) = Bits (Lane), "I8-to-U8 fixed bit cast");
         end loop;
      end;
      declare
         Bits : constant Lane_Values_U16x8 :=
           [0, 1, 16#7FFF#, 16#8000#, 16#FFFF#, 16#AAAA#, 16#5555#, 16#8001#];
         Unsigned_Source : constant U16x8 := From_Lanes (Bits);
         Signed_Source : constant I16x8 := From_Lanes
           ([for Lane in Lane_Index_16x8 => I16_Of_Bits (Bits (Lane))]);
         Scalar_U_To_I : constant I16x8 := Bit_Cast (Unsigned_Source);
         Native_U_To_I : constant I16x8 := Backends.Native.Bit_Cast (Unsigned_Source);
         Scalar_I_To_U : constant U16x8 := Bit_Cast (Signed_Source);
         Native_I_To_U : constant U16x8 := Backends.Native.Bit_Cast (Signed_Source);
      begin
         for Lane in Lane_Index_16x8 loop
            Check (U16_Of_Bits (Extract (Scalar_U_To_I, Lane)) = Bits (Lane) and then U16_Of_Bits (Extract (Native_U_To_I, Lane)) = Bits (Lane), "U16-to-I16 fixed bit cast");
            Check (Extract (Scalar_I_To_U, Lane) = Bits (Lane) and then Extract (Native_I_To_U, Lane) = Bits (Lane), "I16-to-U16 fixed bit cast");
         end loop;
      end;
      Check_F32 ([16#0000_0000#, 16#8000_0000#, 16#0000_0001#, 16#8000_0001#]);
      Check_F32 ([16#7F80_0000#, 16#FF80_0000#, 16#7FC0_0001#, 16#7F80_0001#]);
      Check_F32 ([16#FFC0_0001#, 16#FF80_0001#, 16#007F_FFFF#, 16#807F_FFFF#]);
      Check_F64 ([16#0000_0000_0000_0000#, 16#8000_0000_0000_0000#]);
      Check_F64 ([16#0000_0000_0000_0001#, 16#8000_0000_0000_0001#]);
      Check_F64 ([16#7FF0_0000_0000_0000#, 16#FFF0_0000_0000_0000#]);
      Check_F64 ([16#7FF8_0000_0000_0001#, 16#7FF0_0000_0000_0001#]);
      Check_F64 ([16#FFF8_0000_0000_0001#, 16#FFF0_0000_0000_0001#]);
   end Test_Bit_Cast_Special_Encodings;
...
begin
   ...
   Test_Bit_Cast_Special_Encodings;
