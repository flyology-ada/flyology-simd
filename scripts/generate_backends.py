...
def x86_body() -> str:
    ...
    for vector, scalar, bits, lanes, signed in INTEGER_TYPES:
        ...
        out += [
            ...
            f"   function Shift_Left_Logical (Value : {vector}; Count : Natural) return {vector} is (Native_SHL_{vector} (Value, Interfaces.Unsigned_32 (Natural'Min (Count, {bits}))));",
            f"   function Shift_Right_Logical (Value : {vector}; Count : Natural) return {vector} is (Native_SHR_{vector} (Value, Interfaces.Unsigned_32 (Natural'Min (Count, {bits}))));",
        ]
        if signed:
            if bits < 64:
                out += [
    ...
    for vector, scalar, bits, lanes, signed in INTEGER_TYPES:
        ...
        lines += [
            ...
            *sub_sat_body,
            ...
            f"   function Reference_Comparison_{vector} (Left, Right : {vector}; Relation : Natural) return {mask_storage} is",
            f"      Result : {mask_storage} := 0;",
            "   begin",
            ...
            f"            Result := Result or Interfaces.Shift_Left ({mask_storage} (1), Lane);",
            "         end if;",
            "      end loop;",
            "      return Result;",
            f"   end Reference_Comparison_{vector};",
            ...
            f"   function Reference_Shift_Left_Logical_{vector} (Value : {vector}; Count : Natural) return {vector} is",
            f"      Result : {vector} := Zero;",
            f"      Raw : {unsigned};",
            "   begin",
            f"      for Lane in {lane_index(bits, lanes)} loop",
            f"         Raw := {vector + '_To_Bits (Extract (Value, Lane))' if signed else unsigned + ' (Extract (Value, Lane))'};",
            f"         Result := Replace (Result, Lane, {('Bits_To_' + vector) if signed else scalar} ((if Count >= {bits} then 0 else Interfaces.Shift_Left (Raw, Count))));",
            "      end loop;",
            "      return Result;",
            f"   end Reference_Shift_Left_Logical_{vector};",
            f"   function Reference_Shift_Right_Logical_{vector} (Value : {vector}; Count : Natural) return {vector} is",
            f"      Result : {vector} := Zero;",
            f"      Raw : {unsigned};",
            "   begin",
            f"      for Lane in {lane_index(bits, lanes)} loop",
            f"         Raw := {vector + '_To_Bits (Extract (Value, Lane))' if signed else unsigned + ' (Extract (Value, Lane))'};",
            f"         Result := Replace (Result, Lane, {('Bits_To_' + vector) if signed else scalar} ((if Count >= {bits} then 0 else Interfaces.Shift_Right (Raw, Count))));",
            "      end loop;",
            "      return Result;",
            f"   end Reference_Shift_Right_Logical_{vector};",
            *(
                [
        ...
        lines += [
            ...
            f"         Check (Same (Shift_Left_Logical (A, Shift), Reference_Shift_Left_Logical_{vector} (A, Shift)) and then Same (Backends.Native.Shift_Left_Logical (A, Shift), Reference_Shift_Left_Logical_{vector} (A, Shift)), \"{vector} independent logical left shift\" & Shift'Image);",
            f"         Check (Same (Shift_Right_Logical (A, Shift), Reference_Shift_Right_Logical_{vector} (A, Shift)) and then Same (Backends.Native.Shift_Right_Logical (A, Shift), Reference_Shift_Right_Logical_{vector} (A, Shift)), \"{vector} independent logical right shift\" & Shift'Image);",
        ]
        if signed:
        ...
        lines += [
            ...
            f"      Check (Same (Shift_Left_Logical (A, Natural'Last), Reference_Shift_Left_Logical_{vector} (A, Natural'Last)) and then Same (Backends.Native.Shift_Left_Logical (A, Natural'Last), Reference_Shift_Left_Logical_{vector} (A, Natural'Last)) and then Same (Shift_Right_Logical (A, Natural'Last), Reference_Shift_Right_Logical_{vector} (A, Natural'Last)) and then Same (Backends.Native.Shift_Right_Logical (A, Natural'Last), Reference_Shift_Right_Logical_{vector} (A, Natural'Last)), \"{vector} maximum-count independent logical shifts\");",
        ...
        lines += [
            ...
            f"         for Lane in {lane_index(bits, lanes)} loop",
            f"            Check (Extract (Slide_Lanes_Toward_Low (A, Slide), Lane) = (if Slide < {lanes} and then Lane < {lanes} - Slide then Extract (A, {lane_index(bits, lanes)} (Lane + Slide)) else 0) and then Backends.Native.Extract (Backends.Native.Slide_Lanes_Toward_Low (A, Slide), Lane) = (if Slide < {lanes} and then Lane < {lanes} - Slide then Extract (A, {lane_index(bits, lanes)} (Lane + Slide)) else 0), \"{vector} independent scalar and native slide toward low\" & Slide'Image & Lane'Image);",
            f"            Check (Extract (Slide_Lanes_Toward_High (A, Slide), Lane) = (if Slide < {lanes} and then Lane >= Slide then Extract (A, {lane_index(bits, lanes)} (Lane - Slide)) else 0) and then Backends.Native.Extract (Backends.Native.Slide_Lanes_Toward_High (A, Slide), Lane) = (if Slide < {lanes} and then Lane >= Slide then Extract (A, {lane_index(bits, lanes)} (Lane - Slide)) else 0), \"{vector} independent scalar and native slide toward high\" & Slide'Image & Lane'Image);",
            "         end loop;",
            "      end loop;",
            f"      Check (Same (Slide_Lanes_Toward_Low (A, Natural'Last), Zero) and then Same (Backends.Native.Slide_Lanes_Toward_Low (A, Natural'Last), Zero) and then Same (Slide_Lanes_Toward_High (A, Natural'Last), Zero) and then Same (Backends.Native.Slide_Lanes_Toward_High (A, Natural'Last), Zero), \"{vector} maximum-count lane slides\");",
            f"      for Lane in {lane_index(bits, lanes)} loop",
            f"         Check (Extract (Reverse_Lanes (A), Lane) = Extract (A, {lane_index(bits, lanes)} ({lanes - 1} - Lane)) and then Backends.Scalar.Extract (Backends.Scalar.Reverse_Lanes (A), Lane) = Extract (A, {lane_index(bits, lanes)} ({lanes - 1} - Lane)) and then Backends.Native.Extract (Backends.Native.Reverse_Lanes (A), Lane) = Extract (A, {lane_index(bits, lanes)} ({lanes - 1} - Lane)), \"{vector} independent root, Scalar, and Native reverse\" & Lane'Image);",
            f"         Check (Extract (Interleave_Low (A, B), Lane) = (if Lane mod 2 = 0 then Extract (A, {lane_index(bits, lanes)} (Lane / 2)) else Extract (B, {lane_index(bits, lanes)} (Lane / 2))) and then Backends.Scalar.Extract (Backends.Scalar.Interleave_Low (A, B), Lane) = (if Lane mod 2 = 0 then Extract (A, {lane_index(bits, lanes)} (Lane / 2)) else Extract (B, {lane_index(bits, lanes)} (Lane / 2))) and then Backends.Native.Extract (Backends.Native.Interleave_Low (A, B), Lane) = (if Lane mod 2 = 0 then Extract (A, {lane_index(bits, lanes)} (Lane / 2)) else Extract (B, {lane_index(bits, lanes)} (Lane / 2))), \"{vector} independent root, Scalar, and Native interleave low\" & Lane'Image);",
            f"         Check (Extract (Interleave_High (A, B), Lane) = (if Lane mod 2 = 0 then Extract (A, {lane_index(bits, lanes)} ({lanes // 2} + Lane / 2)) else Extract (B, {lane_index(bits, lanes)} ({lanes // 2} + Lane / 2))) and then Backends.Scalar.Extract (Backends.Scalar.Interleave_High (A, B), Lane) = (if Lane mod 2 = 0 then Extract (A, {lane_index(bits, lanes)} ({lanes // 2} + Lane / 2)) else Extract (B, {lane_index(bits, lanes)} ({lanes // 2} + Lane / 2))) and then Backends.Native.Extract (Backends.Native.Interleave_High (A, B), Lane) = (if Lane mod 2 = 0 then Extract (A, {lane_index(bits, lanes)} ({lanes // 2} + Lane / 2)) else Extract (B, {lane_index(bits, lanes)} ({lanes // 2} + Lane / 2))), \"{vector} independent root, Scalar, and Native interleave high\" & Lane'Image);",
            f"         Check (Extract (Deinterleave_Even (A, B), Lane) = (if Lane < {lanes // 2} then Extract (A, {lane_index(bits, lanes)} (2 * Lane)) else Extract (B, {lane_index(bits, lanes)} (2 * (Lane - {lanes // 2})))) and then Backends.Scalar.Extract (Backends.Scalar.Deinterleave_Even (A, B), Lane) = (if Lane < {lanes // 2} then Extract (A, {lane_index(bits, lanes)} (2 * Lane)) else Extract (B, {lane_index(bits, lanes)} (2 * (Lane - {lanes // 2})))) and then Backends.Native.Extract (Backends.Native.Deinterleave_Even (A, B), Lane) = (if Lane < {lanes // 2} then Extract (A, {lane_index(bits, lanes)} (2 * Lane)) else Extract (B, {lane_index(bits, lanes)} (2 * (Lane - {lanes // 2})))), \"{vector} independent root, Scalar, and Native deinterleave even\" & Lane'Image);",
            f"         Check (Extract (Deinterleave_Odd (A, B), Lane) = (if Lane < {lanes // 2} then Extract (A, {lane_index(bits, lanes)} (2 * Lane + 1)) else Extract (B, {lane_index(bits, lanes)} (2 * (Lane - {lanes // 2}) + 1))) and then Backends.Scalar.Extract (Backends.Scalar.Deinterleave_Odd (A, B), Lane) = (if Lane < {lanes // 2} then Extract (A, {lane_index(bits, lanes)} (2 * Lane + 1)) else Extract (B, {lane_index(bits, lanes)} (2 * (Lane - {lanes // 2}) + 1))) and then Backends.Native.Extract (Backends.Native.Deinterleave_Odd (A, B), Lane) = (if Lane < {lanes // 2} then Extract (A, {lane_index(bits, lanes)} (2 * Lane + 1)) else Extract (B, {lane_index(bits, lanes)} (2 * (Lane - {lanes // 2}) + 1))), \"{vector} independent root, Scalar, and Native deinterleave odd\" & Lane'Image);",
            "      end loop;",
        ]
        for relation, name in enumerate(("Equal", "Less_Than", "Less_Equal", "Greater_Than", "Greater_Equal")):
            lines.append(f"      Check (To_Bit_Mask ({name} (A, B)) = Reference_Comparison_{vector} (A, B, {relation}) and then Backends.Scalar.To_Bit_Mask (Backends.Scalar.{name} (A, B)) = Reference_Comparison_{vector} (A, B, {relation}) and then Backends.Native.To_Bit_Mask (Backends.Native.{name} (A, B)) = Reference_Comparison_{vector} (A, B, {relation}), \"{vector} independent root, Scalar, and Native {name}\");")
        lines += [
            f"      Check (Same (Backends.Scalar.Select_Value (Backends.Scalar.Equal (A, B), A, B), Select_Value (Equal (A, B), A, B)) and then Same (Backends.Native.Select_Value (Equal (A, B), A, B), Select_Value (Equal (A, B), A, B)), \"{vector} scalar and native select\");",
            ...
            f"         Check (Same (Backends.Scalar.Select_Value (Backends.Scalar.Mask_From_Bit_Mask ({mask_storage} (Pattern)), A, B), Select_Value (Mask_From_Bit_Mask ({mask_storage} (Pattern)), A, B)) and then Same (Backends.Native.Select_Value (Backends.Native.Mask_From_Bit_Mask ({mask_storage} (Pattern)), A, B), Select_Value (Mask_From_Bit_Mask ({mask_storage} (Pattern)), A, B)), \"{vector} exhaustive scalar and native select\" & Pattern'Image);",
            ...
            f"         for Lane in {lane_index(bits, lanes)} loop Check (Extract (Select_Value (Mask_From_Bit_Mask ({mask_storage} (Pattern)), A, B), Lane) = (if (Pattern / 2 ** Lane) mod 2 = 1 then Extract (A, Lane) else Extract (B, Lane)) and then Backends.Scalar.Extract (Backends.Scalar.Select_Value (Backends.Scalar.Mask_From_Bit_Mask ({mask_storage} (Pattern)), A, B), Lane) = (if (Pattern / 2 ** Lane) mod 2 = 1 then Extract (A, Lane) else Extract (B, Lane)) and then Backends.Native.Extract (Backends.Native.Select_Value (Backends.Native.Mask_From_Bit_Mask ({mask_storage} (Pattern)), A, B), Lane) = (if (Pattern / 2 ** Lane) mod 2 = 1 then Extract (A, Lane) else Extract (B, Lane)), \"{vector} independent root, Scalar, and Native select\" & Pattern'Image & Lane'Image); end loop;",
            "      end loop;",
            ...
            f"            Check (To_Bit_Mask (Equal (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 0) and then Backends.Scalar.To_Bit_Mask (Backends.Scalar.Equal (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 0) and then Backends.Native.To_Bit_Mask (Backends.Native.Equal (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 0) and then To_Bit_Mask (Less_Than (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 1) and then Backends.Scalar.To_Bit_Mask (Backends.Scalar.Less_Than (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 1) and then Backends.Native.To_Bit_Mask (Backends.Native.Less_Than (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 1) and then To_Bit_Mask (Less_Equal (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 2) and then Backends.Scalar.To_Bit_Mask (Backends.Scalar.Less_Equal (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 2) and then Backends.Native.To_Bit_Mask (Backends.Native.Less_Equal (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 2) and then To_Bit_Mask (Greater_Than (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 3) and then Backends.Scalar.To_Bit_Mask (Backends.Scalar.Greater_Than (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 3) and then Backends.Native.To_Bit_Mask (Backends.Native.Greater_Than (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 3) and then To_Bit_Mask (Greater_Equal (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 4) and then Backends.Scalar.To_Bit_Mask (Backends.Scalar.Greater_Equal (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 4) and then Backends.Native.To_Bit_Mask (Backends.Native.Greater_Equal (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 4), \"{vector} randomized independent root, Scalar, and Native comparisons\");",
            f"            Check (Same (Shift_Left_Logical (R_A, Shift), Reference_Shift_Left_Logical_{vector} (R_A, Shift)) and then Same (Backends.Native.Shift_Left_Logical (R_A, Shift), Reference_Shift_Left_Logical_{vector} (R_A, Shift)) and then Same (Shift_Right_Logical (R_A, Shift), Reference_Shift_Right_Logical_{vector} (R_A, Shift)) and then Same (Backends.Native.Shift_Right_Logical (R_A, Shift), Reference_Shift_Right_Logical_{vector} (R_A, Shift)), \"{vector} randomized independent logical shifts\");",
            *(([
            ...
            f"            Check (Same (Backends.Scalar.Select_Value (Backends.Scalar.Mask_From_Bit_Mask (Pattern), R_A, R_B), Select_Value (Mask_From_Bit_Mask (Pattern), R_A, R_B)) and then Same (Backends.Native.Select_Value (Backends.Native.Mask_From_Bit_Mask (Pattern), R_A, R_B), Select_Value (Mask_From_Bit_Mask (Pattern), R_A, R_B)), \"{vector} randomized scalar and native select\");",
            ...
            f"            for Lane in {lane_index(bits, lanes)} loop",
            f"               Check (Extract (Reverse_Lanes (R_A), Lane) = R_Lanes ({lane_index(bits, lanes)} ({lanes - 1} - Lane)) and then Backends.Scalar.Extract (Backends.Scalar.Reverse_Lanes (R_A), Lane) = R_Lanes ({lane_index(bits, lanes)} ({lanes - 1} - Lane)) and then Backends.Native.Extract (Backends.Native.Reverse_Lanes (R_A), Lane) = R_Lanes ({lane_index(bits, lanes)} ({lanes - 1} - Lane)), \"{vector} randomized independent root, Scalar, and Native reverse\" & Lane'Image);",
            f"               Check (Extract (Interleave_Low (R_A, R_B), Lane) = (if Lane mod 2 = 0 then Extract (R_A, {lane_index(bits, lanes)} (Lane / 2)) else Extract (R_B, {lane_index(bits, lanes)} (Lane / 2))) and then Backends.Scalar.Extract (Backends.Scalar.Interleave_Low (R_A, R_B), Lane) = (if Lane mod 2 = 0 then Extract (R_A, {lane_index(bits, lanes)} (Lane / 2)) else Extract (R_B, {lane_index(bits, lanes)} (Lane / 2))) and then Backends.Native.Extract (Backends.Native.Interleave_Low (R_A, R_B), Lane) = (if Lane mod 2 = 0 then Extract (R_A, {lane_index(bits, lanes)} (Lane / 2)) else Extract (R_B, {lane_index(bits, lanes)} (Lane / 2))), \"{vector} randomized independent root, Scalar, and Native interleave low\" & Lane'Image);",
            f"               Check (Extract (Interleave_High (R_A, R_B), Lane) = (if Lane mod 2 = 0 then Extract (R_A, {lane_index(bits, lanes)} ({lanes // 2} + Lane / 2)) else Extract (R_B, {lane_index(bits, lanes)} ({lanes // 2} + Lane / 2))) and then Backends.Scalar.Extract (Backends.Scalar.Interleave_High (R_A, R_B), Lane) = (if Lane mod 2 = 0 then Extract (R_A, {lane_index(bits, lanes)} ({lanes // 2} + Lane / 2)) else Extract (R_B, {lane_index(bits, lanes)} ({lanes // 2} + Lane / 2))) and then Backends.Native.Extract (Backends.Native.Interleave_High (R_A, R_B), Lane) = (if Lane mod 2 = 0 then Extract (R_A, {lane_index(bits, lanes)} ({lanes // 2} + Lane / 2)) else Extract (R_B, {lane_index(bits, lanes)} ({lanes // 2} + Lane / 2))), \"{vector} randomized independent root, Scalar, and Native interleave high\" & Lane'Image);",
            f"               Check (Extract (Deinterleave_Even (R_A, R_B), Lane) = (if Lane < {lanes // 2} then Extract (R_A, {lane_index(bits, lanes)} (2 * Lane)) else Extract (R_B, {lane_index(bits, lanes)} (2 * (Lane - {lanes // 2})))) and then Backends.Scalar.Extract (Backends.Scalar.Deinterleave_Even (R_A, R_B), Lane) = (if Lane < {lanes // 2} then Extract (R_A, {lane_index(bits, lanes)} (2 * Lane)) else Extract (R_B, {lane_index(bits, lanes)} (2 * (Lane - {lanes // 2})))) and then Backends.Native.Extract (Backends.Native.Deinterleave_Even (R_A, R_B), Lane) = (if Lane < {lanes // 2} then Extract (R_A, {lane_index(bits, lanes)} (2 * Lane)) else Extract (R_B, {lane_index(bits, lanes)} (2 * (Lane - {lanes // 2})))), \"{vector} randomized independent root, Scalar, and Native deinterleave even\" & Lane'Image);",
            f"               Check (Extract (Deinterleave_Odd (R_A, R_B), Lane) = (if Lane < {lanes // 2} then Extract (R_A, {lane_index(bits, lanes)} (2 * Lane + 1)) else Extract (R_B, {lane_index(bits, lanes)} (2 * (Lane - {lanes // 2}) + 1))) and then Backends.Scalar.Extract (Backends.Scalar.Deinterleave_Odd (R_A, R_B), Lane) = (if Lane < {lanes // 2} then Extract (R_A, {lane_index(bits, lanes)} (2 * Lane + 1)) else Extract (R_B, {lane_index(bits, lanes)} (2 * (Lane - {lanes // 2}) + 1))) and then Backends.Native.Extract (Backends.Native.Deinterleave_Odd (R_A, R_B), Lane) = (if Lane < {lanes // 2} then Extract (R_A, {lane_index(bits, lanes)} (2 * Lane + 1)) else Extract (R_B, {lane_index(bits, lanes)} (2 * (Lane - {lanes // 2}) + 1))), \"{vector} randomized independent root, Scalar, and Native deinterleave odd\" & Lane'Image);",
    ...
    for vector, scalar, bits, lanes in FLOAT_TYPES:
        ...
        lines += [
            ...
            *complete_memory_test_lines(vector, scalar, bits, lanes, arr, vals, True),
            ...
            "      return Result;",
            f"   end Reference_Reduce_Max_{vector};",
            f"   function Reference_Comparison_{vector} (Left, Right : {vector}; Relation : Natural) return Interfaces.Unsigned_8 is",
            "      Result : Interfaces.Unsigned_8 := 0;",
            "   begin",
            f"      for Lane in {lane_index(bits, lanes)} loop",
            f"         declare",
            f"            Left_Bits : constant Interfaces.Unsigned_{bits} := Bits_{vector} (Extract (Left, Lane));",
            f"            Right_Bits : constant Interfaces.Unsigned_{bits} := Bits_{vector} (Extract (Right, Lane));",
            f"            Left_NaN : constant Boolean := (Left_Bits and {'16#7F80_0000#' if bits == 32 else '16#7FF0_0000_0000_0000#'}) = {'16#7F80_0000#' if bits == 32 else '16#7FF0_0000_0000_0000#'} and then (Left_Bits and {'16#007F_FFFF#' if bits == 32 else '16#000F_FFFF_FFFF_FFFF#'}) /= 0;",
            f"            Right_NaN : constant Boolean := (Right_Bits and {'16#7F80_0000#' if bits == 32 else '16#7FF0_0000_0000_0000#'}) = {'16#7F80_0000#' if bits == 32 else '16#7FF0_0000_0000_0000#'} and then (Right_Bits and {'16#007F_FFFF#' if bits == 32 else '16#000F_FFFF_FFFF_FFFF#'}) /= 0;",
            "         begin",
            "         if (case Relation is",
            ...
            "      return Result;",
            f"   end Reference_Comparison_{vector};",
            f"   procedure Check_Arrangements_{vector} (Left_Lanes, Right_Lanes : {vals}; Label_Text : String) is",
            f"      Left : constant {vector} := From_Lanes (Left_Lanes);",
            f"      Right : constant {vector} := From_Lanes (Right_Lanes);",
            f"      Reverse_Expected : constant {vector} := From_Lanes ([for Lane in {lane_index(bits, lanes)} => Left_Lanes ({lane_index(bits, lanes)} ({lanes - 1} - Lane))]);",
            f"      Interleave_Low_Expected : constant {vector} := From_Lanes ([for Lane in {lane_index(bits, lanes)} => (if Lane mod 2 = 0 then Left_Lanes ({lane_index(bits, lanes)} (Lane / 2)) else Right_Lanes ({lane_index(bits, lanes)} (Lane / 2)))]);",
            f"      Interleave_High_Expected : constant {vector} := From_Lanes ([for Lane in {lane_index(bits, lanes)} => (if Lane mod 2 = 0 then Left_Lanes ({lane_index(bits, lanes)} ({lanes // 2} + Lane / 2)) else Right_Lanes ({lane_index(bits, lanes)} ({lanes // 2} + Lane / 2)))]);",
            f"      Deinterleave_Even_Expected : constant {vector} := From_Lanes ([for Lane in {lane_index(bits, lanes)} => (if Lane < {lanes // 2} then Left_Lanes ({lane_index(bits, lanes)} (2 * Lane)) else Right_Lanes ({lane_index(bits, lanes)} (2 * (Lane - {lanes // 2}))))]);",
            f"      Deinterleave_Odd_Expected : constant {vector} := From_Lanes ([for Lane in {lane_index(bits, lanes)} => (if Lane < {lanes // 2} then Left_Lanes ({lane_index(bits, lanes)} (2 * Lane + 1)) else Right_Lanes ({lane_index(bits, lanes)} (2 * (Lane - {lanes // 2}) + 1)))]);",
            "   begin",
            f"      Check (Same (Reverse_Lanes (Left), Reverse_Expected) and then Same (Backends.Scalar.Reverse_Lanes (Left), Reverse_Expected) and then Same (Backends.Native.Reverse_Lanes (Left), Reverse_Expected), \"{vector} independent root Scalar Native reverse\" & Label_Text);",
            f"      Check (Same (Interleave_Low (Left, Right), Interleave_Low_Expected) and then Same (Backends.Scalar.Interleave_Low (Left, Right), Interleave_Low_Expected) and then Same (Backends.Native.Interleave_Low (Left, Right), Interleave_Low_Expected), \"{vector} independent root Scalar Native interleave low\" & Label_Text);",
            f"      Check (Same (Interleave_High (Left, Right), Interleave_High_Expected) and then Same (Backends.Scalar.Interleave_High (Left, Right), Interleave_High_Expected) and then Same (Backends.Native.Interleave_High (Left, Right), Interleave_High_Expected), \"{vector} independent root Scalar Native interleave high\" & Label_Text);",
            f"      Check (Same (Deinterleave_Even (Left, Right), Deinterleave_Even_Expected) and then Same (Backends.Scalar.Deinterleave_Even (Left, Right), Deinterleave_Even_Expected) and then Same (Backends.Native.Deinterleave_Even (Left, Right), Deinterleave_Even_Expected), \"{vector} independent root Scalar Native deinterleave even\" & Label_Text);",
            f"      Check (Same (Deinterleave_Odd (Left, Right), Deinterleave_Odd_Expected) and then Same (Backends.Scalar.Deinterleave_Odd (Left, Right), Deinterleave_Odd_Expected) and then Same (Backends.Native.Deinterleave_Odd (Left, Right), Deinterleave_Odd_Expected), \"{vector} independent root Scalar Native deinterleave odd\" & Label_Text);",
            f"   end Check_Arrangements_{vector};",
            f"   procedure Test_{vector} is",
            ...
            *[
            ...
            "   begin",
            f"      Check_Complete_Memory_{vector} (To_Lanes (A), \" fixed\");",
            f"      Check_Arrangements_{vector} (Special_Lanes_1, Special_Lanes_2, \" special bits\");",
            *[
                f"      Check_Arrangements_{vector} (Special_Lanes_{group_index}, Special_Lanes_1, \" special bits {group_index}\");"
                for group_index in range(3, len(special_bits) + 1)
            ],
            *[
        ...
        for name in ("Add", "Subtract", "Multiply", "Divide", "Min_Number", "Max_Number", "Interleave_Low", "Interleave_High", "Deinterleave_Even", "Deinterleave_Odd"):
            lines.append(f"      Check (Same (Backends.Scalar.{name} (A, B), {name} (A, B)) and then Same (Backends.Native.{name} (A, B), {name} (A, B)), \"{vector} scalar and native {name}\");")
        lines += [
            f"      Check (Same (Backends.Native.Reverse_Lanes (A), Reverse_Lanes (A)), \"{vector} reverse\");",
            f"      Check (Same (Backends.Native.Permute_Lanes (A, Fixed_Map), Permute_Lanes (A, Fixed_Map)), \"{vector} native fixed lane permutation\");",
        ...
        lines += [
            ...
            f"         Check (Same (Backends.Native.Slide_Lanes_Toward_Low (A, Slide), Slide_Lanes_Toward_Low (A, Slide)) and then Same (Backends.Native.Slide_Lanes_Toward_High (A, Slide), Slide_Lanes_Toward_High (A, Slide)), \"{vector} native lane slides\" & Slide'Image);",
            f"         for Lane in {lane_index(bits, lanes)} loop",
            f"            Check (Bits_{vector} (Extract (Slide_Lanes_Toward_Low (A, Slide), Lane)) = (if Slide < {lanes} and then Lane < {lanes} - Slide then Bits_{vector} (Extract (A, {lane_index(bits, lanes)} (Lane + Slide))) else 0) and then Bits_{vector} (Backends.Native.Extract (Backends.Native.Slide_Lanes_Toward_Low (A, Slide), Lane)) = (if Slide < {lanes} and then Lane < {lanes} - Slide then Bits_{vector} (Extract (A, {lane_index(bits, lanes)} (Lane + Slide))) else 0), \"{vector} independent scalar and native slide toward low\" & Slide'Image & Lane'Image);",
            f"            Check (Bits_{vector} (Extract (Slide_Lanes_Toward_High (A, Slide), Lane)) = (if Slide < {lanes} and then Lane >= Slide then Bits_{vector} (Extract (A, {lane_index(bits, lanes)} (Lane - Slide))) else 0) and then Bits_{vector} (Backends.Native.Extract (Backends.Native.Slide_Lanes_Toward_High (A, Slide), Lane)) = (if Slide < {lanes} and then Lane >= Slide then Bits_{vector} (Extract (A, {lane_index(bits, lanes)} (Lane - Slide))) else 0), \"{vector} independent scalar and native slide toward high\" & Slide'Image & Lane'Image);",
            "         end loop;",
            "      end loop;",
            f"      Check (Same (Slide_Lanes_Toward_Low (A, Natural'Last), Zero) and then Same (Backends.Native.Slide_Lanes_Toward_Low (A, Natural'Last), Zero) and then Same (Slide_Lanes_Toward_High (A, Natural'Last), Zero) and then Same (Backends.Native.Slide_Lanes_Toward_High (A, Natural'Last), Zero), \"{vector} maximum-count lane slides\");",
            f"      for Lane in {lane_index(bits, lanes)} loop",
        ...
        for relation, name in enumerate(("Equal", "Less_Than", "Less_Equal", "Greater_Than", "Greater_Equal", "Unordered")):
            lines.append(f"      Check (To_Bit_Mask ({name} (A, B)) = Reference_Comparison_{vector} (A, B, {relation}) and then Backends.Scalar.To_Bit_Mask (Backends.Scalar.{name} (A, B)) = Reference_Comparison_{vector} (A, B, {relation}) and then Backends.Native.To_Bit_Mask (Backends.Native.{name} (A, B)) = Reference_Comparison_{vector} (A, B, {relation}), \"{vector} independent root, Scalar, and Native {name}\");")
        lines += [
            f"      Check (Same (Backends.Scalar.Select_Value (Backends.Scalar.Equal (A, B), A, B), Select_Value (Equal (A, B), A, B)) and then Same (Backends.Native.Select_Value (Equal (A, B), A, B), Select_Value (Equal (A, B), A, B)), \"{vector} scalar and native select\");",
            f"      for Pattern in Natural range 0 .. 2 ** {lanes} - 1 loop",
            ...
            f"         Check (Same (Backends.Scalar.Select_Value (Backends.Scalar.Mask_From_Bit_Mask (Interfaces.Unsigned_8 (Pattern)), A, B), Select_Value (Mask_From_Bit_Mask (Interfaces.Unsigned_8 (Pattern)), A, B)) and then Same (Backends.Native.Select_Value (Backends.Native.Mask_From_Bit_Mask (Interfaces.Unsigned_8 (Pattern)), A, B), Select_Value (Mask_From_Bit_Mask (Interfaces.Unsigned_8 (Pattern)), A, B)), \"{vector} exhaustive scalar and native select\" & Pattern'Image);",
            ...
            f"         for Lane in {lane_index(bits, lanes)} loop Check (Bits_{vector} (Extract (Select_Value (Mask_From_Bit_Mask (Interfaces.Unsigned_8 (Pattern)), A, B), Lane)) = Bits_{vector} ((if (Pattern / 2 ** Lane) mod 2 = 1 then Extract (A, Lane) else Extract (B, Lane))) and then Bits_{vector} (Backends.Scalar.Extract (Backends.Scalar.Select_Value (Backends.Scalar.Mask_From_Bit_Mask (Interfaces.Unsigned_8 (Pattern)), A, B), Lane)) = Bits_{vector} ((if (Pattern / 2 ** Lane) mod 2 = 1 then Extract (A, Lane) else Extract (B, Lane))) and then Bits_{vector} (Backends.Native.Extract (Backends.Native.Select_Value (Backends.Native.Mask_From_Bit_Mask (Interfaces.Unsigned_8 (Pattern)), A, B), Lane)) = Bits_{vector} ((if (Pattern / 2 ** Lane) mod 2 = 1 then Extract (A, Lane) else Extract (B, Lane))), \"{vector} independent bitwise root, Scalar, and Native select\" & Pattern'Image & Lane'Image); end loop;",
            "      end loop;",
            ...
            *[
            ...
            f"            Arrangement_Other_Lanes : constant {vals} := [for Lane in {lane_index(bits, lanes)} => Value_From_Bits_{vector} ({'Interfaces.Unsigned_32 (Next_U64 and 16#FFFF_FFFF#)' if bits == 32 else 'Next_U64'})];",
            f"            R_A : constant {vector} := From_Lanes (R_Lanes);",
            f"            R_B : constant {vector} := From_Lanes (Random_{vector}_Lanes);",
            f"            Arrangement_A : constant {vector} := From_Lanes (Memory_Lanes);",
            f"            Arrangement_B : constant {vector} := From_Lanes (Arrangement_Other_Lanes);",
            f"            Tail : constant {count} := {count} (Next_U64 mod {lanes + 1});",
            f"            Slide : constant Natural := Natural (Next_U64 mod {lanes + 3});",
            ...
            f"            R_Two_Source_Map : constant {two_source_lane_map(bits, lanes)} := Make_Two_Source_Lane_Map ([for Lane in {lane_index(bits, lanes)} => (if (Iteration + Lane) mod 2 = 0 then Select_Left_Lane ({lane_index(bits, lanes)} ((Iteration * 3 + Lane * 5) mod {lanes})) else Select_Right_Lane ({lane_index(bits, lanes)} ((Iteration * 3 + Lane * 5) mod {lanes})))]);",
            "         begin",
            f"            Check_Arrangements_{vector} (Memory_Lanes, Arrangement_Other_Lanes, \" raw random\" & Iteration'Image);",
            ...
            f"            Check (Same (Backends.Scalar.Add (R_A, R_B), Add (R_A, R_B)) and then Same (Backends.Native.Add (R_A, R_B), Add (R_A, R_B)) and then Same (Backends.Scalar.Subtract (R_A, R_B), Subtract (R_A, R_B)) and then Same (Backends.Native.Subtract (R_A, R_B), Subtract (R_A, R_B)) and then Same (Backends.Scalar.Multiply (R_A, R_B), Multiply (R_A, R_B)) and then Same (Backends.Native.Multiply (R_A, R_B), Multiply (R_A, R_B)) and then Same (Backends.Scalar.Divide (R_A, R_B), Divide (R_A, R_B)) and then Same (Backends.Native.Divide (R_A, R_B), Divide (R_A, R_B)), \"{vector} randomized scalar and native arithmetic\");",
            f"            Check (Same (Backends.Scalar.Min_Number (R_A, R_B), Min_Number (R_A, R_B)) and then Same (Backends.Native.Min_Number (R_A, R_B), Min_Number (R_A, R_B)) and then Same (Backends.Scalar.Max_Number (R_A, R_B), Max_Number (R_A, R_B)) and then Same (Backends.Native.Max_Number (R_A, R_B), Max_Number (R_A, R_B)), \"{vector} randomized scalar and native min/max\");",
            f"            Check (To_Bit_Mask (Equal (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 0) and then Backends.Scalar.To_Bit_Mask (Backends.Scalar.Equal (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 0) and then Backends.Native.To_Bit_Mask (Backends.Native.Equal (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 0) and then To_Bit_Mask (Less_Than (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 1) and then Backends.Scalar.To_Bit_Mask (Backends.Scalar.Less_Than (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 1) and then Backends.Native.To_Bit_Mask (Backends.Native.Less_Than (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 1) and then To_Bit_Mask (Less_Equal (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 2) and then Backends.Scalar.To_Bit_Mask (Backends.Scalar.Less_Equal (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 2) and then Backends.Native.To_Bit_Mask (Backends.Native.Less_Equal (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 2) and then To_Bit_Mask (Greater_Than (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 3) and then Backends.Scalar.To_Bit_Mask (Backends.Scalar.Greater_Than (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 3) and then Backends.Native.To_Bit_Mask (Backends.Native.Greater_Than (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 3) and then To_Bit_Mask (Greater_Equal (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 4) and then Backends.Scalar.To_Bit_Mask (Backends.Scalar.Greater_Equal (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 4) and then Backends.Native.To_Bit_Mask (Backends.Native.Greater_Equal (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 4) and then To_Bit_Mask (Unordered (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 5) and then Backends.Scalar.To_Bit_Mask (Backends.Scalar.Unordered (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 5) and then Backends.Native.To_Bit_Mask (Backends.Native.Unordered (R_A, R_B)) = Reference_Comparison_{vector} (R_A, R_B, 5), \"{vector} randomized independent root, Scalar, and Native comparisons\");",
            f"            Check (Same (Backends.Native.Reverse_Lanes (R_A), Reverse_Lanes (R_A)) and then Same (Backends.Native.Interleave_Low (R_A, R_B), Interleave_Low (R_A, R_B)) and then Same (Backends.Native.Interleave_High (R_A, R_B), Interleave_High (R_A, R_B)) and then Same (Backends.Native.Deinterleave_Even (R_A, R_B), Deinterleave_Even (R_A, R_B)) and then Same (Backends.Native.Deinterleave_Odd (R_A, R_B), Deinterleave_Odd (R_A, R_B)), \"{vector} randomized native permutations\");",
            f"            Check (Same (Backends.Native.Permute_Lanes (R_A, R_Map), Permute_Lanes (R_A, R_Map)), \"{vector} randomized native lane permutation\");",
            f"            Check (Same (Backends.Native.Permute_Lanes (R_A, R_B, R_Two_Source_Map), Permute_Lanes (R_A, R_B, R_Two_Source_Map)), \"{vector} randomized native two-source lane permutation\");",
            f"            Check (Same (Backends.Native.Slide_Lanes_Toward_Low (R_A, Slide), Slide_Lanes_Toward_Low (R_A, Slide)) and then Same (Backends.Native.Slide_Lanes_Toward_High (R_A, Slide), Slide_Lanes_Toward_High (R_A, Slide)), \"{vector} randomized native lane slides\");",
            ...
            f"            for Lane in {lane_index(bits, lanes)} loop",
            f"               Check (Bits_{vector} (Extract (Reverse_Lanes (Arrangement_A), Lane)) = Bits_{vector} (Memory_Lanes ({lane_index(bits, lanes)} ({lanes - 1} - Lane))) and then Bits_{vector} (Backends.Scalar.Extract (Backends.Scalar.Reverse_Lanes (Arrangement_A), Lane)) = Bits_{vector} (Memory_Lanes ({lane_index(bits, lanes)} ({lanes - 1} - Lane))) and then Bits_{vector} (Backends.Native.Extract (Backends.Native.Reverse_Lanes (Arrangement_A), Lane)) = Bits_{vector} (Memory_Lanes ({lane_index(bits, lanes)} ({lanes - 1} - Lane))), \"{vector} raw-bit root, Scalar, and Native reverse\" & Lane'Image);",
            f"               Check (Bits_{vector} (Extract (Interleave_Low (Arrangement_A, Arrangement_B), Lane)) = Bits_{vector} ((if Lane mod 2 = 0 then Memory_Lanes ({lane_index(bits, lanes)} (Lane / 2)) else Arrangement_Other_Lanes ({lane_index(bits, lanes)} (Lane / 2)))) and then Bits_{vector} (Backends.Scalar.Extract (Backends.Scalar.Interleave_Low (Arrangement_A, Arrangement_B), Lane)) = Bits_{vector} ((if Lane mod 2 = 0 then Memory_Lanes ({lane_index(bits, lanes)} (Lane / 2)) else Arrangement_Other_Lanes ({lane_index(bits, lanes)} (Lane / 2)))) and then Bits_{vector} (Backends.Native.Extract (Backends.Native.Interleave_Low (Arrangement_A, Arrangement_B), Lane)) = Bits_{vector} ((if Lane mod 2 = 0 then Memory_Lanes ({lane_index(bits, lanes)} (Lane / 2)) else Arrangement_Other_Lanes ({lane_index(bits, lanes)} (Lane / 2)))), \"{vector} raw-bit root, Scalar, and Native interleave low\" & Lane'Image);",
            f"               Check (Bits_{vector} (Extract (Interleave_High (Arrangement_A, Arrangement_B), Lane)) = Bits_{vector} ((if Lane mod 2 = 0 then Memory_Lanes ({lane_index(bits, lanes)} ({lanes // 2} + Lane / 2)) else Arrangement_Other_Lanes ({lane_index(bits, lanes)} ({lanes // 2} + Lane / 2)))) and then Bits_{vector} (Backends.Scalar.Extract (Backends.Scalar.Interleave_High (Arrangement_A, Arrangement_B), Lane)) = Bits_{vector} ((if Lane mod 2 = 0 then Memory_Lanes ({lane_index(bits, lanes)} ({lanes // 2} + Lane / 2)) else Arrangement_Other_Lanes ({lane_index(bits, lanes)} ({lanes // 2} + Lane / 2)))) and then Bits_{vector} (Backends.Native.Extract (Backends.Native.Interleave_High (Arrangement_A, Arrangement_B), Lane)) = Bits_{vector} ((if Lane mod 2 = 0 then Memory_Lanes ({lane_index(bits, lanes)} ({lanes // 2} + Lane / 2)) else Arrangement_Other_Lanes ({lane_index(bits, lanes)} ({lanes // 2} + Lane / 2)))), \"{vector} raw-bit root, Scalar, and Native interleave high\" & Lane'Image);",
            f"               Check (Bits_{vector} (Extract (Deinterleave_Even (Arrangement_A, Arrangement_B), Lane)) = Bits_{vector} ((if Lane < {lanes // 2} then Memory_Lanes ({lane_index(bits, lanes)} (2 * Lane)) else Arrangement_Other_Lanes ({lane_index(bits, lanes)} (2 * (Lane - {lanes // 2}))))) and then Bits_{vector} (Backends.Scalar.Extract (Backends.Scalar.Deinterleave_Even (Arrangement_A, Arrangement_B), Lane)) = Bits_{vector} ((if Lane < {lanes // 2} then Memory_Lanes ({lane_index(bits, lanes)} (2 * Lane)) else Arrangement_Other_Lanes ({lane_index(bits, lanes)} (2 * (Lane - {lanes // 2}))))) and then Bits_{vector} (Backends.Native.Extract (Backends.Native.Deinterleave_Even (Arrangement_A, Arrangement_B), Lane)) = Bits_{vector} ((if Lane < {lanes // 2} then Memory_Lanes ({lane_index(bits, lanes)} (2 * Lane)) else Arrangement_Other_Lanes ({lane_index(bits, lanes)} (2 * (Lane - {lanes // 2}))))), \"{vector} raw-bit root, Scalar, and Native deinterleave even\" & Lane'Image);",
            f"               Check (Bits_{vector} (Extract (Deinterleave_Odd (Arrangement_A, Arrangement_B), Lane)) = Bits_{vector} ((if Lane < {lanes // 2} then Memory_Lanes ({lane_index(bits, lanes)} (2 * Lane + 1)) else Arrangement_Other_Lanes ({lane_index(bits, lanes)} (2 * (Lane - {lanes // 2}) + 1)))) and then Bits_{vector} (Backends.Scalar.Extract (Backends.Scalar.Deinterleave_Odd (Arrangement_A, Arrangement_B), Lane)) = Bits_{vector} ((if Lane < {lanes // 2} then Memory_Lanes ({lane_index(bits, lanes)} (2 * Lane + 1)) else Arrangement_Other_Lanes ({lane_index(bits, lanes)} (2 * (Lane - {lanes // 2}) + 1)))) and then Bits_{vector} (Backends.Native.Extract (Backends.Native.Deinterleave_Odd (Arrangement_A, Arrangement_B), Lane)) = Bits_{vector} ((if Lane < {lanes // 2} then Memory_Lanes ({lane_index(bits, lanes)} (2 * Lane + 1)) else Arrangement_Other_Lanes ({lane_index(bits, lanes)} (2 * (Lane - {lanes // 2}) + 1)))), \"{vector} raw-bit root, Scalar, and Native deinterleave odd\" & Lane'Image);",
            ...
            f"               Check (Bits_{vector} (Extract (Add (R_A, R_B), Lane)) = Bits_{vector} (Extract (R_A, Lane) + Extract (R_B, Lane)) and then Bits_{vector} (Extract (Backends.Scalar.Add (R_A, R_B), Lane)) = Bits_{vector} (Extract (R_A, Lane) + Extract (R_B, Lane)) and then Bits_{vector} (Extract (Backends.Native.Add (R_A, R_B), Lane)) = Bits_{vector} (Extract (R_A, Lane) + Extract (R_B, Lane)) and then Bits_{vector} (Extract (Subtract (R_A, R_B), Lane)) = Bits_{vector} (Extract (R_A, Lane) - Extract (R_B, Lane)) and then Bits_{vector} (Extract (Backends.Scalar.Subtract (R_A, R_B), Lane)) = Bits_{vector} (Extract (R_A, Lane) - Extract (R_B, Lane)) and then Bits_{vector} (Extract (Backends.Native.Subtract (R_A, R_B), Lane)) = Bits_{vector} (Extract (R_A, Lane) - Extract (R_B, Lane)) and then Bits_{vector} (Extract (Multiply (R_A, R_B), Lane)) = Bits_{vector} (Extract (R_A, Lane) * Extract (R_B, Lane)) and then Bits_{vector} (Extract (Backends.Scalar.Multiply (R_A, R_B), Lane)) = Bits_{vector} (Extract (R_A, Lane) * Extract (R_B, Lane)) and then Bits_{vector} (Extract (Backends.Native.Multiply (R_A, R_B), Lane)) = Bits_{vector} (Extract (R_A, Lane) * Extract (R_B, Lane)), \"{vector} randomized independent root, scalar, and native arithmetic\" & Lane'Image);",
            f"               if Extract (R_B, Lane) /= 0.0 then Check (Bits_{vector} (Extract (Divide (R_A, R_B), Lane)) = Bits_{vector} (Extract (R_A, Lane) / Extract (R_B, Lane)) and then Bits_{vector} (Extract (Backends.Scalar.Divide (R_A, R_B), Lane)) = Bits_{vector} (Extract (R_A, Lane) / Extract (R_B, Lane)) and then Bits_{vector} (Extract (Backends.Native.Divide (R_A, R_B), Lane)) = Bits_{vector} (Extract (R_A, Lane) / Extract (R_B, Lane)), \"{vector} randomized independent root, scalar, and native division\" & Lane'Image); end if;",
            f"               Check (Test (Equal (R_A, R_B), Lane) = (Extract (R_A, Lane) = Extract (R_B, Lane)) and then Test (Less_Than (R_A, R_B), Lane) = (Extract (R_A, Lane) < Extract (R_B, Lane)) and then Test (Less_Equal (R_A, R_B), Lane) = (Extract (R_A, Lane) <= Extract (R_B, Lane)) and then Test (Greater_Than (R_A, R_B), Lane) = (Extract (R_A, Lane) > Extract (R_B, Lane)) and then Test (Greater_Equal (R_A, R_B), Lane) = (Extract (R_A, Lane) >= Extract (R_B, Lane)), \"{vector} randomized independent comparison\" & Lane'Image);",
            f"               Check (Extract (Min_Number (R_A, R_B), Lane) = (if Extract (R_A, Lane) <= Extract (R_B, Lane) then Extract (R_A, Lane) else Extract (R_B, Lane)) and then Extract (Backends.Scalar.Min_Number (R_A, R_B), Lane) = (if Extract (R_A, Lane) <= Extract (R_B, Lane) then Extract (R_A, Lane) else Extract (R_B, Lane)) and then Extract (Backends.Native.Min_Number (R_A, R_B), Lane) = (if Extract (R_A, Lane) <= Extract (R_B, Lane) then Extract (R_A, Lane) else Extract (R_B, Lane)) and then Extract (Max_Number (R_A, R_B), Lane) = (if Extract (R_A, Lane) >= Extract (R_B, Lane) then Extract (R_A, Lane) else Extract (R_B, Lane)) and then Extract (Backends.Scalar.Max_Number (R_A, R_B), Lane) = (if Extract (R_A, Lane) >= Extract (R_B, Lane) then Extract (R_A, Lane) else Extract (R_B, Lane)) and then Extract (Backends.Native.Max_Number (R_A, R_B), Lane) = (if Extract (R_A, Lane) >= Extract (R_B, Lane) then Extract (R_A, Lane) else Extract (R_B, Lane)), \"{vector} randomized independent root, scalar, and native min/max\" & Lane'Image);",
            "            end loop;",
            "         end;",
            "      end loop;",
            f"   end Test_{vector};", "",
