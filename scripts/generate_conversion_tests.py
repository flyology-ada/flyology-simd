...
def emit_bit_cast_test(source: str, source_scalar: str, target: str, target_scalar: str) -> list[str]:
    ...
    source_expression = (
        f"Random_Convert_{source}" if source in {"F32x4", "F64x2"}
        else f"Random_{source}"
    )
    return [
        "      declare",
        f"         Source : constant {source} := {source_expression};",
        f"         Root_Result : constant {target} := Bit_Cast (Source);",
        f"         Scalar_Result : constant {target} := Backends.Scalar.Bit_Cast (Source);",
        f"         Native_Result : constant {target} := Backends.Native.Bit_Cast (Source);",
        "      begin",
        f"         for Lane in {lane_index(bits, lanes)} loop",
        f"            Check (Same (Extract (Root_Result, Lane), Cast_{source_scalar}_To_{target_scalar} (Extract (Source, Lane))), \"root {source} to {target} bit cast lane\");",
        f"            Check (Same (Extract (Scalar_Result, Lane), Cast_{source_scalar}_To_{target_scalar} (Extract (Source, Lane))), \"Backends.Scalar {source} to {target} bit cast lane\");",
...
def emit_widen_test(source: str, source_scalar: str, target: str, target_scalar: str, result_lanes: int) -> list[str]:
    return [
        ...
        f"         Root_Low : constant {target} := Widen_Low (Source);",
        f"         Root_High : constant {target} := Widen_High (Source);",
        f"         Scalar_Low : constant {target} := Backends.Scalar.Widen_Low (Source);",
        f"         Scalar_High : constant {target} := Backends.Scalar.Widen_High (Source);",
        ...
        f"            Check (Extract (Root_Low, Lane) = {target_scalar} (Extract (Source, Lane)), \"root {source} low widening lane\");",
        f"            Check (Extract (Scalar_Low, Lane) = {target_scalar} (Extract (Source, Lane)), \"Backends.Scalar {source} low widening lane\");",
        f"            Check (Extract (Native_Low, Lane) = {target_scalar} (Extract (Source, Lane)), \"native {source} low widening lane\");",
        f"            Check (Extract (Root_High, Lane) = {target_scalar} (Extract (Source, Lane + {result_lanes})), \"root {source} high widening lane\");",
        f"            Check (Extract (Scalar_High, Lane) = {target_scalar} (Extract (Source, Lane + {result_lanes})), \"Backends.Scalar {source} high widening lane\");",
...
def emit_narrow_test(
    ...
    root_low = comparison(
        "Extract (Root_Result, Lane)", f"{helper} (Extract (Low, Lane))"
    )
    ...
    root_high = comparison(
        f"Extract (Root_Result, Lane + {source_lanes})",
    ...
    return [
        ...
        f"         Root_Result : constant {target} := {name} (Low, High);",
        f"         Scalar_Result : constant {target} := Backends.Scalar.{name} (Low, High);",
        f"         Native_Result : constant {target} := Backends.Native.{name} (Low, High);",
        "      begin",
        f"         for Lane in Natural range 0 .. {source_lanes - 1} loop",
        f"            Check ({root_low}, \"root {name} {source} low lane\");",
        f"            Check ({scalar_low}, \"Backends.Scalar {name} {source} low lane\");",
        f"            Check ({native_low}, \"native {name} {source} low lane\");",
        f"            Check ({root_high}, \"root {name} {source} high lane\");",
        f"            Check ({scalar_high}, \"Backends.Scalar {name} {source} high lane\");",
...
def emit_convert_round_test(source: str, target: str, bits: int, lanes: int) -> list[str]:
    return [
        "      declare",
        f"         Source : constant {source} := Random_{source};",
        f"         Root_Result : constant {target} := Convert_Round (Source);",
        f"         Scalar_Result : constant {target} := Backends.Scalar.Convert_Round (Source);",
        f"         Native_Result : constant {target} := Backends.Native.Convert_Round (Source);",
        "      begin",
        f"         for Lane in {lane_index(bits, lanes)} loop",
        f"            Check (Same (Extract (Root_Result, Lane), Oracle_Convert_Round_{source}_To_{target} (Extract (Source, Lane))), \"root Convert_Round {source} lane\");",
        f"            Check (Same (Extract (Scalar_Result, Lane), Oracle_Convert_Round_{source}_To_{target} (Extract (Source, Lane))), \"Backends.Scalar Convert_Round {source} lane\");",
...
def emit_float_to_integer_test(source: str, target: str, bits: int, lanes: int) -> list[str]:
    return [
        ...
        f"         Root_Result : constant {target} := Convert_Truncate_Saturate (Source);",
        f"         Scalar_Result : constant {target} := Backends.Scalar.Convert_Truncate_Saturate (Source);",
        f"         Native_Result : constant {target} := Backends.Native.Convert_Truncate_Saturate (Source);",
        "      begin",
        f"         for Lane in {lane_index(bits, lanes)} loop",
        f"            Check (Extract (Root_Result, Lane) = Oracle_Convert_Truncate_Saturate_{source}_To_{target} (Extract (Source, Lane)), \"root Convert_Truncate_Saturate {source} lane\");",
        f"            Check (Extract (Scalar_Result, Lane) = Oracle_Convert_Truncate_Saturate_{source}_To_{target} (Extract (Source, Lane)), \"Backends.Scalar Convert_Truncate_Saturate {source} lane\");",
...
def emit_signed_unsigned_test(
    ...
    return [
        "      declare",
        f"         Source : constant {source} := {source_expression};",
        f"         Root_Result : constant {target} := Convert_Saturate (Source);",
        f"         Scalar_Result : constant {target} := Backends.Scalar.Convert_Saturate (Source);",
        f"         Native_Result : constant {target} := Backends.Native.Convert_Saturate (Source);",
        "      begin",
        f"         for Lane in {lane_index(bits, lanes)} loop",
        f"            Check (Extract (Root_Result, Lane) = Oracle_Convert_Saturate_{source}_To_{target} (Extract (Source, Lane)), \"root Convert_Saturate {source} lane\");",
        f"            Check (Extract (Scalar_Result, Lane) = Oracle_Convert_Saturate_{source}_To_{target} (Extract (Source, Lane)), \"Backends.Scalar Convert_Saturate {source} lane\");",
...
def emit_signed_unsigned_literal_test(
    ...
    return [
        ...
        "      begin",
        f"         for Lane in {lane_index(bits, lanes)} loop",
        f"            Check (Extract (Root_Result, Lane) = Expected (Lane), \"root literal Convert_Saturate {source} lane\");",
        f"            Check (Extract (Scalar_Result, Lane) = Expected (Lane), \"Backends.Scalar literal Convert_Saturate {source} lane\");",
