---
description: Apply Flyology SIMD's documentation rules within the website tree.
applyTo: "website/**"
---

# Flyology SIMD website documentation guide

## Scope

Apply the technical writing style to hand-written documentation in \`guide/**\`,
\`architecture/**\`, and \`support/**\`. Apply the journal register below to
\`journal/**\`. These rules do not apply to the home page, reports, or generated
API reference.
...
- Define an unfamiliar term before its first use. Keep Flyology SIMD
  identifiers, Ada terms, compiler terms, and instruction-set names exact.
...
On each Guide, Architecture, Support, or Journal page, link the first visible
explanatory mention of a public Flyology SIMD API entity to its generated
...
- Do not link Ada language constructs, compiler internals, instruction names,
  environment variables, shell commands, scripts, or external APIs to Flyology
  SIMD GNATdoc.
- When no generated entry exists for a public Flyology SIMD identifier, treat
...
- Compare the rewrite with the implementation, tests, scripts, and root
  \`AGENTS.md\` invariants.
- Check that each page links the first explanatory mention of every public
  Flyology SIMD API entity to the correct generated GNATdoc entry.
- Check every semantic condition, ownership rule, exception, dispatch boundary,
  alignment rule, extent rule, compiler limit, and experimental qualification.
